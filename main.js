...
function applyLang(){
  ...
  // Filter group labels (labeled toolbar redesign)
  const _filterLabels = document.querySelectorAll('.filter-label');
  if(_filterLabels[0]) _filterLabels[0].textContent = isEn ? 'Country'   : 'País';
  if(_filterLabels[1]) _filterLabels[1].textContent = isEn ? 'Format'    : 'Formato';
  if(_filterLabels[2]) _filterLabels[2].textContent = isEn ? 'Sort by'   : 'Ordenar por';
  // Security btn
