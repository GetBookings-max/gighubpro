...
function _updateHeroCount(){
  ...
  if(!total) return;
  // Count only platforms that appear in CAT_EXPLORER_DEFS (what the user actually sees)
  // so that the hero total matches the sum of category counts.
  const _pNamesInDefs = new Set();
  CAT_EXPLORER_DEFS.forEach(cat => {
    (cat.platforms || []).forEach(p => _pNamesInDefs.add(p.name.toLowerCase().trim()));
  });
  const _visibleCount = P.filter(p => _pNamesInDefs.has(p.name.toLowerCase().trim())).length;
  const displayTotal = _visibleCount || total;

  // Round down to nearest 10, show with "+" prefix (e.g. 87 → "+100")
  const rounded = displayTotal >= 100 ? '+100' : ('+' + (Math.floor(displayTotal / 10) * 10));
...
const _NAME_OVERRIDES = {
  ...
  'NOS Cinemas Recrutamento':   'NOS Cinemas',
  // DB names que diferem do nome no CAT_EXPLORER_DEFS
  'AVA Clinic Doação':          'Ava Clinic',
  'Aqualand Portugal':          'Aqualand',
  'Aquashow Park':              'Aquashow',
  'Booking.com Afiliados':      'Booking.com Affiliates',
  'Brisa Recrutamento':         'Brisa',
  'Campanha Tomate Portugal':   'Campanha Tomate',
  'Crowd Agência de Talentos':  'Crowd',
  'Fujitsu Careers PT':         'Fujitsu Careers',
  'Fundação INATEL':            'INATEL',
  'IKEA Portugal':              'IKEA',
  'Leroy Merlin Portugal':      'Leroy Merlin',
  'Makro Portugal':             'Makro',
  'Midas Portugal':             'Midas',
  'Monitores JM Oeiras':        'Monitores Oeiras',
  'PRIO Carreiras':             'Prio',
  'PRIO Recrutamento':          'Prio',
  'Payshop Agente':             'Payshop',
  'ProCME Recrutamento':        'Procme',
  'Ryanair Careers':            'Ryanair',
  'Shopify Afiliados':          'Shopify Affiliates',
  'Solverde Recrutamento':      'Solverde',
  'Sonae Emprego':              'Sonae',
  'Temu Afiliados':             'Temu Affiliates',
  'Trade Republic Afiliados':   'Trade Republic Affiliates',
  'Transdev Portugal':          'Transdev',
  'Vinted Go Parceiro':         'Vinted Go Partner',
  'Zoomarine Recrutamento':     'Zoomarine',
};
...
(function _bindEvents(){
  ...
  if(hoursRangeEl) hoursRangeEl.addEventListener('input', calcEarnings);
  ...
  // ── Cat explorer card grid (click/keydown delegation) ──
  const catGridEl = document.getElementById('catGrid');
  if(catGridEl) {
    catGridEl.addEventListener('click', e => {
      const card = e.target.closest('.cat-card[data-catid]');
      if(card) _toggleCatCard(card.dataset.catid);
    });
    catGridEl.addEventListener('keydown', e => {
      if(e.key === 'Enter' || e.key === ' ') {
        const card = e.target.closest('.cat-card[data-catid]');
        if(card) { e.preventDefault(); _toggleCatCard(card.dataset.catid); }
      }
    });
  }
    ...
    subgroups: [
      ...
      },
      {
        ...
        platforms: [
          ...
          { name:'Quick Casting',  desc:'Casting rápido para figurantes, promotores e staff de eventos', url:'https://www.quickcasting.pt', icon:'🎬' },
    ...
    subgroups: [
      {
        ...
        platforms: [
          ...
          { name:'Ponto CTT',          desc:'Agente CTT — serviços postais e financeiros em loja', url:'https://www.ctt.pt/agentes', icon:'📮' },
          { name:'Western Union Agente', desc:'Agente de remessas internacionais — instalação em loja', url:'https://www.westernunion.com/pt/pt/agentlocator.html', icon:'💸' },
        ]
      },
      {
      ...
      },
      {
        ...
        platforms: [
          ...
          { name:'Revolut Afiliados',  desc:'Programa de afiliados Revolut — comissão por referências', url:'https://www.revolut.com/referral', icon:'💜' },
          { name:'XTB Afiliados',      desc:'Programa de parceiros XTB — comissão por referências de clientes de trading', url:'https://www.xtb.com/pt/parceiros', icon:'📈' },
        ]
      },
    ]
  },
    ...
    subgroups: [
      {
        ...
        platforms: [
          ...
          { name:'EM Estudos de Mercado', desc:'Painel de consumidores português — inquéritos online pagos', url:'https://www.em-estudosdemercado.com', icon:'📊' },
          { name:'Lootup',            desc:'Plataforma de recompensas por surveys, ofertas e jogos', url:'https://lootup.me', icon:'🎮' },
      ...
      },
      {
        ...
        platforms: [
          ...
          { name:'Pontis',            desc:'Cliente mistério e auditorias de qualidade em Portugal', url:'https://www.pontis.pt', icon:'🔎' },
          { name:'Sograma',           desc:'Estudos de mercado, cliente mistério e auditorias em PT', url:'https://www.sograma.pt', icon:'🔎' },
        ]
      },
    ]
  },
    ...
    subgroups: [
      {
        ...
        platforms: [
          ...
          { name:'Pitagorica',       desc:'Contact center e BPO — atendimento ao cliente e suporte técnico', url:'https://www.pitagorica.pt/emprego', icon:'📞' },
        ]
      },
      {
        ...
        platforms: [
          ...
          { name:'Mercadona',          desc:'Supermercado — operador de loja, caixa e reposição', url:'https://www.mercadona.pt/en-es/trabaja-con-nosotros-pt', icon:'🛒' },
          ...
          { name:'Eurest',             desc:'Restauração empresarial — colaborador de refeitório e cozinha', url:'https://www.eurest.pt/carreiras', icon:'🍽️' },
          ...
          { name:'Cinema City',        desc:'Staff de cinema — bilheteira, sala e serviços de alimentação', url:'https://www.cinema-city.pt/about-us/jobs', icon:'🎬' },
          { name:'NOS Cinemas',        desc:'Staff de cinema — bilheteira e sala', url:'https://www.nos.pt/particulares/a-nos/trabalha-connosco/', icon:'🎬' },
          { name:'Prio',               desc:'Postos de combustível e lojas de conveniência', url:'https://www.prioenergy.pt/carreiras/', icon:'⛽' },
          { name:'Grupo Primor',       desc:'Lojas de cosmética e perfumaria — vendedor e gestor de loja', url:'https://www.primor.eu/emprego/', icon:'🛍️' },
          { name:'Newrest',            desc:'Catering e restauração em aeroportos e meios de transporte', url:'https://www.newrest.eu/pt/carreiras/', icon:'✈️' },
          { name:'Jardim Zoológico',   desc:'Staff de zoo — guarda, receção, animação e serviços de alimentação', url:'https://www.zoo.pt/recrutamento', icon:'🦁' },
        ]
      },
      {
      ...
      },
      {
        ...
        platforms: [
          ...
          { name:'Solverde',             desc:'Staff de casino e hotelaria — dealer, croupier e operações', url:'https://www.solverde.pt/emprego', icon:'🃏' },
          { name:'Pestana Group',        desc:'Hotelaria e resorts — receção, restauração e operações', url:'https://www.pestana.com/pt/carreiras', icon:'🏨' },
          ...
          { name:'Quantum Parks',        desc:'Staff de parque de lazer e entretenimento', url:'https://www.quantumparks.pt/emprego', icon:'🎡' },
          ...
          { name:'Living Tours Group',   desc:'Guias e staff de tours a pé, de bicicleta e culturais em Lisboa e Porto', url:'https://www.livingtours.com/careers', icon:'🚶' },
        ]
      },
      {
        ...
        platforms: [
          ...
          { name:'Procme',              desc:'Recrutamento para trabalhos operacionais — limpeza, segurança e suporte', url:'https://www.procme.pt/recrutamento', icon:'🧹' },
          ...
          { name:'JF Ajuda Emprega',    desc:'Bolsa de emprego da Junta de Freguesia de Ajuda — oportunidades locais de trabalho em Lisboa', url:'https://www.jf-ajuda.pt/emprego', icon:'🏛️' },
        ]
      },
      {
        ...
        platforms: [
          ...
          { name:'ACNUR',               desc:'Angariador de fundos para o Alto Comissariado das Nações Unidas para os Refugiados (F2F)', url:'https://www.acnur.org/trabalha-connosco', icon:'🕊️' },
          { name:'Aldeias SOS',          desc:'Angariador de fundos para a Aldeias de Crianças SOS Portugal (F2F)', url:'https://www.aldeiassos.pt/trabalha-connosco', icon:'🏡' },
          ...
          { name:'Metlife Portugal',     desc:'Agente de seguros — trabalho por comissões', url:'https://www.metlife.pt/sobre-a-metlife/carreiras/', icon:'🛡️' },
          { name:'ERA Portugal',         desc:'Agente imobiliário — comissões por venda e arrendamento', url:'https://www.era.pt/trabalha-connosco/', icon:'🏡' },
          { name:'Boost Portugal',       desc:'Guia turístico, promotor e staff de eventos', url:'https://www.boostportugal.pt/emprego/', icon:'🎤' },
        ]
      },
    ]
  },
...
function renderCatExplorer() {
  ...
  function _platformPasses(p) {
    ...
    if(!dbP) return false; // not in DB/allowlist — don't count or show
