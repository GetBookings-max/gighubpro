...
function calcEarnings(){
  ...
  const sPT={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats ou Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.pt.',10:'99designs + Freelancer.com (projetos específicos).',18:'UserInterviews (disponibilidade muito limitada em PT).'};
  const sEN={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats or Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.',10:'99designs + Freelancer.com (specific projects).',18:'UserInterviews (very limited availability in PT).'};
...
const translations = {
  ...
  // GigBoost
    ...
    calcOpt18: 'Entrevistas UX / Estudos de utilizador (muito seletivo, PT limitado)',
  ...
  // GigBoost
    ...
    calcOpt8: 'Pet Sitting / Babysitting (€8/h)',
    calcOpt18: 'UX Interviews / User studies (very selective, limited PT)',
...
const _BLOCKED = new Set([
  ...
  // ── Removidas do site (eram dimmed) ──
  'Dynamite Jobs','FlexJobs','Wellfound','Working Nomads','Arc.dev',
  'Remotive','Fiverr','Freelancer','Upwork','Braintrust','Remote.com',
  'Respondent.io','Respondent',
  // ── Removidas: testes de utilizador ──
  'Usertesting','UserTesting','Trymata',
  // ── Dedup: Sua Opinião Conta vem de _EXTRA_ENTRIES ──
  'Sua Opinião Conta'
]);

// ── Secção 2 — Dimmed set cleared: all previously dimmed platforms are now removed or shown normally ──
...
function _mergeExtra(){
  ...
  const toAdd = _EXTRA_ENTRIES.filter(e=>!existing.has(norm(e.name)));
