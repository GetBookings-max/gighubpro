...
async function checkAccess(){
  ...
  if(urlToken){ await validarTokenSupabase(urlToken); return; }
  showPasswordMode();
  applyLockLang();
}
...
async function unlock(){
  ...
  await validarTokenSupabase(v.trim().replace(/[^a-zA-Z0-9_\-]/g,'').substring(0,64));
...
function calcEarnings(){
  ...
  const sPT={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats ou Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.pt.',10:'Braintrust + Arc.dev.',18:'Respondent.io + UserInterviews (disponibilidade limitada em PT).'};
  const sEN={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats or Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.',10:'Braintrust + Arc.dev.',18:'Respondent.io + UserInterviews (limited availability in PT).'};
...
function easyBar(n){
  ...
  // Color the filled dots by difficulty level
...
function render(){
  ...
  const topCount = list.filter(p=>p.top).length;
  ...
  const _curationTitles={
    pt:{portugal:'Top picks Portugal 🇵🇹',beginners:'Boas para iniciantes 🌱',bestpay:'Melhores pagamentos 💰',fastest:'Payout rápido ⚡',noexp:'Sem experiência necessária 🚀'},
    en:{portugal:'Top picks Portugal 🇵🇹',beginners:'Good for beginners 🌱',bestpay:'Best payouts 💰',fastest:'Fast payout ⚡',noexp:'No experience needed 🚀'}
  };
  ...
  grid.innerHTML=list.map((p,i)=>{
    ...
    const cardClass = p.top?'top':(p.dimmed?'dimmed':(p.beginner && p.cat!=='f2f'?'beginner-pick':''));
    return `
...
const translations = {
  pt: {
    ...
    sortEasy: '✓ Mais fácil',
    sortName: 'A–Z Nome',
    heroTag: '✅ 130+ plataformas verificadas · Atualizado',
    ...
    lockHeadline: '130+ plataformas<br><em>verificadas</em> num só lugar',
    ...
    lockIncluiDesc: '130+ plataformas<br>Acesso de longa duração',
    lockStep1Label: 'Passo 1 — Pagar &amp; Pedir acesso',
    lockStep1Desc: 'Envia <strong>14,99€ via MB Way</strong> para <strong>938 556 803</strong> e clica no botão. Recebes o código em minutos.',
    ...
    welcomeBody: 'Tens acesso a <strong style="color:var(--ink)">130+ plataformas verificadas</strong> para ganhar dinheiro online — surveys, freelance, IA, gigs físicos e muito mais.<br><br>Usa os filtros para encontrar o que funciona para ti. Começa pelas marcadas como <strong style="color:var(--green)">TOP PICK</strong>.',
    ...
    guideTip: '<strong>★ Top 3 para Portugal em 2026 —</strong> <strong>DataAnnotation.tech</strong> (treino de IA, sem entrevista inicial, boa disponibilidade PT) · <strong>Prolific</strong> (surveys académicos, muito fiável, payout rápido) · <strong>Respondent.io</strong> (entrevistas UX em inglês, remuneração elevada mas acesso limitado e seletivo para PT)',
    // Monetization
    monoTitle: 'Partilha o acesso com alguém?',
    monoDesc: 'Envia 14,99€ por MB Way para <strong>938 556 803</strong> e envia o comprovativo via WhatsApp. Recebes o código em minutos.',
  ...
  // GigBoost
    ...
    calcOpt18: 'Entrevistas UX / Respondent (muito seletivo, PT limitado)',
  ...
  en: {
    ...
    sortName: 'A–Z Name',
    heroTag: '✅ 130+ verified platforms · Updated',
    ...
    lockHeadline: '130+ verified<br><em>platforms</em> in one place',
    ...
    welcomeBody: 'You have access to <strong style="color:var(--ink)">115 verified platforms</strong> to earn money online — surveys, freelance, AI, physical gigs and much more.<br><br>Use the filters to find what works for you. Start with those marked as <strong style="color:var(--green)">TOP PICK</strong>.',
    ...
    guideTip: '<strong>★ Top 3 for 2026 —</strong> <strong>DataAnnotation.tech</strong> (AI training, no initial interview, good PT availability) · <strong>Prolific</strong> (academic surveys, very reliable, fast payout) · <strong>Respondent.io</strong> (UX interviews in English, high pay but limited and selective access for PT users)',
    // Monetization
    monoTitle: 'Share access with someone?',
    monoDesc: 'Send €14.99 via MB Way to <strong>938 556 803</strong> and share the receipt via WhatsApp. You\'ll receive the code in minutes.',
    ...
    lockIncluiDesc: '130+ platforms<br>Long-term access',
    lockStep1Label: 'Step 1 — Pay &amp; Request Access',
    lockStep1Desc: 'Send <strong>€14.99 via MB Way</strong> to <strong>938 556 803</strong> and click the button. You will receive the code in minutes.',
  ...
  // GigBoost
    ...
    calcOpt8: 'Pet Sitting / Babysitting (€8/h)',
    calcOpt18: 'UX Interviews / Respondent (very selective, limited PT)',
...
function applyLockLang(){
  ...
  setText('lockIncluiDesc', '130+ plataformas · Acesso de longa duração', '130+ platforms · Long-term access');
  // Step labels
  ...
  if(desc1) desc1.innerHTML = isEn
    ? 'Send <strong>€14.99 via MB Way</strong> to <strong>938 556 803</strong> and click the button. You will receive the code in minutes.'
    : 'Envia <strong>14,99€ via MB Way</strong> para <strong>938 556 803</strong> e clica no botão. Recebes o código em minutos.';
  ...
  setText('lockHint', t.lockHintText || 'Acesso enviado pelo autor da plataforma', t.lockHintText || 'Access code sent by the platform author');
  ...
  if(abuseEl) abuseEl.innerHTML = isEn
    ? '🛡️ Access is personal and non-transferable. For security purposes, access activity per token may be monitored. Shared or abused tokens may be permanently revoked. See <a href="#" data-modal="terms" style="color:rgba(247,245,240,.45);text-decoration:underline">Terms of Use</a>.'
    : '🛡️ O acesso é pessoal e intransmissível. Por razões de segurança, a atividade de cada token pode ser monitorizada. Tokens partilhados ou com uso abusivo poderão ser permanentemente revogados. Ver <a href="#" data-modal="terms" style="color:rgba(247,245,240,.45);text-decoration:underline">Termos de Utilização</a>.';
...
function renderBoostResults(r){
  ...
  box.innerHTML = `
    ...
    <div class="boost-results">
      <div class="boost-badge" style="margin-bottom:16px">✨ GigBoost · ${isEn ? 'Analysis complete' : 'Análise concluída'}</div>

      <!-- Persona card -->
      <div class="boost-persona">
        <div class="boost-persona-emoji">${escHtml(r.persona?.emoji||'🎯')}</div>
        <div class="boost-persona-type">${escHtml(r.persona?.tipo||( isEn ? 'Personalised profile' : 'Perfil personalizado'))}</div>
        <div class="boost-persona-name">${escHtml(r.persona?.nome||(isEn ? 'Your profile' : 'O teu perfil'))}</div>
        ...
        <div class="boost-earn-range">
          ...
          <div class="boost-earn-label">${escHtml(r.ganhos_estimados?.label||(isEn ? 'estimated' : 'estimado'))}</div>
        </div>
      </div>

      <!-- Top platforms -->
      <div class="boost-section-title">🏆 ${isEn ? 'Your 5 ideal platforms' : 'As tuas 5 plataformas ideais'}</div>
      ${platformsHtml}

      <!-- Tips -->
      <div class="boost-section-title">💡 ${isEn ? 'Tips for your profile' : 'Dicas para o teu perfil'}</div>
      ${tipsHtml}

      <!-- Action plan -->
      <div class="boost-section-title">📅 ${isEn ? 'Your plan for the next 7 days' : 'O teu plano para os próximos 7 dias'}</div>
      <div class="boost-action-card">
        <div class="boost-action-title">${isEn ? 'Start tomorrow' : 'Começa amanhã'}</div>
        ${planHtml}
      </div>

      <div style="margin-top:20px;display:flex;gap:10px;flex-wrap:wrap">
        <button class="boost-redo-btn" style="flex:1;height:40px;border-radius:10px;border:1.5px solid var(--border-md);background:transparent;font-family:'Instrument Sans',sans-serif;font-size:12px;font-weight:600;cursor:pointer;color:var(--grey)">↺ ${isEn ? 'Redo analysis' : 'Refazer análise'}</button>
        <button class="boost-explore-btn" style="flex:2;height:40px;border-radius:10px;border:none;background:var(--ink);color:var(--paper);font-family:'Instrument Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">${isEn ? 'Explore platforms →' : 'Explorar plataformas →'}</button>
      </div>
    </div>`;
...
// Platforms not suitable for PT users — removed from display
const _BLOCKED = new Set([
  'Hostbreak','Billo','iFood Entregador','Pinecone Research',
  // ── Secção 1 — Removidas do website (verificação Gighub) ──
  'Toluna','Kantar','We Work Remotely','Jobspresso','Remote OK',
  'Support Driven Jobs','Malt','Contra','PeoplePerHour','Toptal',
  'italki','Wyzant','RWS','Appen','Outlier','Alignerr','Surge AI',
  'Influenster','Redbubble','Welocalize','DataForce','JoinBrands',
  'Trend.io','Pearpop','TaskRabbit','BeMyEye','Ipsos iShopForIpsos','Gumroad'
]);

// ── Secção 2 — Manter mas sem destaque (dimmed + aviso) ──────────
// Estas plataformas anunciam muitos dos mesmos trabalhos; acesso difícil.
const _DIMMED_SET = new Set([
  'Dynamite Jobs','FlexJobs','Wellfound','Working Nomads','Arc.dev',
  'Remotive','Fiverr','Freelancer','Upwork','Braintrust',
  'Remote.com','Respondent.io','Swagbucks'
]);

// Default earnN per category (fallback when Supabase column is missing/null)
...
const _REDUCED_EXPECTATIONS = {
  'Respondent.io': {
    desc: '⚠️ Acesso seletivo · Volume limitado de oportunidades. Plataforma de entrevistas UX com remuneração elevada. Acesso muito limitado para utilizadores PT — poucos estudos disponíveis, alta barreira de seleção e maioria dos estudos requer inglês fluente.',
    descEn: '⚠️ Selective entry · Limited opportunity volume. High-paying UX interview platform. Very limited availability for PT users — few studies available, high selection bar, and most studies require fluent English.'
  },
  'UserInterviews': {
  ...
  // ── Secção 3 — Notas de entrada/dificuldade ──────────────────────
  '99designs': {
  ...
  'Telus': {
    desc: '⚠️ Plataforma exigente — difícil de entrar. Requer qualificação prévia e avaliação criteriosa. Processo de candidatura longo com testes. Para AI training e avaliação de qualidade de dados.',
    descEn: '⚠️ Demanding platform — hard to get in. Requires prior qualification and careful evaluation. Long application process with tests. For AI training and data quality assessment.'
  },
  'OneForma': {
    desc: '⚠️ Plataforma exigente — difícil de entrar. Requer qualificação e aprovação em testes. Processo de candidatura rigoroso. Para tradução, transcrição e tarefas linguísticas.',
    descEn: '⚠️ Demanding platform — hard to get in. Requires qualification and passing tests. Rigorous application process. For translation, transcription and linguistic tasks.'
  },
  'GoTranscript': {
    desc: '⚠️ Entrada difícil — requer aprovação em teste de transcrição. Plataforma de transcrição profissional com padrões elevados de qualidade.',
    descEn: '⚠️ Hard to join — requires passing a transcription test. Professional transcription platform with high quality standards.'
  },
  'TranscribeMe': {
    desc: '⚠️ Entrada difícil — requer exame de transcrição. Plataforma de transcrição com bons valores por hora, mas difícil de passar na avaliação inicial.',
    descEn: '⚠️ Hard to join — requires passing a transcription exam. Transcription platform with good hourly rates, but the initial assessment is tough.'
  },
  'Medium': {
...
function _fmt(r){
  if(_BLOCKED.has(r.name)) return null; // skip blocked platforms
  const ov = _REDUCED_EXPECTATIONS[r.name];
  const isDimmed = _DIMMED_SET.has(r.name) || r.dimmed || false;
  // Only add generic dim warning if no custom override already handles the description
  const hasCustomOverride = ov && ov.desc;
  const warnPt = (_DIMMED_SET.has(r.name) && !hasCustomOverride)
    ? '⚠️ Acesso seletivo · Muitas plataformas desta secção anunciam os mesmos trabalhos — gere as expectativas. '
    : '';
  const warnEn = (_DIMMED_SET.has(r.name) && !hasCustomOverride)
    ? '⚠️ Selective entry · Many platforms in this section advertise the same jobs — manage your expectations. '
    : '';
...
const _EXTRA_ENTRIES = [
  ...
  {name:'AttaPoll',cat:'surveys',icon:'📊',
   ...
   earn:'Variável por inquérito',earnN:3,minPay:'Por inquérito',pt:true,eu:true,
   url:'https://attapoll.com/pt-pt/earnapp',easy:5,geo:'🇵🇹 Portugal + UE',top:false,dimmed:false,beginner:true,
  ...
  // ── Research / Academic ──────────────────────────────────────────
  {name:'CLSBE PEO (Nova SBE)',cat:'surveys',icon:'🎓',
   desc:'Painel de investigação da Nova School of Business and Economics. Participa em estudos académicos remunerados — acesso facilitado para estudantes em Portugal.',
   descEn:'Research panel from Nova School of Business and Economics. Participate in paid academic studies — easier access for students in Portugal.',
  ...
  {name:'CloudWorkers',cat:'tasks',icon:'💬',
   desc:'Plataforma de trabalho online com conversação e moderação de conteúdo. Entrada fácil/média. ⚠️ Aviso: pode envolver conversação adulta — lê os termos e condições com atenção antes de te inscreveres.',
   descEn:'Online work platform involving conversation and content moderation. Easy/medium entry. ⚠️ Note: may involve adult conversation — read the terms and conditions carefully before signing up.',
   earn:'Variável por tarefa',earnN:7,minPay:'Por tarefa',pt:false,eu:true,
   url:'https://cloudworkers.company/en?lang=en',easy:3,geo:'🌍 Global',top:false,dimmed:false,beginner:false,
  ...
  // ── Tutoring / Coaching ──────────────────────────────────────────
  {name:'TrueCoach',cat:'tutoring',icon:'💪',
   desc:'Plataforma para instrutores de fitness online. Cria o teu perfil, define o teu preço e treina clientes remotamente com planos personalizados.',
   descEn:'Platform for online fitness instructors. Create your profile, set your rate and coach clients remotely with personalised plans.',
...
function _mergeExtra(){
  // Only add entries that are not already present in P (avoid duplicates)
  const existing = new Set(P.map(p=>p.name));
  const toAdd = _EXTRA_ENTRIES.filter(e=>!existing.has(e.name));
...
window.unlock = async function() {
  ...
  if(!cleanToken) { if(btn){btn.textContent = t.lockEnter || 'Entrar →'; btn.disabled=false;} return; }
...
(function _bindEvents(){
  ...
  window.addEventListener('scroll', function(){
    ...
    _tabScrollLastY = y;
    if(Math.abs(delta) < 6) return; // ignore micro-jitter to prevent glitch loop
    if(delta > 0 && y > 120) tabsWrapEl.classList.add('collapsed');
    else if(delta < 0) tabsWrapEl.classList.remove('collapsed');
  }, {passive:true});
