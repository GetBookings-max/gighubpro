...
// Session token — stored after successful auth
...
function _applyBoostPayLang(){
  ...
  if(_bModal2) {
    const _priceDisp = document.getElementById('boostPayPriceDisp') || _bModal2.querySelector('[style*="font-size:26px"]');
...
function calcEarnings(){
  const _hrEl = document.getElementById('hoursRange');
  const _ctEl = document.getElementById('calcType');
  if(!_hrEl || !_ctEl) return;
  const h=parseInt(_hrEl.value);
  const rateRaw=_ctEl.value;
...
function openBoostPay(){
  ...
  if(modal) {
    ...
    const priceEl = document.getElementById('boostPayPriceDisp') || modal.querySelector('[style*="font-size:26px"]');
...
const _CAT_EARNINGS_FALLBACK = {
  ...
  clinical:'variavel', retail:'variavel', support:'estavel',
};
...
function render(){
  ...
  // Independent work+earnings fbox filters (can be combined)
  const _workFn     = activeWorkFilter     ? curationFilters[activeWorkFilter]     : null;
  const _earningsFn = activeEarningsFilter ? curationFilters[activeEarningsFilter] : null;

  let list=P.filter(p=>{
    if(curationFn && !curationFn(p)) return false;
    if(_workFn && !_workFn(p)) return false;
    if(_earningsFn && !_earningsFn(p)) return false;
  ...
  const _isFiltered = cat !== '' || activeCuration !== '' || activeWorkFilter !== '' || activeEarningsFilter !== '' || q !== '';
  ...
  // Combined fbox filter title (e.g. "🤝 Presencial · ⚡ Estáveis")
  const _ctMap = (_curationTitles[currentLang]||_curationTitles.pt);
  const _comboTitle = (activeWorkFilter || activeEarningsFilter)
    ? [activeWorkFilter && _ctMap[activeWorkFilter], activeEarningsFilter && _ctMap[activeEarningsFilter]].filter(Boolean).join(' · ')
    : null;
  const _tabGroupLabel = (TAB_GROUP_LABELS[currentLang]||TAB_GROUP_LABELS.pt)[cat];
  const _barTitleEl=document.getElementById('barTitle'); if(_barTitleEl) _barTitleEl.textContent= _curationTitle || _comboTitle || _tabGroupLabel || translations[currentLang].barTitle;
  ...
  if(_catNoteEl) {
    ...
    if(_noteData && !activeCuration && !activeWorkFilter && !activeEarningsFilter) {
    ...
    <div class="card ${cardClass}" role="button" aria-label="${_xss(p.name)}" data-domain="${_xss(domain)}" data-url="${_xss(effectiveUrl||'')}" tabindex="0" style="animation-delay:${Math.min(i,16)*.025}s">
      <div class="card-top">
        ...
        <div class="card-meta-top">
          <div class="card-name">
            ${q ? _highlightMatch(p.name, q) : _xss(p.name)}
...
// ── INDEPENDENT FILTER STATE — work type + earnings type can be combined ──────
let activeWorkFilter = '';     // '' | 'work_f2f' | 'work_remote'
let activeEarningsFilter = ''; // '' | 'earnings_stable' | 'earnings_variable'

function _getActiveComboKey() {
  // Used for bar title and curation-pill active state
  if(activeWorkFilter && activeEarningsFilter) return '';
  return activeWorkFilter || activeEarningsFilter || '';
}

function setFboxFilter(key) {
  const isWork     = key === 'work_f2f' || key === 'work_remote';
  const isEarnings = key === 'earnings_stable' || key === 'earnings_variable';
  const isAll = key === '';
  ...
  if(isAll) {
    ...
    activeWorkFilter = (activeWorkFilter === key) ? '' : key; // toggle
  } else if(isEarnings) {
    activeEarningsFilter = (activeEarningsFilter === key) ? '' : key; // toggle
  }

  // Keep activeCuration in sync for backwards-compat with setCuration callers
  // (curation pills use setCuration directly and reset both fbox filters)
  activeCuration = _getActiveComboKey();

  // Reset category tab when fbox filters are active
  activeTab = '';
  document.querySelectorAll('.tab').forEach(t => {
    const isAllTab = t.dataset.v === '';
    t.classList.toggle('active', isAllTab);
    t.setAttribute('aria-selected', isAllTab ? 'true' : 'false');
    t.setAttribute('tabindex', isAllTab ? '0' : '-1');
  });

  // Update curation pills (none active when fbox filters are set independently)
  document.querySelectorAll('.curation-pill').forEach(el =>
    el.classList.toggle('active', el.dataset.curation === '' && !activeWorkFilter && !activeEarningsFilter));

  // Sync fbox button active states
  const _noFilter = !activeWorkFilter && !activeEarningsFilter;
  const _fboxMap = {
    fboxAll:      _noFilter,
    fboxF2f:      activeWorkFilter === 'work_f2f',
    fboxRemote:   activeWorkFilter === 'work_remote',
    fboxStable:   activeEarningsFilter === 'earnings_stable',
    fboxVariable: activeEarningsFilter === 'earnings_variable',
  };
  Object.entries(_fboxMap).forEach(([id, active]) => {
    const el = document.getElementById(id);
    if(!el) return;
...
function setCuration(key) {
  activeCuration = key;
  // Reset independent fbox filters when a curation pill is selected
  activeWorkFilter = '';
  activeEarningsFilter = '';
  ...
  // Update all 5 filter boxes — curation pill resets fbox to All
...
const translations = {
  pt: {
    ...
    heroDesc: 'Emprego, gigs, surveys, rendimento passivo — +100 formas de ganhar dinheiro em Portugal.',
    ...
    lockHeadline: '+100 formas de<br>ganhar <em>dinheiro</em>',
    ...
    lockEnter: 'Entrar →',
    lockStep1Desc: 'Transfere <strong style="color:rgba(247,245,240,.8)">4,99€</strong> via <strong style="color:rgba(247,245,240,.8)">MB Way ou IBAN</strong> e envia o comprovativo para <strong style="color:rgba(247,245,240,.8)">gighubpt@gmail.com</strong>. Recebes a chave em minutos.',
    ...
    welcomeBody: 'Tens acesso a <strong style="color:var(--ink)">+100 plataformas verificadas</strong> para ganhar dinheiro — emprego, gigs, surveys, rendimento passivo e muito mais.',
    welcomeTip: '💡 <strong>Dica de membro:</strong> Usa a calculadora no topo para estimar quanto podes ganhar com cada tipo de trabalho.',
  ...
  en: {
    ...
    lockHeadline: '+100 ways to<br>earn <em>money</em>',
    ...
    welcomeBody: 'You have access to <strong style="color:var(--ink)">+100 verified platforms</strong> to earn money — surveys, freelance, AI, physical gigs and much more.',
    welcomeTip: '💡 <strong>Member tip:</strong> Use the calculator at the top to estimate how much you can earn with each type of work.',
    ...
    lockStep1Desc: 'Transfer <strong style="color:rgba(247,245,240,.8)">€4.99</strong> via <strong style="color:rgba(247,245,240,.8)">MB Way or IBAN</strong> to <strong style="color:rgba(247,245,240,.8)">gighubpt@gmail.com</strong> and send your receipt. You\'ll receive your key within minutes.',
...
function applyLockLang(){
  ...
  // Sync access price display from constant
  const _lapEl = document.getElementById('lockAccessPrice');
  if(_lapEl) _lapEl.textContent = isEn ? ('€' + PRICE_PER_ACCESS.toFixed(2)) : (PRICE_PER_ACCESS.toFixed(2).replace('.', ',') + '€');
  // Sync access email CTA href from constant
  const _lockWaLink = document.querySelector('.wa-cta-btn');
  if(_lockWaLink) _lockWaLink.href = _waAccessUrl(isEn ? 'en' : 'pt');
...
function applyLang(){
  ...
  const _calcModalEl = document.getElementById('calcModal');
  if(_calcModalEl && _calcModalEl.style.display==='flex') _applyCalcLang();
  const _boostPayModalEl = document.getElementById('boostPayModal');
  if(_boostPayModalEl && _boostPayModalEl.style.display==='flex') _applyBoostPayLang();
  updateFooterLang();
  ...
  Object.entries(_fbTr).forEach(([id, [label, key]]) => {
    ...
    if(el) {
      ...
      const _noFilter = !activeWorkFilter && !activeEarningsFilter && !activeCuration;
      const _active = id === 'fboxAll'
        ? _noFilter
        : (key === 'work_f2f' ? activeWorkFilter === key
          : key === 'work_remote' ? activeWorkFilter === key
          : key === 'earnings_stable' ? activeEarningsFilter === key
          : key === 'earnings_variable' ? activeEarningsFilter === key
          : activeCuration === key);
      el.classList.toggle('active', _active);
    }
  });
...
function _sanitize(s){ 
  ...
  if(/javascript:|on\w+\s*=|eval\s*\(|document\.|window\.|vbscript:|data:text|__proto__|constructor\s*\[|prototype\s*\[|Object\.prototype|<script|src\s*=/i.test(str)) return '';
...
// Operates on raw (unescaped) text to avoid matching HTML entity boundaries
function _highlightMatch(rawText, query) {
  if(!query || query.length < 2) return escHtml(rawText);
  try {
    const re = new RegExp('(' + query.replace(/[.*+?^${}()|[\]\\]/g,'\\$&') + ')', 'gi');
    const parts = rawText.split(re);
    return parts.map((part, i) =>
      (i % 2 === 1)
        ? '<mark class="search-highlight">' + escHtml(part) + '</mark>'
        : escHtml(part)
    ).join('');
  } catch(e) { return escHtml(rawText); }
}
// ─────────────────────────────────────────────────────────────────────────────
...
const _BLOCKED_PLATFORMS = new Set(['Marktest', 'Multidados', 'Quickcasting', 'Kria Eventos', 'Crowd']);
...
// _DESC_PATCH, _URL_OVERRIDES, _CAT_OVERRIDES, _ICON_OVERRIDES, _NAME_OVERRIDES, _WORK_TYPE_OVERRIDES removed.
// All platform data (name, url, icon, work_type, cat) is now canonical in Supabase.
// See migration_fixes_icons_worktype.sql for the DB corrections applied.
...
function _fmt(r){
  ...
  const basePt = (ov && ov.desc)   ? ov.desc   : (r.desc_pt||r.description||'');
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">Não usamos cookies de rastreamento nem ferramentas de analytics de terceiros. O acesso é gerido por tokens únicos sem identificação pessoal; os tokens de acesso são verificados em tempo real via Supabase e <strong>guardados localmente para restauração automática de sessão</strong>. O localStorage é usado exclusivamente para preferências locais (idioma, token de sessão, confirmação do aviso RGPD). As fontes tipográficas são carregadas a partir do serviço Google Fonts, o que implica uma ligação aos servidores da Google. Consulta a <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer" style="color:var(--gold)">Política de Privacidade da Google</a>.</p>
...
(function _bindEvents(){
  ...
  if(hoursRangeEl) hoursRangeEl.addEventListener('input', function() { this.setAttribute('aria-valuenow', this.value); calcEarnings(); });
  ...
  (function(){
    ...
    window.addEventListener('appinstalled', () => {
      ...
      _showToast(currentLang === 'en' ? '🎉 GigHub installed successfully!' : '🎉 GigHub instalado com sucesso!');
    });
      ...
      function _updateGdprText(){
        if(textEl){
          textEl.innerHTML = currentLang === 'en'
            ? 'This site uses localStorage for preferences (language, session token) — no tracking cookies or third-party analytics. <a href="#" data-modal="privacy">Privacy Policy</a>'
            : 'Este site usa localStorage para preferências locais (idioma, token de sessão) — sem cookies de rastreamento nem analytics de terceiros. <a href="#" data-modal="privacy">Política de Privacidade</a>';
        }
...
// Platforms that belong to Eventos & Audiovisual — excluded from Serviços locais
const _EVENTOS_NAMES = new Set(['Aporfest','Spring Events','Cloe Events','Casamentos.pt']);
// Platforms that belong to Sazonal/Turismo (emprego com contrato) — excluded from Serviços locais
const _SAZONAL_NAMES = new Set(['Lisbon Boats','Lisbon Pub Crawl','City Sightseeing','GuruWalk','New Europe Tours','Worldpackers','Solverde','Pestana Group (Carreiras)','Badoca Safari Park','Zoomarine','Aqualand','Aquashow','Sirius Park','Quantum Parks','Orbitur','INATEL','Ritmos Fortes','SalvaMais','Guia Tuk Tuk','Oeiras Monitores','Living Tours','Pitagórica','Douro Azul','Campanha Tomate']);
...
const CAT_EXPLORER_DEFS = [
  {
    ...
    subgroups: [
      ...
      { labelPt:'🔧 Serviços locais (Biscates)', labelEn:'🔧 Local service gigs',
        cats:['gigs','petsitting','babysitting'], filter: p => !p.is_delivery && !['Food Delivery Brands (Telepizza)','Carteiro CTT'].includes(p.name) && p.cat !== 'f2f' && p.cat !== 'support' && !_EVENTOS_NAMES.has(p.name) && !_SAZONAL_NAMES.has(p.name) },
      ...
      { labelPt:'🎭 Eventos & Audiovisual', labelEn:'🎭 Events & Audiovisual',
        cats:['gigs'], filter: p => _EVENTOS_NAMES.has(p.name) },
    ]
  },
  {
    id: 'ativos',
    ...
    subgroups: [
      { labelPt:'🏠 Espaço Físico ou Loja', labelEn:'🏠 Physical space or shop',
        cats:['passive'], filter: p => { const n = p.name.toLowerCase(); return !['revolut affiliate','xtb partnerships','booking affiliate','amazon affiliate','shopify affiliate','trade republic affiliate','temu affiliate','letyshops','beruby','cashback.pt','topcashback','picodi','klarna cashback'].includes(n); } },
      { labelPt:'🧪 Estudos Científicos & Médicos', labelEn:'🧪 Scientific & medical studies',
        cats:['clinical'], filter: p => true },
      { labelPt:'💳 Cashback (Ganha ao Comprar)', labelEn:'💳 Cashback (Earn when you shop)',
        cats:['passive'], filter: p => { const n = p.name.toLowerCase(); return ['letyshops','beruby','cashback.pt','topcashback','picodi','klarna cashback'].includes(n); } },
      { labelPt:'🔗 Tráfego / Redes Sociais (Afiliados)', labelEn:'🔗 Traffic / social media (Affiliates)',
        cats:['passive'], filter: p => ['Revolut Affiliate','XTB Partnerships','Booking Affiliate','Amazon Affiliate','Shopify Affiliate','Trade Republic Affiliate','Temu Affiliate'].includes(p.name) },
    ]
  },
  {
    id: 'microtarefas',
  ...
  },
  {
    ...
    subgroups: [
      ...
      { labelPt:'🎪 Sazonal, Turismo & Parques', labelEn:'🎪 Seasonal, Tourism & Parks',
        cats:['f2f','gigs'], filter: p => _SAZONAL_NAMES.has(p.name) },
...
function renderCatExplorer() {
  ...
  const curationFn = activeCuration ? curationFilters[activeCuration] : null;
  const _ceWorkFn     = activeWorkFilter     ? curationFilters[activeWorkFilter]     : null;
  const _ceEarningsFn = activeEarningsFilter ? curationFilters[activeEarningsFilter] : null;

  // Update count badge
  const _cecEl = document.getElementById('catExplorerCount');
  if(_cecEl && P.length) {
    const _anyFilter = curationFn || _ceWorkFn || _ceEarningsFn;
    const _vis = _anyFilter ? P.filter(p => (!curationFn||curationFn(p)) && (!_ceWorkFn||_ceWorkFn(p)) && (!_ceEarningsFn||_ceEarningsFn(p))).length : P.length;
  ...
  function _sgPlatforms(sg) {
    ...
    return P.filter(p => {
      ...
      if(curationFn && !curationFn(p)) return false;
      if(_ceWorkFn && !_ceWorkFn(p)) return false;
      if(_ceEarningsFn && !_ceEarningsFn(p)) return false;
  ...
  const html = CAT_EXPLORER_DEFS.map(cat => {
    ...
    // Count unique platforms across all subgroups for this cat (deduplicated)
    let globalIdx = 0;
    const _seenInCat = new Set();
    const subgroupsHtml = cat.subgroups.map(sg => {
      ...
      const itemsHtml = sgPlatforms.map(p => {
        _seenInCat.add(p.name);
        return _renderPlatformItem(p, globalIdx++);
      }).join('');
      return `
    ...
    const count = _seenInCat.size; // unique platforms rendered (no double-count)
...
document.addEventListener('keydown', function(e) {
  ...
  if(e.key === 'Escape') {
    ...
    if(activeCuration || activeTab || activeWorkFilter || activeEarningsFilter) { activeWorkFilter=''; activeEarningsFilter=''; setCuration(''); activeTab = ''; render(); renderCatExplorer(); }
  }
});
// ─────────────────────────────────────────────────────────────────────────────
