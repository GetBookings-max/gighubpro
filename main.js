...
function setTab(v){
  ...
  // Clear any active curation so both filters never fight each other
  if(v !== '') {
    activeCuration = '';
...
function render(){
  ...
  let list=P.filter(p=>{
    if(p.geo_blocked) return false; // explicitly blocked for PT audience — hidden unconditionally
  ...
  // ── Populate tab counts (respects geo/search/curation but not the tab filter itself)
  const _visibleAll = P.filter(p => {
    if(p.geo_blocked) return false;
    if(curationFn && !curationFn(p)) return false;
    if(q && !p.name.toLowerCase().includes(q) && !p.desc.toLowerCase().includes(q) && !(catLabel[p.cat]||'').toLowerCase().includes(q)) return false;
    if(geo==='pt' && !p.pt) return false;
    if(geo==='eu' && !p.eu) return false;
  ...
  document.querySelectorAll('.tab[data-v]').forEach(tab => {
    ...
    if(!countEl || v === '') return;
    const n = _visibleAll.filter(p => p.cat === v).length;
...
function setCuration(key) {
  ...
  // Clear any active category tab so both filters never fight each other
  if(key !== '') {
    activeTab = '';
...
const translations = {
  pt: {
    ...
    lockIncluiDesc: '119 plataformas<br>Acesso de longa duração',
    ...
    guideTip: '<strong>★ Top 3 para Portugal em 2026 —</strong> <strong>DataAnnotation.tech</strong> (treino de IA, acesso directo, sem entrevista) · <strong>Prolific</strong> (surveys académicos, muito fiável, disponível em PT) · <strong>Respondent.io</strong> (entrevistas em inglês, pagamentos acima da média, barreira de entrada mais alta)',
    // Monetization
  ...
  // GigBoost
    ...
    calcOpt2: 'Surveys / Micro-tarefas',
    calcOpt6: 'Gigs físicos — entregas',
    calcOpt7: 'Treino de IA / Anotação',
    calcOpt10: 'Freelance (design, código)',
    calcOpt8: 'Pet Sitting / Babysitting',
    calcOpt18: 'Entrevistas UX / Respondent',
  ...
  en: {
    ...
    guideTip: '<strong>★ Top 3 for 2026 —</strong> <strong>DataAnnotation.tech</strong> (AI training, direct access, no interview) · <strong>Prolific</strong> (academic surveys, very reliable, PT available) · <strong>Respondent.io</strong> (English interviews, above-average pay, higher barrier to entry)',
    // Monetization
  ...
  // GigBoost
    ...
    calcOpt2: 'Surveys / Micro-tasks',
    calcOpt6: 'Physical gigs — deliveries',
    calcOpt7: 'AI Training / Annotation',
    calcOpt10: 'Freelance (design, code)',
    calcOpt8: 'Pet Sitting / Babysitting',
    calcOpt18: 'UX Interviews / Respondent',
...
function _fmt(r){
  return {name:r.name,cat:r.cat,icon:r.icon,desc:r.desc_pt,descEn:r.desc_en,
    ...
    geo_blocked:r.geo_blocked||false,
