...
function _showErr(){
  ...
  err.textContent='Chave de acesso inválida. Tenta novamente.';
...
function saveSheetUrl(){
  ...
  if(val&&val.startsWith('https://docs.google.com/spreadsheets/')){
...
function renderRatings(p){
  ...
  // ── Payout speed ─────────────────────────────────────────────────────
  if(r.payout >= 4) tags += `<span class="rtag rtag-payout-fast">⚡ ${isEn?'Fast payout':'Pagamento rápido'}</span>`;
...
// XSS sanitisation for platform data — see escHtml() alias above
...
function render(){
  ...
  const _curationTitles={
    pt:{portugal:'Top picks Portugal 🇵🇹',beginners:'Boas para iniciantes 🌱',bestpay:'Melhores pagamentos 💰',fastest:'Pagamento rápido ⚡',noexp:'Sem experiência necessária 🚀'},
...
const translations = {
  pt: {
    ...
    welcomeBody: 'Tens acesso a <strong style="color:var(--ink)">+100 plataformas verificadas</strong> para ganhar dinheiro online — surveys, freelance, IA, gigs físicos e muito mais.<br><br>Usa os filtros para encontrar o que funciona para ti. Começa pelas marcadas como <strong style="color:var(--green)">TOP PICK</strong>.',
    ...
    // Monetization
    monoTitle: 'Dar acesso a outra pessoa?',
  ...
  en: {
    ...
    welcomeBody: 'You have access to <strong style="color:var(--ink)">+100 verified platforms</strong> to earn money online — surveys, freelance, AI, physical gigs and much more.<br><br>Use the filters to find what works for you. Start with those marked as <strong style="color:var(--green)">TOP PICK</strong>.',
    ...
    // Monetization
    monoTitle: 'Give access to someone else?',
...
function applyLockLang(){
  ...
  if(secEl) secEl.innerHTML = isEn
    ? '🔒 <strong style="color:rgba(247,245,240,.5)">Security notice:</strong> We will never ask for your password, tax number, full banking details or your access key back. We only ask for proof of payment. If you receive a message asking for these, it is fraud.'
    : '🔒 <strong style="color:rgba(247,245,240,.5)">Aviso de segurança:</strong> Nunca te pedimos a tua palavra-passe, NIF, dados bancários completos ou a tua chave de acesso de volta. Apenas pedimos comprovativo de pagamento. Se receberes uma mensagem a pedir esses dados, é fraude.';
  ...
  if(abuseEl) abuseEl.innerHTML = isEn
    ...
    : '🛡️ O acesso é pessoal e intransmissível. A partilha ou uso indevido da chave de acesso pode resultar na revogação permanente sem reembolso. Ver <a href="#" data-modal="terms" style="color:rgba(247,245,240,.45);text-decoration:underline">Termos de Utilização</a>.';
...
function applyLang(){
  ...
  const _curationLabels = {
    pt: { '':'Todas', portugal:'Top picks Portugal 🇵🇹', beginners:'Boas para iniciantes 🌱',
          bestpay:'Melhores pagamentos 💰', fastest:'Pagamento rápido ⚡',
...
function submitBoostForm(){
  ...
  if(sessionStorage.getItem('gh_boost_submitted')) {
    const isEn = currentLang === 'en';
    alert(isEn?'You have already submitted GigBoost in this session.':'Já submeteste o teu perfil GigBoost nesta sessão.');
    return;
  }
...
function renderBoostResults(r){
  ...
  box.innerHTML = `
    ...
    <div class="boost-results">
      <div class="boost-badge" style="margin-bottom:16px">✨ GigBoost · ${isEn ? 'Recommendation ready' : 'Recomendação pronta'}</div>
      ...
      <div style="margin-top:20px;display:flex;gap:10px;flex-wrap:wrap">
        <button class="boost-redo-btn" style="flex:1;height:40px;border-radius:10px;border:1.5px solid var(--border-md);background:transparent;font-family:'Instrument Sans',sans-serif;font-size:12px;font-weight:600;cursor:pointer;color:var(--grey)">↺ ${isEn ? 'Start over' : 'Recomeçar'}</button>
