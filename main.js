...
// ── FREQUENCY LABEL ───────────────────────────────────────────────────────────
// frequency_type comes from DB. Fallback by category when NULL.
const _CAT_FREQ_FALLBACK = {
  f2f:'recorrente', remote:'recorrente', support:'recorrente', retail:'recorrente',
  gigs:'recorrente', surveys:'recorrente', tasks:'recorrente', micro:'recorrente',
  testing:'recorrente', mystery:'recorrente', freelance:'recorrente',
  tutoring:'recorrente', petsitting:'recorrente', babysitting:'recorrente',
  criativo:'recorrente', conteudo:'recorrente', ugc:'recorrente', transcricao:'recorrente',
  clinical:'pontual',
  passive:'experimental',
};
const _FREQ_LABELS = {
  pt:{ recorrente:'🟢 Recorrente', pontual:'🟡 Pontual', sazonal:'🔵 Sazonal', experimental:'⚪ Experimental' },
  en:{ recorrente:'🟢 Recurring',  pontual:'🟡 One-time', sazonal:'🔵 Seasonal',  experimental:'⚪ Experimental' },
};
function _freqChip(p){
  const freq = p.frequency_type || _CAT_FREQ_FALLBACK[p.cat] || 'recorrente';
  const label = (_FREQ_LABELS[currentLang]||_FREQ_LABELS.pt)[freq];
  return `<span class="chip ch-freq ch-freq-${freq}">${label}</span>`;
}

// ── PLATFORM GROUPS ───────────────────────────────────────────────────────────
// Platforms that are functionally identical are merged into one card.
// members: exact DB names. The group card replaces all members in the render list.
const _PLATFORM_GROUPS = [
  {
    name:  '📦 Ponto de recolha de encomendas',
    nameEn:'📦 Parcel Collection Point',
    icon:  '📦',
    cat:   'passive',
    frequency_type: 'recorrente',
    desc:  'Usa a tua loja, café ou espaço comercial como ponto de recolha de encomendas. Os clientes vêm levantar as encomendas e tu recebes por cada uma. Disponível via DHL, UPS e DPD.',
    descEn:'Use your shop, café or commercial space as a parcel collection point. Customers come to collect parcels and you earn per delivery. Available via DHL, UPS and DPD.',
    earn:  'Por encomenda', earnN: 5, minPay: 'Por encomenda',
    easy: 4, geo: '🇵🇹 Portugal', top: false, dimmed: false, beginner: true,
    suggested: false, direct_hire: false, is_delivery: false, no_curation_tag: false,
    entry_level: 'facil', earnings_type: 'variavel', tier: 1,
    ratings: {}, aff: { has: false },
    members: ['DHL ServicePoints','UPS Access Point','DPD Pickup'],
    memberLinks: [
      { name: 'DHL', url: 'https://www.dhl.com/pt-pt/home/parcel-services/servicepoint.html' },
      { name: 'UPS', url: 'https://www.ups.com/pt/pt/locations/access-point.page' },
      { name: 'DPD', url: 'https://www.dpd.com/pt/pt/enviar/pontos-dpd/' },
    ],
  },
  {
    name:  '📸 Vende fotos e vídeos online',
    nameEn:'📸 Sell photos & videos online',
    icon:  '📸',
    cat:   'criativo',
    frequency_type: 'experimental',
    desc:  'Vende as tuas fotos, vídeos e ilustrações em bancos de imagens. Exige criação ativa de conteúdo e os ganhos são baixos para a maioria — bom complemento se já produces conteúdo regularmente. Disponível via Adobe Stock, Shutterstock e Pond5.',
    descEn:'Sell your photos, videos and illustrations on stock image platforms. Requires active content creation and earnings are low for most — a good complement if you already produce content regularly. Available via Adobe Stock, Shutterstock and Pond5.',
    earn:  'Por licença', earnN: 4, minPay: 'Por download',
    easy: 3, geo: '🌍 Internacional', top: false, dimmed: false, beginner: false,
    suggested: false, direct_hire: false, is_delivery: false, no_curation_tag: false,
    entry_level: 'moderado', earnings_type: 'variavel', tier: 1,
    ratings: {}, aff: { has: false },
    members: ['Adobe Stock','Shutterstock','Pond5'],
    memberLinks: [
      { name: 'Adobe Stock', url: 'https://stock.adobe.com/pt/contributor' },
      { name: 'Shutterstock', url: 'https://submit.shutterstock.com/' },
      { name: 'Pond5',        url: 'https://www.pond5.com/sell' },
    ],
  },
];

// Pre-process: replace group members with synthetic group card.
// Preserves list order (group card appears where first member would appear).
function _applyGroups(platforms){
  const grouped = new Map(); // memberName → group
  _PLATFORM_GROUPS.forEach(g => g.members.forEach(m => grouped.set(m, g)));

  const seen = new Set(); // group objects already injected
  const result = [];
  for(const p of platforms){
    ...
    if(g){
      if(!seen.has(g)){
        seen.add(g);
        // Build synthetic platform object
        result.push({
          ...g,
          name:   currentLang === 'en' ? g.nameEn : g.name,
          desc:   g.desc,
          descEn: g.descEn,
          _isGroup: true,
        });
      }
      // else: skip — group card already injected
    } else {
      result.push(p);
    }
  }
...
function render(){
  ...
  grid.innerHTML=list.map((p,i)=>{
    let domain = '';
    try { domain = p.url ? new URL(p.url).hostname.replace('www.','') : ''; } catch(e) { domain = ''; }
    const rawUrl = p.url||'';
    ...
    <div class="card ${cardClass}" role="button" aria-label="${_xss(p.name)}" data-domain="${_xss(domain)}" data-url="${_xss(effectiveUrl||'')}" tabindex="0" style="animation-delay:${Math.min(i,16)*.025}s">
      ...
      <div class="card-foot">
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
          ...
          ${p._isGroup && p.memberLinks ? p.memberLinks.map(ml=>`<a href="${_xss(ml.url)}" target="_blank" rel="noopener noreferrer" class="open-btn" style="font-size:12px;padding:6px 10px">${_xss(ml.name)} <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>`).join('') :
            effectiveUrl ? `<a href="${_xss(effectiveUrl)}" target="_blank" rel="noopener noreferrer" class="open-btn">${translations[currentLang].openBtn}<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg></a>` :
            `<span class="open-btn" style="opacity:.5;cursor:not-allowed">${translations[currentLang].openBtn}</span>`}
        </div>
      </div>
    </div>`;
  }).join('');
...
const translations = {
  pt: {
    ...
    lockPayText: 'Para obter acesso, transfere 4,99€ por MB Way ou transferência bancária e envia o comprovativo para <a href="mailto:gighubpt@gmail.com" style="color:var(--gold);text-decoration:none">gighubpt@gmail.com</a>',
    lockVerifying: 'A verificar acesso…',
    lockEnter: 'Entrar →',
    lockStep1Label: 'Como obter acesso',
    lockStep1Desc: 'Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">9,99€</span><strong>4,99€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.',
  ...
  // GigBoost
    ...
    boostPayInstr: 'Transfere <strong>14,99€</strong> via <strong>MB Way</strong> ou IBAN e envia o comprovativo para <strong>gighubpt@gmail.com</strong>.<br>O acesso GigBoost é enviado após confirmação.',
  ...
  en: {
    ...
    lockPayText: 'To get access, transfer €4.99 via MB Way or bank transfer and email your receipt to <a href="mailto:gighubpt@gmail.com" style="color:var(--gold);text-decoration:none">gighubpt@gmail.com</a>',
    ...
    monoDesc: 'Transfer €4.99 via MB Way or IBAN and email your receipt to <strong>gighubpt@gmail.com</strong>. You\'ll receive the access key in minutes. Receipts may contain personal data — see <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Privacy Policy</a>. Never share your access key.',
    ...
    lockStep1Desc: 'Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€9.99</span><strong>€4.99 via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.',
  ...
  // GigBoost
    ...
    boostPayInstr: 'Transfer <strong>€14.99</strong> via <strong>MB Way</strong> or IBAN and email your receipt to <strong>gighubpt@gmail.com</strong>.<br>Access is sent after payment confirmation.',
...
function applyLockLang(){
  ...
  setText('lockCatSkills',     '🧑‍🏫 Ensino & Tutorias',      '🧑‍🏫 Teaching & Skills');
  ...
  if(desc1) desc1.innerHTML = isEn
    ? `Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€9.99</span><strong>€${PRICE_PER_ACCESS.toFixed(2)} via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.`
    : `Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">9,99€</span><strong>${PRICE_PER_ACCESS.toFixed(2).replace('.',',')}€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.`;
...
function renderBoostStep(){
  ...
  let fieldsHtml = step.fields.map(f => {
    ...
    if(f.type === 'textarea'){
      ...
      return `<div class="boost-field">
        <label class="boost-label">${f.label}${optLabel}</label>
...
const _LOCAL_EXTRA_PLATFORMS = [
  {
    name: 'DHL ServicePoints',
    cat: 'passive',
    icon: '📦',
    desc: 'Torna o teu negócio (loja, café, papelaria…) num ponto de recolha e entrega DHL. Recebes comissão por cada encomenda processada. Sem custos de adesão.',
    descEn: 'Turn your business (shop, café, stationery…) into a DHL collection and delivery point. Earn a commission per parcel handled. No sign-up costs.',
    earn: 'Comissão por encomenda',
    earnN: 6,
    minPay: 'Por entrega',
    pt: true, eu: true,
    url: 'https://www.dhl.com/pt-en/ecommerce/home/business-to-business-shipments/dhl-servicepoints/become-partner.html',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 2,
    suggested: false, direct_hire: false, is_delivery: false, no_curation_tag: false,
  },
  {
    ...
    cat: 'passive',
    icon: '🟤',
    desc: 'Regista o teu negócio como ponto de recolha e entrega UPS. Recebe clientes UPS e ganha por cada encomenda processada. Sem investimento inicial.',
    descEn: 'Register your business as a UPS collection and delivery access point. Receive UPS customers and earn per parcel processed. No upfront investment.',
    earn: 'Comissão por encomenda',
    earnN: 6,
    minPay: 'Por entrega',
    pt: true, eu: true,
    url: 'https://www.ups.com/pt/en/business-solutions/expand-your-online-business/ups-access-point',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 2,
  ...
  },
  {
    ...
    cat: 'passive',
    icon: '🔴',
    desc: 'Candidata o teu negócio a parceiro Pickup da DPD. Recebes e entregas encomendas DPD sem investimento inicial — comissão por cada pacote tratado.',
    descEn: 'Register your business as a DPD Pickup partner. Receive and hand out DPD parcels with no upfront investment — commission per parcel handled.',
    earn: 'Comissão por encomenda',
    earnN: 6,
    minPay: 'Por entrega',
    pt: true, eu: true,
    url: 'https://www.dpd.com/pt/pt/pickup-parcelshops/parceiro-pickup/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 2,
  ...
  },
  {
    ...
    cat: 'f2f',
    icon: '🌿',
    desc: 'Empresa portuguesa de jardinagem e paisagismo com recrutamento direto para técnicos de jardim, operadores de sistemas de rega e montadores de estruturas exteriores (decks, pérgulas, vedações). Candidatura direta no site.',
    descEn: 'Portuguese landscaping company with direct hiring for garden technicians, irrigation system operators and outdoor structure installers (decks, pergolas, fencing). Apply directly on their site.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://www.sograma.pt/candidatura/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
    suggested: false, direct_hire: true, is_delivery: false, no_curation_tag: false,
  },
  {
    name: 'GetYourGuide',
    cat: 'criativo',
    icon: '🔗',
    desc: 'Programa de afiliados da maior plataforma mundial de reserva de experiências. Cria conteúdo sobre destinos e atividades e ganha comissão (até 8%) em cada reserva feita via os teus links — blog, YouTube, redes sociais ou site próprio.',
    descEn: "Affiliate program from the world's largest experiences booking platform. Create content about destinations and activities and earn commission (up to 8%) on every booking made through your links — blog, YouTube, social media or your own site.",
    earn: 'Variável por conteúdo/experiência',
    earnN: 10,
    minPay: 'Por projeto',
    pt: true, eu: true,
    url: 'https://partner.getyourguide.com/pt-pt/content-creators',
    easy: 3, geo: '🌍 Global',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 2,
  ...
  },
  {
    ...
    cat: 'gigs',
    icon: '🗺️',
    desc: 'Torna-te guia de free tours na tua cidade. Registas-te, crias o teu tour e recebes gorjetas dos participantes. Sem exclusividade, sem taxas de entrada.',
    descEn: 'Become a free tour guide in your city. Register, set up your tour and earn tips from participants. No exclusivity, no entry fees.',
    earn: 'Gorjetas (variável)',
    earnN: 8,
    minPay: 'Por visita',
    pt: true, eu: true,
    url: 'https://www.guruwalk.com/work-free-tour-guide',
    easy: 4, geo: '🌍 Global',
    top: false, dimmed: false, beginner: true,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'variavel', tier: 1,
    suggested: false, direct_hire: false, is_delivery: false, no_curation_tag: false,
  },
  {
    name: 'Central de Cervejas',
    cat: 'f2f',
    icon: '🍺',
    desc: 'Portal de carreiras da Central de Cervejas e Bebidas (Grupo Heineken Portugal). Vagas em produção, logística, comercial, marketing e administração em todo o país.',
    descEn: 'Careers portal for Central de Cervejas e Bebidas (Heineken Group Portugal). Vacancies in production, logistics, commercial, marketing and administration across the country.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://careers.theheinekencompany.com/Portugues/Central-Cervejas-e-Bebidas',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
  ...
  {
    name: 'Blue & Green Hotels',
    cat: 'f2f',
    icon: '🌊',
    desc: 'Portal de recrutamento do grupo Blue & Green Hotels & Resorts — hotéis, resorts e spas de luxo em Portugal. Vagas em hotelaria, restauração, spa e gestão hoteleira.',
    descEn: 'Recruitment portal for Blue & Green Hotels & Resorts — luxury hotels, resorts and spas in Portugal. Vacancies in hospitality, food & beverage, spa and hotel management.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://recrutamento.blueandgreen.com/pt/ofertas-de-emprego/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
  ...
  },
  {
    ...
    cat: 'f2f',
    icon: '🚌',
    desc: 'Empresa portuguesa de transportes rodoviários de passageiros. Candidaturas para motoristas, técnicos de manutenção, mecânicos e funções administrativas na rede de transportes.',
    descEn: 'Portuguese road passenger transport company. Applications for drivers, maintenance technicians, mechanics and administrative roles across the transport network.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://www.barraqueiro.com/recrutamento/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
  ...
  },
  {
    ...
    cat: 'gigs',
    icon: '🔴',
    desc: 'Empresa global de autocarros turísticos hop-on hop-off com operações em Lisboa e Porto. Vagas para motoristas, guias turísticos, staff de bilheteira e funções operacionais em destinos turísticos.',
    descEn: 'Global hop-on hop-off sightseeing bus company operating in Lisbon and Porto. Vacancies for drivers, tour guides, ticketing staff and operational roles in tourist destinations.',
    earn: 'Salário + comissões',
    earnN: 8,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://city-sightseeing.com/en/careers',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'estavel', tier: 1,
    suggested: false, direct_hire: true, is_delivery: false, no_curation_tag: false,
  },
  {
    name: 'ERA Portugal',
    cat: 'f2f',
    icon: '🏠',
    desc: 'Torna-te consultor imobiliário independente na rede ERA. Formação completa fornecida, sem custos de entrada. Ganhos 100% por comissão nas transações que realizas — potencial elevado mas variável. Requer obtenção de licença AMI (processo assistido pela ERA).',
    descEn: 'Become an independent real estate consultant in the ERA network. Full training provided at no cost. 100% commission-based earnings on completed transactions — high potential but variable. Requires obtaining an AMI licence (ERA-assisted process).',
    earn: 'Comissão por transação',
    earnN: 9,
    minPay: 'Por venda',
    pt: true, eu: false,
    url: 'https://www.era.pt/trabalhar-na-era/recrutamento/candidatura-espontanea',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 1,
  ...
  {
    name: 'MetLife Portugal',
    cat: 'f2f',
    icon: '💼',
    desc: 'Torna-te agente de seguros independente MetLife. Formação gratuita até à certificação incluída, sem custos para o candidato. Ganhos 100% por comissão sobre seguros de vida e acidentes pessoais.',
    descEn: 'Become an independent MetLife insurance agent. Free training through to certification included, at no cost to the candidate. 100% commission on life and personal accident insurance.',
    earn: 'Comissão por apólice',
    earnN: 9,
    minPay: 'Por venda',
    pt: true, eu: false,
    url: 'https://www.metlife.pt/novos-agentes/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'variavel', tier: 1,
  ...
  },
  {
    ...
    cat: 'retail',
    icon: '♻️',
    desc: 'Trabalha para a Sociedade Ponto Verde (SPV) a promover a reciclagem de embalagens junto de municípios, empresas e escolas. Vagas em sensibilização ambiental, educação e comercial. Contrato de trabalho, salário fixo.',
    descEn: 'Work for Sociedade Ponto Verde (SPV) promoting packaging recycling with municipalities, companies and schools. Roles in environmental awareness, education and commercial. Employment contract, fixed salary.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://www.pontoverde.pt/sobre-nos/junta-te-a-nos/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
  ...
  },
  {
    ...
    cat: 'f2f',
    icon: '🌿',
    desc: 'Candidatura na Ecoambiente, empresa de gestão de resíduos, limpeza urbana e de praias com mais de 170 camiões em operação diária. Vagas operacionais para condutores, operadores de recolha, técnicos de manutenção e limpeza urbana. Contrato de trabalho, salário fixo.',
    descEn: 'Apply to Ecoambiente, a waste management, urban and beach cleaning company with over 170 trucks in daily operation. Operational roles for drivers, collection operators, maintenance technicians and urban cleaning staff. Employment contract, fixed salary.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://www.ecoambiente.pt/contactos/recrutamento-2/',
    easy: 4, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: true,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'estavel', tier: 1,
  ...
  {
    name: 'Trablisa ESEgur',
    cat: 'f2f',
    icon: '🔐',
    desc: 'Trabalha como vigilante ou técnico de segurança na Trablisa ESEgur, líder em Portugal no transporte e tratamento de valores. Vagas para vigilantes (com e sem cartão profissional), técnicos de segurança eletrónica e funções operacionais. Obrigatório não ter antecedentes criminais.',
    descEn: 'Work as a security guard or technician at Trablisa ESEgur, Portugal\'s leader in value transport and handling. Roles for guards (with and without professional licence), electronic security technicians and operational functions. No criminal record required.',
    earn: 'Salário mensal',
    earnN: 9,
    minPay: 'Mensal',
    pt: true, eu: false,
    url: 'https://trablisa.esegur.pt/trabalhe-connosco/',
    easy: 3, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: false,
    ratings: {}, aff: { has: false },
    entry_level: 'moderado', earnings_type: 'estavel', tier: 1,
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  // Adicionados junho 2026 — f2f, surveys, tasks, clinical
  'prio carreiras',
  'monitores jm oeiras',
  'makro',
  'ava clinic',
  'psicologia ulisboa',
  'em estudos de mercado','e.m. estudos de mercado',
  'lootup',
]);
  ...
  if(favExportBtn) favExportBtn.addEventListener('click', () => {
    ...
    _box.innerHTML = `
      ...
      <div style="margin-top:12px;background:rgba(201,168,76,.08);border:1px solid rgba(201,168,76,.2);border-radius:10px;padding:12px 14px;font-size:12px;color:var(--ink);line-height:1.6">
        ...
        <div style="margin-top:8px;word-break:break-all;font-size:11px;color:var(--grey);background:var(--cream);border-radius:6px;padding:6px 8px;font-family:monospace">${bookmarkUrl}</div>
      </div>
