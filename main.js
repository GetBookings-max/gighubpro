...
function _waAccessUrl(lang) {
  ...
  const pt = `Olá! Quero acesso ao GigHub (+80 plataformas verificadas). Acabei de enviar ${amt} via MB Way para 938 556 803. Segue o comprovativo 👇`;
  const en = `Hi! I want access to GigHub (+80 verified platforms). I just sent €${PRICE_PER_ACCESS.toFixed(2)} via MB Way to 938 556 803. Receipt follows 👇`;
...
function calcEarnings(){
  ...
  const isClinical = rateRaw === '999';
  const t = translations[currentLang]||translations['pt'];
  document.getElementById('hoursVal').textContent=h+'h';

  if(isClinical){
    // Clinical trials are project-based — hourly calc doesn't apply
    ...
    _s('calcTitleEl','calcTitle'); _s('calcDescEl','calcDesc');
    _s('calcHoursLabelEl','calcHoursLabel'); _s('calcTypeLabel','calcTypeLabel');
    ['2','6','7','8','10','18','999'].forEach(v=>{const el=document.getElementById('calcOpt'+v);if(el&&t['calcOpt'+v])el.textContent=t['calcOpt'+v];});
    const rlEl=document.getElementById('calcResultLabelEl');
    if(rlEl) rlEl.textContent=currentLang==='en'?'Per project · very limited availability':'Por projeto · disponibilidade muito limitada';
    const estLabel=currentLang==='en'?'Conservative estimate · no guarantees':'Estimativa conservadora · sem garantias';
    document.getElementById('calcSuggest').innerHTML='<div style="margin-bottom:6px;color:var(--grey)">'+estLabel+'</div><div style="font-size:13px;font-weight:600;color:var(--ink)">💡 '+(currentLang==='en'?'Suggested':'Sugestões')+': <span style="font-weight:400;color:var(--grey)">BlueClinical</span></div>';
    return;
  }

  // For UX testing (rate=18): realistic PT-availability factor (~0.15).
  ...
  const sPT={
    ...
    18:'Hotjar Engage · PlaytestCloud · WearTesters'
  };
  const sEN={
    2:'AttaPoll · Netsonda · YouGov',
    6:'Uber Eats · Glovo · Bolt Food',
...
// ── DATA ──
const catLabels = {
  pt: {surveys:'Inquéritos & Microtasks',gigs:'Gigs',freelance:'Freelance',micro:'Treino de IA',testing:'Testes',criativo:'Conteúdo & Criativo',conteudo:'Conteúdo & Criativo',tasks:'Inquéritos & Microtasks',mystery:'Cliente Mistério',transcricao:'Transcrição',tutoring:'Tutoria',ugc:'Conteúdo & Criativo',passive:'Nicho',remote:'Emprego Remoto',petsitting:'Pet Sitting',babysitting:'Babysitting',f2f:'Trabalho Presencial',clinical:'Ensaios Clínicos',retail:'Operações & Retalho'},
  en: {surveys:'Surveys & Microtasks',gigs:'Gigs',freelance:'Freelance',micro:'AI Training',testing:'App Testing',criativo:'Content & Creative',conteudo:'Content & Creative',tasks:'Surveys & Microtasks',mystery:'Mystery Shopping',transcricao:'Transcription',tutoring:'Tutoring',ugc:'Content & Creative',passive:'Niche',remote:'Remote Jobs',petsitting:'Pet Sitting',babysitting:'Babysitting',f2f:'In-Person Work',clinical:'Clinical Trials',retail:'Operations & Retail'}
};
...
const _MODERADO_PLATFORMS = new Set([
  ...
  // 🏭 Operações & Retalho — candidatura + seleção obrigatória
  'DHL','RGIS','Securitas','Prosegur','Salesland',
  'Portway','Mercadona','Prezero','Jardim Zoológico','Jardim Zoologico','Eurest',
]);
...
const _ESTAVEL_PLATFORMS = new Set([
  ...
  'Uber Driver',
  // 🏭 Operações & Retalho — salário mensal fixo
  'DHL','RGIS','Securitas','Prosegur','Salesland',
  'Portway','Mercadona','Prezero','Jardim Zoológico','Jardim Zoologico','Eurest',
]);
...
const _CAT_ENTRY_FALLBACK = {
  ...
  clinical:'facil', retail:'moderado',
};
...
const _CAT_EARNINGS_FALLBACK = {
  ...
  clinical:'variavel', retail:'estavel',
};

function renderRatings(p){
  const r = p.ratings || {};
  const isEn = currentLang === 'en';
  const isRetail = p.cat === 'retail';

  // ── RETAIL: tags fixas (Direct Hiring + Seleção + Estável) ───────────────────
  if(isRetail){
    const directTag   = `<span class="rtag rtag-direct">🟢 ${isEn?'Direct Hiring':'Contratação Directa'}</span>`;
    const selectTag   = `<span class="rtag rtag-entry-mid">🎯 ${isEn?'Selection required':'Seleção necessária'}</span>`;
    const stableTag   = `<span class="rtag rtag-realistic-high">⚡️ ${isEn?'Stable income':'Ganhos estáveis'}</span>`;
    const highlightTag = _SUGGESTED_SET.has(p.name)
      ? `<span class="rtag rtag-suggested">⭐ ${isEn?'Recommended':'Recomendado'}</span>` : '';
    return `<div class="ratings-row">${directTag}${selectTag}${stableTag}${highlightTag}</div>`;
  }
...
const TAB_CAT_FILTERS = {
  '':           () => true,
  'surveys':    p => ['surveys', 'tasks'].includes(p.cat),
...
const TAB_GROUP_LABELS = {
  pt: {
    ...
    'surveys':    '📝 Inquéritos, Microtasks e Get-Paid-To',
    ...
    'clinical':   '🩺 Ensaios Clínicos e Programas de Investigação',
    'retail':     '🏭 Operações, Retalho e Serviços',
    'other':      '💚 Outras Plataformas e Serviços de Nicho',
  },
  en: {
    '':           'All platforms',
    'surveys':    '📝 Surveys, Micro-tasks & Get-Paid-To',
    ...
    'clinical':   '🩺 Clinical Trials & Research Programmes',
...
const translations = {
  pt: {
    ...
    sortEasy: '✓ Mais fácil',
    sortName: 'A–Z Nome',
    heroTag: '✅ +80 plataformas verificadas · Atualizado',
    ...
    lockHeadline: '+80 plataformas<br><em>verificadas</em> num só lugar',
    ...
    welcomeBody: 'Tens acesso a <strong style="color:var(--ink)">+80 plataformas verificadas</strong> para ganhar dinheiro online — surveys, freelance, IA, gigs físicos e muito mais.<br><br>Usa os filtros para encontrar o que funciona para ti. Começa pelas marcadas como <strong style="color:#8a6820">⭐ Recomendado</strong>.',
    ...
    guideTip: '<strong>★ Top 3 para Portugal em 2026 —</strong> <strong>AttaPoll</strong> (app de surveys, simples, paga por PayPal, disponível em PT) · <strong>Toloka AI</strong> (micro-tarefas de IA, boa disponibilidade PT, entrada fácil) · <strong>Netsonda</strong> (estudos de mercado em PT, fácil de começar, sem requisitos)',
    // Monetization
  ...
  // GigBoost
    ...
    calcOpt999: '🩺 Ensaios Clínicos (por projeto)',
  ...
  en: {
    ...
    sortName: 'A–Z Name',
    heroTag: '✅ +80 verified platforms · Updated',
    ...
    lockHeadline: '+80 verified<br><em>platforms</em> in one place',
    ...
    welcomeBody: 'You have access to <strong style="color:var(--ink)">+80 verified platforms</strong> to earn money online — surveys, freelance, AI, physical gigs and much more.<br><br>Use the filters to find what works for you. Start with those marked as <strong style="color:#8a6820">⭐ Recommended</strong>.',
  ...
  // GigBoost
    ...
    calcOpt999: '🩺 Clinical Trials (per project)',
...
function applyLockLang(){
  ...
  setText('lockCatClinical',   '🩺 Ensaios Clínicos',         '🩺 Clinical Trials');
...
function applyLang(){
  ...
  const _TAB_SHORT = {
    pt:{'':'Todas','surveys':'📝 Inquéritos & Microtasks','testing':'🔬 Testes','micro':'🤖 Treino IA',
      ...
      'f2f':'🤝 Trabalho Presencial','clinical':'🩺 Ensaios Clínicos',
      'retail':'🏭 Operações & Retalho','other':'💚 Outros'},
    en:{'':'All','surveys':'📝 Surveys & Microtasks','testing':'🔬 Testing','micro':'🤖 AI Training',
      ...
      'f2f':'🤝 In-Person Work','clinical':'🩺 Clinical Trials',
      'retail':'🏭 Operations & Retail','other':'💚 Other'}
  };
...
const _BLOCKED_PLATFORMS = new Set([
  ...
  'Casamentos',
  // ── Removed by user request (batch May 2026) ─────────────────────────────
  // 📝 Surveys
  'NiceQuest',
  'PaidViewpoint', 'Paid Viewpoint',
  'Zap Surveys',
  ...
  // 💼 Remote
  'HomeFromCollege', 'Home From College', 'Home from College',
  // ── Removed May 2026 (batch 2) ───────────────────────────────────────────
  'Swagbucks', 'ySense',
  'Lootup', 'EarnLab',
  'Opinionz', 'Surveoo',
  'Questionários Online', 'Questionarios Online',
  'Opiday', 'Palpito',
  'Mundo de Opiniões', 'Mundo de Opinioes',
  'Voissy', 'OnePulse',
]);

// ── Suggested starters — shown with a subtle inline tag, no corner badge ──
// One recommendation per category tab. Only platforms consistently available in PT.
...
const _EARN_N_DEFAULT = {
  ...
  petsitting:9, babysitting:9, gigs:7, f2f:9, mystery:6, clinical:500, retail:900
};
...
const _REDUCED_EXPECTATIONS = {
  ...
  'Superprof': {
    desc: 'Define o teu horário e tarifa. Os novos professores são obrigados a oferecer uma primeira aula gratuita. Ganhos variáveis conforme disponibilidade e número de alunos.',
    descEn: 'Set your own schedule and rate. New teachers must offer a free first lesson. Earnings vary depending on availability and number of students.'
  },
  // ── Research / UX ─────────────────────────────────────────────────────────
  'Hotjar Engage': {
    desc: 'Partilha a tua opinião em entrevistas de UX e testes de produto. Remuneração variável conforme a duração e tipo de estudo.',
    descEn: 'Share your opinion in UX interviews and product tests. Compensation varies by study length and type.'
  },
  'Hotjar': {
    desc: 'Partilha a tua opinião em entrevistas de UX e testes de produto. Remuneração variável conforme a duração e tipo de estudo.',
    descEn: 'Share your opinion in UX interviews and product tests. Compensation varies by study length and type.'
  ...
  'PlaytestCloud': {
    desc: 'Testes de apps e jogos mobile. Os ganhos variam consoante a duração e tipo de teste — consulta o email de convite para o valor específico.',
    descEn: 'Test mobile apps and games. Earnings vary by test length and type — check the invitation email for the specific amount.'
  },
  'PetBacker': {
  ...
  'AttaPoll': {
    desc: 'Ganha por completar surveys curtos de mercado, jogar jogos mobile e testar apps. Levantamento por PayPal, gift cards digitais ou transferência bancária direta a partir de um valor mínimo baixo.',
    descEn: 'Get paid for completing short market surveys, playing mobile games and testing apps. Withdraw via PayPal, digital gift cards, or direct bank transfer from a low minimum threshold.'
  },
  'Triaba': {
    desc: 'Surveys pagos por PayPal, Bitcoin ou vale-presente. Ganhos variáveis por inquérito.',
    descEn: 'Paid surveys via PayPal, Bitcoin or gift card. Variable earnings per survey.'
  },
  // ── Gigs / Other ─────────────────────────────────────────────────────────
  'Merytu': {
  ...
  // ── Medium / keep existing description ───────────────────────────────────
  'Medium': {
  ...
  // ── 🏭 Operações & Retalho ────────────────────────────────────────────────
  'DHL': {
    desc: 'Vagas de armazém, motoristas e entregas. Candidatura online — verificação de antecedentes e carta de condução podem ser exigidas consoante a função.',
    descEn: 'Warehouse, driver and delivery roles. Online application — background check and driving licence may be required depending on the role.'
  },
  'RGIS': {
    desc: 'Auditor de inventário: conta e regista produtos em loja, normalmente em horários noturnos ou ao fim de semana.',
    descEn: 'Inventory auditor: count and record store products, usually on night shifts or weekends.'
  },
  'Securitas': {
    desc: 'Trabalha como segurança em superfícies comerciais, eventos ou edifícios. Idealmente ter cartão profissional de segurança, mas a empresa pode financiar a formação.',
    descEn: 'Work as a security guard in retail, events or buildings. Security professional card preferred, but training can be financed by the company.'
  },
  'Prosegur': {
    desc: 'Trabalha como segurança. Sem registo criminal obrigatório. Idealmente ter cartão profissional de segurança, mas a empresa dá formação se necessário.',
    descEn: 'Security guard roles. Clean criminal record required. Security card preferred, but the company provides training if needed.'
  },
  'Salesland': {
    desc: 'Comercial part-time em ações de venda em loja, stand ou porta-a-porta. Ganhos variáveis com componente de comissão.',
    descEn: 'Part-time sales roles in stores, stands or door-to-door. Variable pay with commission component.'
  },
  'Portway': {
    desc: 'Assistente de informações ao cliente, técnico de manutenção e outras funções no aeroporto de Lisboa. Candidatura formal com entrevista.',
    descEn: 'Customer information assistant, maintenance technician and other airport roles in Lisbon. Formal application with interview.'
  },
  'Mercadona': {
    desc: 'Operador de loja: reposição, caixa e atendimento ao cliente. Contrato de trabalho com horário definido.',
    descEn: 'Store operator: shelf stocking, checkout and customer service. Employment contract with set hours.'
  },
  'Prezero': {
    desc: 'Recolha de resíduos e limpeza urbana. Trabalho físico ao ar livre com turnos rotativos.',
    descEn: 'Waste collection and urban cleaning. Physical outdoor work with rotating shifts.'
  },
  'Jardim Zoológico': {
    desc: 'Atendimento ao público, bilheteira e apoio a visitantes no Zoo de Lisboa. Candidatura espontânea ou resposta a vagas abertas.',
    descEn: 'Visitor services, ticket sales and visitor support at Lisbon Zoo. Spontaneous application or response to open vacancies.'
  },
  'Jardim Zoologico': {
    desc: 'Atendimento ao público, bilheteira e apoio a visitantes no Zoo de Lisboa. Candidatura espontânea ou resposta a vagas abertas.',
    descEn: 'Visitor services, ticket sales and visitor support at Lisbon Zoo. Spontaneous application or response to open vacancies.'
  ...
  'Eurest': {
    desc: 'Ajudante de cozinha, empregado de balcão e cafetaria em empresas, hospitais e escolas. Contrato de trabalho.',
    descEn: 'Kitchen helper, counter and cafeteria staff in companies, hospitals and schools. Employment contract.'
  },
  // ── Ensaios Clínicos ──────────────────────────────────────────────────────
  'BlueClinical': {
    desc: 'Ensaios clínicos remunerados para pessoas saudáveis no Porto. Requer disponibilidade para fins de semana ou dias internados em clínica. Paga entre 300€ e 2000€ limpos por projeto.',
    descEn: 'Paid clinical trials for healthy individuals in Porto. Requires availability for weekends or overnight stays at the clinic. Pays between €300 and €2,000 per project.'
  },
  // ── Outros / Nicho ────────────────────────────────────────────────────────
  'BeRuby': {
    desc: 'Cashback em lojas online como Worten, Fnac e Booking. O dinheiro do cashback demora entre 30 a 90 dias a ser validado pelas lojas. O registo é imediato e dá para começar a ganhar no próprio dia.',
    descEn: 'Cashback from online shops like Worten, Fnac and Booking. Cashback takes 30–90 days to be validated by stores. Registration is instant and you can start earning on the same day.'
  },
  'Radical Storage': {
    desc: 'Ganha dinheiro a guardar bagagens de turistas. Apenas para quem tem um espaço comercial, escritório, café ou arrecadação segura em zonas turísticas (Lisboa, Porto, Algarve, etc.).',
    descEn: 'Earn money storing luggage for tourists. For those with a commercial space, office, café or secure storage in tourist areas (Lisbon, Porto, Algarve, etc.).'
  },
  'Bounce': {
    desc: 'Ganha dinheiro a guardar malas e bagagens de turistas. Apenas para quem tem um espaço comercial, escritório, café ou arrecadação segura em zonas turísticas (Lisboa, Porto, Algarve, etc.).',
    descEn: 'Earn money storing bags and luggage for travellers. For those with a commercial space, office, café or secure storage in tourist areas (Lisbon, Porto, Algarve, etc.).'
  },
  'Yescapa': {
    desc: 'Aluga a tua autocaravana ou campervan a outros viajantes. Tens de ter uma autocaravana ou campervan própria. A plataforma trata dos seguros e pagamentos.',
    descEn: 'Rent out your motorhome or campervan to other travellers. You need to own a motorhome or campervan. The platform handles insurance and payments.'
  },
  'Sonder People': {
    desc: 'Casting para publicidade, fotografia e vídeo. Podes ficar meses sem ser chamado — o volume de trabalho é imprevisível e sazonal.',
    descEn: 'Casting agency for advertising, photography and video. You may go months without being called — workload is unpredictable and seasonal.'
  },
  // ── Face to Face ──────────────────────────────────────────────────────────
  'ACNUR': {
...
const _URL_OVERRIDES = {
  ...
  // ── 🏭 Operações & Retalho ────────────────────────────────────────────────
  'DHL':              'https://careers.dhl.com/amer/pt/frontlineworkers',
  'RGIS':             'https://www.rgis.pt/carreiras/',
  'Securitas':        'https://jobs.securitas.es/ssia/pt/pais/portugal/8',
  'Prosegur':         'https://www.prosegur.pt/trabalhe-connosco',
  'Salesland':        'https://empleo.salesland.net/pt/ofertas-emprego',
  'Portway':          'https://www.portway.pt/pt/carreiras/carreiras/',
  'Mercadona':        'https://mercadona.avature.net/pt_PT/Careers/SearchJobs/?2307=244&2307_format=2105&listFilterMode=1&jobRecordsPerPage=6&',
  'Prezero':          'https://jobs.prezero.com/Portugal/search/?searchby=distance&createNewAlert=false&q=&geolocation=&d=15&lat=&lon=&optionsFacetsDD_customfield2=&optionsFacetsDD_dept=&optionsFacetsDD_shifttype=&optionsFacetsDD_city=',
  'Jardim Zoológico': 'https://www.zoo.pt/pt/apoiar/candidaturas/',
  'Jardim Zoologico': 'https://www.zoo.pt/pt/apoiar/candidaturas/',
  'Eurest':           'https://recrutamento.eurest.pt/',
  ...
  // ── New platforms (batch May 2026) ────────────────────────────────────────
  'BeRuby':               'https://pt.beruby.com/',
  'BlueClinical':         'https://www.blueclinical.com/participants/',
  'Radical Storage':      'https://join.radicalstorage.com/',
  'Bounce':               'https://bounce.com/partner',
  'Yescapa':              'https://www.yescapa.pt/rent-my-motorhome',
  'Sonder People':        'https://sonderpeople.com/en/i-want-to-be-a-sonder/',
};
...
const _CAT_OVERRIDES = {
  ...
  'Smartspotter':       'mystery',
  // 🏭 Operações & Retalho — força cat=retail independentemente do valor da DB
  'DHL':               'retail',
  'RGIS':              'retail',
  'Securitas':         'retail',
  'Prosegur':          'retail',
  ...
  'Mercadona':         'retail',
  'Prezero':           'retail',
  'Jardim Zoológico':  'retail',
  'Jardim Zoologico':  'retail',
  'Eurest':            'retail',
  // 🩺 Ensaios Clínicos — donation/clinical programmes
  'iVidador':           'clinical',
  'iVidoa':             'clinical',
  'BlueClinical':       'clinical',
  // Other / Nicho — new passive platforms
  'Radical Storage':    'passive',
  'Bounce':             'passive',
  'Yescapa':            'passive',
  'Sonder People':      'passive',
  // Gigs & Events
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  'bookycar','buyandSellhair','buyandSell hair','buyandSellHair',
