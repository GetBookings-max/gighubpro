...
function admRender(){
  ...
  <div style="background:var(--green-pale);border:1.5px solid rgba(45,122,79,.3);border-radius:12px;padding:20px 18px;margin-bottom:16px">
    ...
    ${safeSheetUrl
      ? `<a href="${escHtml(safeSheetUrl)}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;height:40px;padding:0 18px;border-radius:8px;background:var(--green);color:#fff;text-decoration:none;font-size:13px;font-weight:700">
           📊 Abrir Google Sheet →
         </a>`
...
function toggleFavView(){
  ...
  if(favBtn){
    favBtn.innerHTML='⭐<span class="fav-txt"> '+t.navFavs.replace(/^⭐\s*/,'')+'</span> (<span id="favCount">'+favs.length+'</span>)';
...
function applyLang(){
  ...
  const _setGuideText = (id, key) => { const el = document.getElementById(id); if(el && t[key] !== undefined) el.textContent = t[key]; };
  _setGuideText('guideStep1H', 'guideStep1H');
  _setGuideText('guideStep2H', 'guideStep2H');
  _setGuideText('guideStep3H', 'guideStep3H');
