...
async function validarTokenSupabase(token) {
  ...
  try {
    ...
    // Safety-net: ensure no hash lingers in the URL bar after a successful login.
    // The primary strip happens immediately when the hash is read (before this async call),
    // but this handles any edge-case where a hash somehow survived to this point.
    try{
...
(function _bindEvents(){
  ...
  if(copyIbanBtn) copyIbanBtn.addEventListener('click', function() {
    navigator.clipboard.writeText('PT50002300004569719247094').then(() => {
  ...
  (async function() {
    ...
    // so the token never lingers in the URL or browser history regardless of
