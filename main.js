...
function _waBoostUrl() {
  ...
  const sub = encodeURIComponent(`Pedido GigBoost — Comprovativo MB Way (${amt})`);
  const body = encodeURIComponent(`Olá,\n\nAcabei de enviar ${amt} por MB Way para o GigBoost.\nSegue em anexo o comprovativo.\n\nObrigado!`);
  return `mailto:gighubpt@gmail.com?subject=${sub}&body=${body}`;
}
...
const translations = {
  pt: {
    ...
    monoDesc: 'Transfere 24,99€ por MB Way ou IBAN e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a chave de acesso em minutos. Os recibos podem conter dados pessoais — ver <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Política de Privacidade</a>.',
  ...
  en: {
    ...
    guideTip: '<strong>★ Top 3 for 2026 —</strong> <strong>AttaPoll</strong> (survey app, simple, pays via PayPal, available in PT) · <strong>Toloka</strong> (AI micro-tasks, good PT availability, easy entry) · <strong>Netsonda</strong> (market research in PT, easy to start, no requirements)',
    // Monetization
    monoTitle: 'Get access to GigHub',
    monoDesc: 'Transfer €24.99 via MB Way or IBAN and email your receipt to <strong>gighubpt@gmail.com</strong>. You\'ll receive the access key in minutes. Receipts may contain personal data — see <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Privacy Policy</a>.',
...
function applyLang(){
  ...
  // Footer lock badge + monetização email button
  const _isEn2 = currentLang === 'en';
  // Sync monetization WA link
  const monoWaLink = document.querySelector('#monetizacao a[href*="mailto"]');
  ...
  if(monoWaBtnEl) monoWaBtnEl.textContent = t.monoWaBtn || (_isEn2 ? 'Request access by Email →' : 'Pedir acesso por Email →');
...
Preferências: ${prefs||'-'}`
  ...
  setTimeout(() => {
    const w=window.open(mailtoLink,'_blank','noopener,noreferrer'); if(w)w.opener=null;
  }, 500);
}
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">Os pagamentos são efetuados via MB Way ou transferência bancária. Não armazenamos dados de pagamento. Os recibos enviados por email podem conter dados pessoais (nome, IBAN) que são tratados exclusivamente para confirmação do pedido.</p>
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">Payments are made via MB Way or bank transfer. Receipts and GigBoost profile data (optional) are sent by email to gighubpt@gmail.com. Email is processed by Google (Gmail). We do not use this data for marketing. To exercise your rights, contact us by email.</p>
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">Payments are made via MB Way or bank transfer. We do not store payment data. Payment receipts sent by email may contain personal data (name, IBAN) which is processed solely for order confirmation.</p>
