...
function _saveFavsCookie(arr) {
  try {
    ...
    if (val.length < 3500) // stay within 4KB cookie limit
      document.cookie = 'gh_favs=' + val + ';expires=' + d.toUTCString() + ';path=/;SameSite=Strict';
  } catch(e) {}
}
...
let favs = (() => { try { const v = JSON.parse(localStorage.getItem('gh_favs')||'[]'); if(Array.isArray(v) && v.length > 0) return v.filter(x=>typeof x==='string' && x.length < 200); return _loadFavsCookie(); } catch(e){ return _loadFavsCookie(); } })();
...
function calcEarnings(){
  ...
  const s={
    2:  'Multidados · Marktest',
    3:  'DHL ServicePoints · Radical Storage · Yescapa',
...
const curationFilters = {
  ...
  noexp: p => {
    ...
    if(['Uber Driver','Bolt Driver','CLSBE PEO (Católica)'].includes(p.name)) return true;
...
const _REDUCED_EXPECTATIONS = {
  ...
  'Vanguardista': {
    desc: 'Empresa portuguesa de operações com drones — fotografia e vídeo aéreo, inspeção de infraestruturas, topografia e mapeamento. Vagas ocasionais para pilotos de drones certificados (ANAC) e técnicos de operações.',
    descEn: 'Portuguese drone operations company — aerial photography and video, infrastructure inspection, surveying and mapping. Occasional vacancies for ANAC-certified drone pilots and operations technicians.',
  },
  'iVidador': {
    desc: 'Programa de doação de esperma da Clínica IVI Lisboa. Portugal paga uma compensação estipulada por lei. O processo é simples e confidencial, mas a compensação é bastante reduzida.',
    descEn: 'Sperm donation programme at IVI Lisboa clinic. Portugal pays a legally stipulated compensation. The process is simple and confidential, but the compensation is quite low.',
  },
  'iVidoa': {
...
const _CAT_OVERRIDES = {
  // Sonder People é conteúdo/criativo — usa chip cor criativo (rosa) para consistência visual
  'Sonder People': 'criativo',
...
const _LOCAL_EXTRA_PLATFORMS = [
  ...
  },
  {
    ...
    cat: 'f2f',
    icon: '💼',
    desc: 'Agente de seguros independente MetLife. Formação gratuita até à certificação incluída, sem custos para o candidato. Ferramentas digitais e suporte contínuo. Ganhos 100% por comissão sobre seguros de vida e acidentes pessoais.',
    descEn: 'Independent MetLife insurance agent. Free training through to certification included, at no cost to the candidate. Digital tools and ongoing support. 100% commission on life and personal accident insurance.',
  ...
  },
  {
    ...
    cat: 'f2f',
    icon: '🍳',
    desc: 'Torna-te consultor de vendas diretas da Bimby (Thermomix) da Vorwerk em Portugal. Fazes demonstrações culinárias em casa de clientes e ganhas comissão por cada equipamento vendido. Horário completamente flexível, formação incluída, sem necessidade de stock inicial.',
    descEn: 'Become a direct sales consultant for Bimby (Thermomix) by Vorwerk in Portugal. Conduct cooking demonstrations at clients\' homes and earn commission on each unit sold. Fully flexible schedule, training included, no initial stock required.',
    earn: 'Comissão por venda',
    earnN: 8,
    minPay: 'Por venda',
    pt: true, eu: false,
    url: 'https://www.vorwerk.com/pt/pt/c/home/carreiras/equipa-bimby',
    easy: 4, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: true,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'variavel', tier: 1,
  ...
  },
  {
    ...
    cat: 'criativo',
    icon: '🎬',
    desc: 'Plataforma portuguesa de castings pagos para figurantes, atores, modelos, cantores e dançarinos. Inscrição gratuita, aberta a iniciantes. +49 mil artistas registados e 300 recrutadores ativos em cinema, televisão, teatro e publicidade.',
    descEn: 'Portuguese platform for paid castings for extras, actors, models, singers and dancers. Free registration, open to beginners. 49k+ registered artists and 300 active recruiters across cinema, television, theatre and advertising.',
    earn: 'Variável por casting',
    earnN: 7,
    minPay: 'Por projeto',
    pt: true, eu: false,
    url: 'https://www.becasting.pt/castingo',
    easy: 4, geo: '🇵🇹 Portugal',
    top: false, dimmed: false, beginner: true,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'variavel', tier: 2,
    suggested: false, direct_hire: false, is_delivery: false, no_curation_tag: false,
  },
];
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  'metlife portugal',
  'equipa bimby',
  // Adicionados — segurança e casting
  'trablisa esegur',
  'becasting',
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    try {
      let _storedFavs = JSON.parse(localStorage.getItem('gh_favs') || '[]');
      // If localStorage is empty (e.g. iOS Safari ITP cleared it), fall back to cookie
      if (!Array.isArray(_storedFavs) || _storedFavs.length === 0) {
        _storedFavs = _loadFavsCookie();
        // Re-populate localStorage from cookie so the rest of the session works normally
        if (_storedFavs.length > 0) {
