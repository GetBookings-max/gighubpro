...
// ── CONTACT CONFIG ───────────────────────────────────────────────
// Values assembled at runtime to reduce plaintext exposure in source code.
// This is NOT cryptographic protection — it prevents casual grep/scraping.
// To update: run in browser console → btoa('newvalue')
const _CONTACT = (function(){
  const _d = s => atob(s);
  const ph   = _d('OTM4NTU2ODAz');           // 938556803
  const waPh = _d('MzUxOTM4NTU2ODAz');        // 351938556803
  const iban = _d('UFQ1MDAwMjMwMDAwNDU2OTcxOTI0NzA5NA=='); // PT50002300004569719247094
  return {
    phoneDisplay : ph.replace(/(\d{3})(\d{3})(\d{3})/, '$1 $2 $3'), // '938 556 803'
    phoneWA      : waPh,
    ibanRaw      : iban,
    ibanFmt      : iban.replace(/(.{4})/g, '$1 ').trim()              // 'PT50 0023 ...'
  };
})();
...
// Build WA URLs using _CONTACT so phone is never hardcoded as a literal
function _waAccessUrl(lang) {
  const amt = PRICE_PER_ACCESS.toFixed(2).replace('.', ',') + '€';
  const pt = `Olá! Quero acesso ao GigHub (+100 plataformas verificadas). Acabei de enviar ${amt} via MB Way para ${_CONTACT.phoneDisplay}. Segue o comprovativo 👇`;
  const en = `Hi! I want access to GigHub (+100 verified platforms). I just sent €${PRICE_PER_ACCESS.toFixed(2)} via MB Way to ${_CONTACT.phoneDisplay}. Receipt follows 👇`;
  return `https://wa.me/${_CONTACT.phoneWA}?text=${encodeURIComponent(lang === 'en' ? en : pt)}`;
}
function _waBoostUrl() {
  const amt = PRICE_PER_BOOST.toFixed(2).replace('.', ',') + '€';
  return `https://wa.me/${_CONTACT.phoneWA}?text=${encodeURIComponent(`Oi! Acabei de pagar ${amt} pelo GigBoost. Segue o comprovativo.`)}`;
}
...
function admRender(){
  ...
  <div style="background:var(--green-pale);border:1.5px solid rgba(45,122,79,.3);border-radius:12px;padding:20px 18px;margin-bottom:16px">
    ...
    ${safeSheetUrlEncoded
      ? `<a href="${safeSheetUrlEncoded.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;height:40px;padding:0 18px;border-radius:8px;background:var(--green);color:#fff;text-decoration:none;font-size:13px;font-weight:700">
           📊 Abrir Google Sheet →
         </a>`
...
// ── DATA ──
const catLabels = {
  pt: {surveys:'Surveys',gigs:'Gigs',freelance:'Freelance',micro:'Treino de IA',testing:'Testes',criativo:'Stock & Design',conteudo:'Conteúdo',tasks:'Get-Paid-To',mystery:'Cliente Mistério',transcricao:'Transcrição',tutoring:'Tutoria',ugc:'UGC',passive:'Nicho',remote:'Emprego Remoto',petsitting:'Pet Sitting',babysitting:'Babysitting',f2f:'ONGs'},
  en: {surveys:'Surveys',gigs:'Gigs',freelance:'Freelance',micro:'AI Training',testing:'App Testing',criativo:'Stock & Design',conteudo:'Content',tasks:'Get-Paid-To',mystery:'Mystery Shopping',transcricao:'Transcription',tutoring:'Tutoring',ugc:'UGC',passive:'Niche',remote:'Remote Jobs',petsitting:'Pet Sitting',babysitting:'Babysitting',f2f:'Fundraising'}
};
let catLabel = catLabels['pt'];

// ── TAG FRAMEWORK — client-side overrides for known platforms ─────────────────
//
// CRITERIA (applied in priority order):
//
// ENTRY TAG — one of three states, never both:
//   "Seleção necessária" → waiting list, admission test, interview, degree required,
//                          invitation-only, or competitive vetting. easy <= 2 OR in _SELECTIVE_PLATFORMS.
//   "Registo simples"    → open signup, no test, no interview, fast approval.
//                          easy >= 4 OR in _EASY_ENTRY_PLATFORMS (and NOT selective).
//   [nothing]            → easy = 3, not in either override set (neutral entry).
//
// EARNINGS TAG — one of two states:
//   "Ganhos estáveis"    → predictable income once active (deliveries, scheduled caregiving,
//                          consistent micro-tasks). r.realistic >= 4 OR in _STABLE_EARNINGS_PLATFORMS.
//   "Ganhos variáveis"   → earnings depend on study availability, demand, commissions, etc.
//                          r.realistic <= 2 OR in _VARIABLE_EARNINGS_PLATFORMS.
//   [fallback]           → if a platform would otherwise show ZERO tags, apply category default.
//
// SUGGESTED TAG — from curated _SUGGESTED_SET, shown in addition to the above.
// Max visible tags per card: 2 (suggested counts as 1; entry + earnings = the other 1).
// ─────────────────────────────────────────────────────────────────────────────

// Platforms requiring test / waitlist / interview / competitive vetting
const _SELECTIVE_PLATFORMS = new Set([
  'Prolific',
...
// Platforms with open, fast registration — no test or interview required
const _EASY_ENTRY_PLATFORMS = new Set([
  'AttaPoll','Netsonda','YouGov','LifePoints','Triaba',
  'ZapSurveys','Zap Surveys','Swagbucks','ySense','NiceQuest',
  'Freecash','WearTesters','Wearetesters',
  'Uber Eats (Courier)','Glovo','Bolt Food (Courier)',
  'Babysits','Rover','PetBacker',
  'Clickworker','Superprof','Ko-fi','Substack','Preply',
  'YouGov','Insense','Yoopies',
]);

// Platforms with stable/consistent earnings once accepted
const _STABLE_EARNINGS_PLATFORMS = new Set([
  'Uber Eats (Courier)','Glovo','Bolt Food (Courier)',
  'Clickworker','Preply','Superprof','Cambly',
]);

// Platforms with highly variable/unpredictable earnings
const _VARIABLE_EARNINGS_PLATFORMS = new Set([
  'Freecash','ySense','Swagbucks','AttaPoll',
  'UserInterviews','Respondent.io','Respondent','Prolific',
  'Etsy','Adobe Stock','Shutterstock',
  'Substack','Ko-fi','Medium',
  'dscout','PlaytestCloud','TestingTime',
  'Triaba','NiceQuest','ZapSurveys','Zap Surveys',
]);

// Category-level earnings fallback — used only when no tag would otherwise show
const _CAT_EARNINGS_FALLBACK = {
  gigs:'stable', petsitting:'stable', babysitting:'stable', f2f:'stable', micro:'stable',
  surveys:'variable', testing:'variable', criativo:'variable',
  conteudo:'variable', tasks:'variable', ugc:'variable',
  passive:'variable', transcricao:'variable', mystery:'variable',
  tutoring:'variable', freelance:'variable', remote:'variable',
};
...
function renderRatings(p){
  ...
  // ── 1. Suggested starter (curated set) ───────────────────────────
  const suggestedTag = _SUGGESTED_SET.has(p.name)
    ? `<span class="rtag rtag-suggested">✦ ${isEn?'Good to start with':'Sugestão para começar'}</span>`
    : '';

  // ── 2. Entry difficulty — mutually exclusive ──────────────────────
  // Selective overrides easy score; _EASY_ENTRY_PLATFORMS never overrides selective
  const isSelective = _SELECTIVE_PLATFORMS.has(p.name) || p.easy <= 2;
  const isEasyEntry  = !isSelective && (_EASY_ENTRY_PLATFORMS.has(p.name) || p.easy >= 4);
  const entryTag = isSelective
    ? `<span class="rtag rtag-selective">🎯 ${isEn?'Selective entry':'Seleção necessária'}</span>`
    : isEasyEntry
      ? `<span class="rtag rtag-entry-easy">🟢 ${isEn?'Simple sign-up':'Registo simples'}</span>`
      : '';

  // ── 3. Earnings predictability — mutually exclusive ───────────────
  const isStable   = _STABLE_EARNINGS_PLATFORMS.has(p.name)   || r.realistic >= 4;
  const isVariable = !isStable && (_VARIABLE_EARNINGS_PLATFORMS.has(p.name) || r.realistic <= 2);
  let earningsTag = isStable
    ? `<span class="rtag rtag-realistic-high">⚡ ${isEn?'Stable earnings':'Ganhos estáveis'}</span>`
    : isVariable
      ? `<span class="rtag rtag-realistic-low">📊 ${isEn?'Variable earnings':'Ganhos variáveis'}</span>`
      : '';

  // ── 4. Fallback: guarantee at least one visible indicator ─────────
  if(!suggestedTag && !entryTag && !earningsTag) {
    const fb = _CAT_EARNINGS_FALLBACK[p.cat];
    earningsTag = fb === 'stable'
      ? `<span class="rtag rtag-realistic-high">⚡ ${isEn?'Stable earnings':'Ganhos estáveis'}</span>`
      : `<span class="rtag rtag-realistic-low">📊 ${isEn?'Variable earnings':'Ganhos variáveis'}</span>`;
  }

  const tags = suggestedTag + entryTag + earningsTag;
...
function render(){
  ...
  grid.innerHTML=list.map((p,i)=>{
    ...
    <div class="card ${cardClass}" role="button" aria-label="${_xss(p.name)}" data-domain="${_xss(domain)}" data-url="${_xss(effectiveUrl||'')}" tabindex="0" style="animation-delay:${Math.min(i,16)*.025}s">
      <div class="card-top">
        <div class="card-ico" aria-hidden="true">${_xss(p.icon||'📌')}</div>
...
const translations = {
  pt: {
    ...
    lstat2: '13 Categorias diferentes',
    lstat3: 'Acesso único, sem subscrição',
    lockFlagGlobal: 'Global',
    lockCatAI: '🧠 Treino IA',
    lockCatCreative: '📸 Criativo',
    lockCatContent: '✍️ Conteúdo',
    lockPayText: `Para obter acesso envia 24,99€ por MB Way ou transferência para<br><a href="https://wa.me/${_CONTACT.phoneWA}" target="_blank" rel="noopener noreferrer" style="color:#25D166;text-decoration:none">WhatsApp → ${_CONTACT.phoneDisplay}</a>`,
    lockVerifying: 'A verificar acesso…',
    lockEnter: 'Entrar →',
    lockStep1Label: 'Como obter acesso',
    lockStep1Desc: `Envia <strong>24,99€ via MB Way</strong> para <strong>${_CONTACT.phoneDisplay}</strong> e clica no botão. Recebes a tua chave de acesso em minutos.`,
    ...
    monoDesc: `Envia 24,99€ por MB Way para <strong>${_CONTACT.phoneDisplay}</strong> e envia o comprovativo via WhatsApp. Recebes a chave de acesso em minutos. Os recibos partilhados podem conter dados pessoais — ver <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Política de Privacidade</a>.`,
  ...
  // GigBoost
    ...
    boostPayInstr: `Envia <strong>4,99€</strong> via <strong>MB Way</strong> para <strong>${_CONTACT.phoneDisplay}</strong> e envia o comprovativo no WhatsApp.<br>O acesso é enviado após confirmação do pagamento.`,
    ...
    calcOpt2: 'Surveys & Get-Paid-To (2€/h)',
    calcOpt6: 'Entregas & Condução (6€/h)',
    calcOpt7: 'Treino de IA & Microtarefas (7€/h)',
    calcOpt10: 'Tutoria, Freelance & Biscates (10€/h)',
    calcOpt8: 'Pet Sitting & Babysitting (8€/h)',
    calcOpt18: 'Testes de Usabilidade & Entrevistas UX (pagamento variável, vagas limitadas em PT)',
  ...
  en: {
    ...
    lstat2: '13 Different categories',
    lstat3: 'One-time access, no subscription',
    lockFlagGlobal: 'Global',
    lockCatAI: '🧠 AI Training',
    lockCatCreative: '📸 Creative',
    lockCatContent: '✍️ Content',
    lockPayText: `To get access send €24.99 via MB Way to <strong>${_CONTACT.phoneDisplay}</strong> or by bank transfer — then click the button below.`,
    ...
    monoDesc: `Send €24.99 via MB Way to <strong>${_CONTACT.phoneDisplay}</strong> and share the receipt via WhatsApp. You'll receive the access key in minutes. Receipts shared may contain personal data — see <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Privacy Policy</a>.`,
    ...
    lockStep1Desc: `Send <strong>€24.99 via MB Way</strong> to <strong>${_CONTACT.phoneDisplay}</strong> and click the button. You will receive your access key in minutes.`,
  ...
  // GigBoost
    ...
    boostPayInstr: `Send <strong>€4.99</strong> via <strong>MB Way</strong> to <strong>${_CONTACT.phoneDisplay}</strong> and send the receipt on WhatsApp.<br>Access is sent after payment confirmation.`,
    ...
    calcOpt2: 'Surveys & Get-Paid-To (€2/h)',
    calcOpt6: 'Deliveries & Driving (€6/h)',
    calcOpt7: 'AI Training & Micro-tasks (€7/h)',
    calcOpt10: 'Tutoring, Freelance & Local Gigs (€10/h)',
    calcOpt8: 'Pet Sitting & Babysitting (€8/h)',
    calcOpt18: 'Usability Testing & UX Interviews (variable pay, limited slots in PT)',
...
function applyLockLang(){
  ...
  setText('lockCatF2F',        '🤝 Angariação de Fundos',  '🤝 F2F Fundraising');
  ...
  if(desc1) desc1.innerHTML = isEn
    ? `Send <strong>€24.99 via MB Way</strong> to <strong>${_CONTACT.phoneDisplay}</strong> and click the button. You will receive your access key in minutes.`
    : `Envia <strong>24,99€ via MB Way</strong> para <strong>${_CONTACT.phoneDisplay}</strong> e clica no botão. Recebes a tua chave de acesso em minutos.`;
...
function applyLang(){
  ...
  const _TAB_SHORT = {
    pt:{'':'Todas','surveys':'📝 Inquéritos','testing':'🔬 Testes','micro':'🤖 Treino IA',
      ...
      'f2f':'🤝 Angariação de Fundos','other':'💚 Outros'},
...
Preferências: ${prefs||'-'}`
  ...
  const waLink = `https://wa.me/${_CONTACT.phoneWA}?text=${msg}`;
  ...
  setTimeout(() => {
    if(waLink.startsWith(`https://wa.me/${_CONTACT.phoneWA}?`)) {
...
const _SUGGESTED_SET = new Set([
  ...
  // 🤝 ONGs
  'Amnistia Internacional','WWF Portugal','Operação Nariz Vermelho',
  // 💚 Nicho
...
const _REDUCED_EXPECTATIONS = {
  ...
  'Preply': {
    desc: 'Decide quando e quantas horas queres ensinar. Os tutores mais populares ganham até $550 por semana. Para tutores recém-registados: começa com uma taxa horária mais baixa para conseguir alunos mais rápido. Podes alterar a tua taxa a qualquer altura.',
    descEn: 'Decide when and how many hours you want to teach. The most popular tutors earn up to $550 a week. For newly registered tutors: start with a lower hourly rate to get students faster. You can change your rate anytime.'
  },
  'CourseHero': {
  ...
  // ── Research / UX ─────────────────────────────────────────────────────────
  'dscout': {
    desc: 'Quanto mais extensa a tarefa, maior o pagamento. Seja um compromisso de curto prazo ou entrevistas aprofundadas, o teu tempo será devidamente recompensado (média de 1$ por minuto).',
    descEn: 'The more extensive the task, the larger your payment will be. Whether you\'re looking for a short-term commitment or in-depth interviews, your time will be properly rewarded (average 1$ per minute).'
  },
  '99designs': {
  ...
  'Hotjar Engage': {
    desc: 'O valor do pagamento depende da duração da entrevista e do tipo de teste. Serás sempre informado sobre o pagamento no email de convite. Os pagamentos por testes concluídos serão enviados dentro de 7 dias.',
    descEn: 'The payment amount depends on the length of the interview and the type of test. You will always be informed about the payment in the invitation email. Payments for completed tests will be sent within 7 days.'
  },
  'Hotjar': {
    desc: 'O valor do pagamento depende da duração da entrevista e do tipo de teste. Serás sempre informado sobre o pagamento no email de convite. Os pagamentos por testes concluídos serão enviados dentro de 7 dias.',
    descEn: 'The payment amount depends on the length of the interview and the type of test. You will always be informed about the payment in the invitation email. Payments for completed tests will be sent within 7 days.'
  ...
  'Yoopies': {
    desc: 'Cuidados infantis, aulas particulares e aulas de reforço, serviços de limpeza, cuidados domiciliários e cuidados com animais de estimação.',
    descEn: 'Childcare, private tuition and tutoring, cleaning services, home care and pet care.'
  },
  // ── Remote Jobs ───────────────────────────────────────────────────────────
  'HomeFromCollege': {
    desc: 'Funções digitais e de marketing de nível inicial, maioritariamente nos EUA mas também com algumas oportunidades remotas.',
    descEn: 'Entry-level digital and marketing roles, mostly US but there are also some remote opportunities.'
  },
  'Home From College': {
    desc: 'Funções digitais e de marketing de nível inicial, maioritariamente nos EUA mas também com algumas oportunidades remotas.',
    descEn: 'Entry-level digital and marketing roles, mostly US but there are also some remote opportunities.'
...
const _DESC_SUFFIX = {
  'ACNUR':             ' Base rate from €5 per hour. Performance incentives based on achieved results.',
  'APDES':             ' PT only.',
  'Aldeias SOS':       ' PT only.',
};
...
const _DESC_PATCH = {
  'Associação Salvador': {
    ...
    to:   'Base salary plus performance-based fees with training and support. PT only.'
  },
  'Associacao Salvador': {
    from: 'part-time available',
    to:   'Base salary plus performance-based fees with training and support. PT only.'
...
const _CAT_OVERRIDES = {
  ...
  'Home from College':  'remote',
  // Mystery shopping / field platforms: face-to-face, not remote
  'More Results':       'f2f',
  'More results':       'f2f',
  'Pontis':             'f2f',
  'Qualidade 21':       'f2f',
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">Não usamos cookies de rastreamento nem ferramentas de analytics de terceiros. O acesso é gerido por tokens únicos sem identificação pessoal. As fontes tipográficas são carregadas a partir do serviço Google Fonts, o que implica uma ligação aos servidores da Google. Consulta a <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer" style="color:var(--gold)">Política de Privacidade da Google</a>.</p>
...
<p style="font-size:13px;color:var(--grey);line-height:1.7;margin-bottom:12px">We do not use advertising trackers or invasive third-party analytics tools. Access is managed by unique tokens without personal identification. Typefaces are loaded from Google Fonts, which involves a connection to Google's servers. See <a href="https://policies.google.com/privacy" target="_blank" rel="noopener noreferrer" style="color:var(--gold)">Google's Privacy Policy</a>.</p>
...
(function _bindEvents(){
  ...
  if(copyIbanBtn) copyIbanBtn.addEventListener('click', function() {
    navigator.clipboard.writeText(_CONTACT.ibanRaw).then(() => {
      this.textContent = currentLang === 'en' ? '✓ Copied' : '✓ Copiado';
      setTimeout(() => { this.textContent = currentLang === 'en' ? 'Copy' : 'Copiar'; }, 1800);
    });
  });

  // ── Populate WA links and IBAN at runtime (phone/IBAN never in static HTML) ──
  (function _injectContactInfo() {
    // IBAN display
    const ibanEl = document.getElementById('ibanDisplay');
    if(ibanEl) ibanEl.textContent = _CONTACT.ibanFmt;
    // Monetization section WA link
    const monoWa = document.getElementById('monoWaLink');
    if(monoWa) monoWa.href = _waAccessUrl(currentLang);
    // Lock screen WA button
    const lockWa = document.getElementById('lockWaBtn');
    if(lockWa) lockWa.href = _waAccessUrl(currentLang);
    // Boost pay WA button
    const boostWa = document.getElementById('boostWaBtn');
    if(boostWa) boostWa.href = _waBoostUrl();
  })();
      ...
      function _updateGdprText(){
        if(textEl){
          textEl.innerHTML = currentLang === 'en'
            ? 'This site uses localStorage for preferences, favourites and your access token — no tracking cookies or third-party analytics. <a href="#" data-modal="privacy">Privacy Policy</a>'
            : 'Este site usa localStorage para preferências, favoritos e token de acesso — sem cookies de rastreamento nem analytics de terceiros. <a href="#" data-modal="privacy">Política de Privacidade</a>';
        }
  ...
  (async function() {
    ...
    if(t) {
      // Clear hash IMMEDIATELY — token must never sit in the URL bar or browser history,
      // regardless of whether validation succeeds or fails.
      try { history.replaceState(null, '', location.pathname + (location.search || '')); } catch(e) {}
      await validarTokenSupabase(t);
      return;
    }
