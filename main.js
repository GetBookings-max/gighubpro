...
function _updateHeroCount(){
  ...
  if(!total) return;
  // Round down to nearest 10, show with "+" prefix (e.g. 87 → "+100")
  ...
  if(sTotalEl && sTotalEl.textContent === '—') {
    // First load: animate from 0 to total
    const _start = Date.now();
    const _dur = 600;
    const _tick = () => {
      const _pct = Math.min(1, (Date.now() - _start) / _dur);
      const _ease = 1 - Math.pow(1 - _pct, 3); // ease-out-cubic
      sTotalEl.textContent = Math.round(_ease * total);
      if(_pct < 1) requestAnimationFrame(_tick);
      else sTotalEl.textContent = total;
    };
  ...
  if(_cecEl) {
    _cecEl.textContent = total + (isEn ? ' platforms' : ' plataformas');
...
function admRender(){
    ...
    ${safeSheetUrlEncoded
      ...
      : `<div style="font-size:12px;color:var(--amber);background:var(--amber-pale);border:1px solid rgba(212,130,10,.3);border-radius:8px;padding:10px 12px;margin-bottom:10px">
         ...
         <div style="display:flex;gap:8px">
           <input id="sheetUrlInput" type="url" maxlength="500" placeholder="https://docs.google.com/spreadsheets/d/..." style="flex:1;height:36px;border:1px solid var(--border-md);border-radius:8px;padding:0 10px;font-size:12px;font-family:'Instrument Sans',sans-serif">
  ...
  <div style="background:var(--cream);border:1px solid var(--border);border-radius:10px;padding:16px 18px">
    ...
    <code style="font-size:12px;background:#fff;border:1px solid var(--border-md);border-radius:6px;padding:8px 10px;display:block;word-break:break-all;color:var(--ink)">
      ${escHtml(window.location.origin + window.location.pathname)}<strong>#key=TOKEN_AQUI</strong>
    </code>
...
// Stub — webhook status UI is not currently implemented
...
function setWorkType(val) {
  // setWorkType is kept for back-compat; work type is now a curation filter
  // Map legacy val to the new curation key
  if(val === 'f2f') setCuration('work_f2f');
  else if(val === 'remote') setCuration('work_remote');
  else setCuration('');
}
...
const translations = {
  pt: {
    ...
    heroDesc: 'Surveys pagos, freelance, treino de IA, gigs presenciais, caregiving e muito mais. Curado, verificado e atualizado — sem spam, sem afiliações.',
    ...
    lockTagline: 'Surveys, freelance, treino de IA, gigs físicos, caregiving e muito mais — curado, verificado e atualizado regularmente.',
    ...
    kbdPanelTitle: '⌨️ Atalhos de teclado',
    kbd1Desc: 'Pesquisar', kbd2Desc: 'Favoritos', kbd3Desc: 'Calculadora',
    kbd4Desc: 'Limpar pesquisa', kbd5Desc: 'Ir para o topo', kbd6Desc: 'Fechar este painel',
  ...
  en: {
    ...
    heroDesc: 'Surveys, freelance, AI training, physical gigs, caregiving and more. Curated, verified and updated — no spam, no paid placements.',
    statPt: 'Available PT',
    statCats: 'Categories',
    ...
    lockTagline: 'Surveys, freelance, AI training, gigs, caregiving and more — curated, verified and regularly updated.',
    ...
    kbdPanelTitle: '⌨️ Keyboard shortcuts',
    kbd1Desc: 'Search', kbd2Desc: 'Favourites', kbd3Desc: 'Calculator',
    kbd4Desc: 'Clear search', kbd5Desc: 'Go to top', kbd6Desc: 'Close this panel',
...
function applyLang(){
  ...
  const _kbdHintTxt = document.getElementById('kbdHintTxt');
  if(_kbdHintTxt) _kbdHintTxt.textContent = isEn ? 'Shortcuts' : 'Atalhos';
  ...
  // Keyboard panel
  const _kbdKeys = ['kbdPanelTitle','kbd1Desc','kbd2Desc','kbd3Desc','kbd4Desc','kbd5Desc','kbd6Desc'];
  _kbdKeys.forEach(k => { const el = document.getElementById(k); if(el && t[k]) el.textContent = t[k]; });
...
const _DESC_PATCH = {
  'Psicologia ULisboa': { from: 'behavioural', to: 'behavioral' },
};
...
const _CAT_OVERRIDES = {
  ...
  // Lisbon Pub Crawl é um gig de eventos/hotelaria (aparece na tab Biscates & Eventos)
  'Lisbon Pub Crawl': 'gigs',
  // Gigs de eventos, turismo e hotelaria
  'SalvaMais': 'gigs',
  'Ritmos Fortes': 'gigs',
  'New Europe Tours': 'gigs',
  'Guia Tuk Tuk': 'gigs',
  'Douro Azul': 'gigs',
  'Lisbon Boats': 'gigs',
  'Cinema City': 'gigs',
  'NOS Cinemas': 'gigs',
  'NOS Cinemas Recrutamento': 'gigs',
  'Worldpackers': 'gigs',
  // Operações, retalho e serviços → Trabalho Presencial (f2f)
  'DHL': 'f2f',
  'RGIS': 'f2f',
  'Securitas': 'f2f',
  'Prosegur': 'f2f',
  ...
  'Mercadona': 'f2f',
  'Prezero': 'f2f',
  'Jardim Zoológico': 'f2f',
  'Jardim Zoologico': 'f2f',
  'Eurest': 'f2f',
  // Novos — emprego presencial directo
  'Brisa Recrutamento': 'f2f',
  'Sonae Emprego': 'f2f',
  // Atendimento & Suporte — directo
  'Fujitsu Careers PT': 'support',
  // Angariação de Fundos / ONGs → categoria retail (agora renomeada para fundraising)
  'WWF Portugal': 'retail',
  'Amnistia Internacional': 'retail',
  'ACNUR': 'retail',
  'APDES': 'retail',
  'Aldeias SOS': 'retail',
  'Associação Salvador': 'retail',
  'Associacao Salvador': 'retail',
  // Castings → Biscates & Eventos (gigs)
  'Castings': 'gigs',
  // Afiliados → Rendimento Passivo (passive)
  'Afiliados': 'passive',
  // Tradução → Biscates & Eventos (gigs)
  'Acclaro': 'gigs',
  'TransPerfect': 'gigs',
  // Ensino → Biscates & Eventos (gig/serviço local)
  'Explicas.me': 'gigs',
  'Preply': 'gigs',
};
...
// ── Plataformas locais extra — ainda não na Supabase ─────────────────────────
// Adicionadas directamente aqui enquanto aguardam migração para a DB.
// Estas entradas são pré-formatadas (não passam por _fmt) e são concatenadas
// a P logo após o carregamento dos dados Supabase em validarTokenSupabase().
const _LOCAL_EXTRA_PLATFORMS = [
  // Todas as plataformas foram migradas para Supabase.
  // Ver: add_platforms_junho2026.sql + add_platforms_local_remanescentes.sql
  // Para adicionar novas plataformas pendentes de migração, inserir aqui temporariamente
  // no mesmo formato e adicionar o nome à _ALLOWED_PLATFORMS abaixo.
];
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  // 🕵️ Cliente Mistério
  'more results','more results (pt)','pontis','smartspotter',
  // 🎁 Get-Paid-To
  'freecash',
  // 🚗 Entregas / Condução
  ...
  // 🛠️ Biscates & Eventos
  'casamentos.pt','casamentos',
  'merytu','new europe tours','taskrabbit',
  'guia tuk tuk',
  // 🧑‍🏫 Ensino & Skills
  'explicas.me','explicasme','preply',
  ...
  'campanha tomate portugal',
  'ividador','ividoa',
  'oscar','óscar',
  'roamler',
  'radical storage','bounce',
  // 🏭 Operações & Retalho
  ...
  'douro azul',
  'lisbon boats',
  // Adicionados junho 2026
  'mota-engil',
  'pitagorica',
  'repsol',
  'parques de sintra',
  'luz saúde','luz saude',
  'lusíadas saúde','lusiadas saude',
  // Adicionados via add_platforms.sql
  'servilusa',
  // Adicionados — Fujitsu, Brisa, Sonae
  'fujitsu careers pt',
  'brisa recrutamento',
  'sonae emprego',
  // Adicionados — parceiros de encomendas, emprego, turismo
  'dhl servicepoints',
  'ups access point',
  'dpd pickup',
  'sograma',
  'getyourguide',
  'guruwalk',
  // Adicionados — recrutamento directo empresas PT
  'central de cervejas',
  'blue & green hotels',
  'barraqueiro',
  'city sightseeing',
  // Adicionados — eventos, emprego presencial
  'badoca safari park',
  'aporfest',
  'kria eventos',
  'spring events',
  'cloe events',
  'norauto',
  // Adicionados — imobiliário
  'era portugal',
  // Adicionados — seguros e vendas diretas
  'metlife portugal',
  'consultor kobold',
  // Adicionados — segurança
  'trablisa esegur',
  // Adicionados — ambiente e resíduos
  'ponto verde',
  'ecoambiente',
  // Adicionados junho 2026 — restauração, parceiros, entregas, explicações, retalho
  'grupo ibersol',
  'too good to go',
  "domino's pizza",
  "domino's pizza carreiras",
  'food delivery brands (telepizza)','food delivery brands',
  'ikea portugal',
  'leroy merlin portugal',
  'aldi campanha verão','aldi campanha verao',
  'midas portugal',
  'portugal duty free (ptdf)','portugal duty free',
  // Adicionados junho 2026 — logística, fintech, testes, mystery, criativo
  'torrestir',
  'comparajá','comparaja',
  'clicks research',
  'gbw mystery shopping','gbw',
  'crowd agência de talentos','crowd agencia de talentos',
  // Novas plataformas adicionadas
  'prio recrutamento','prio',
  'monitores oeiras',
  'makro portugal',
  'ava clinic doação',
  'psicologia ulisboa participantes','psicologia ulisboa',
  'em estudos de mercado','em-estudosdemercado',
  'lootup',
  // Adicionados — plataformas invisíveis (na DB mas fora da allowlist)
  'ava clinic',
  'makro',
  'monitores jm oeiras',
  'prio carreiras',
  // Junho 2026 — plataformas migradas para Supabase (add_platforms_junho2026.sql)
  'vinted go parceiro','ponto ctt','payshop agente',
  'zoomarine recrutamento','aqualand portugal','sirius park',
  'aquashow park','orbitur','quick casting',
  'fundação inatel','fundacao inatel','residências montepio','residencias montepio',
  'jf ajuda emprega','procme recrutamento',
  'temu afiliados','booking.com afiliados','amazon associates',
  'shopify afiliados','trade republic afiliados',
  // Junho 2026 — batch 2
  'revolut afiliados',
  'western union agente','western union agent',
  'pestana group','pestana careers',
  'xtb partnerships','xtb afiliados',
  'boost portugal',
  'living tours group','living tours',
  'newrest portugal','newrest careers',
  'quantum parks',
  'grupo primor','primor',
]);
...
function _fmt(r){
  ...
  // ── Detecção de categoria órfã ──────────────────────────────────────────────
  // Se cat for null, vazia ou desconhecida, a plataforma apareceria em "Todas"
  // mas NÃO em nenhuma tab — invisível para o utilizador.
  // Fallback: 'gigs' garante visibilidade; o console.error avisa para corrigir na DB.
  if(!r.cat || !_VALID_CATS.has(r.cat)) {
    console.error('[GigHub] Plataforma órfã — categoria inválida/nula:', r.name, '| cat recebida:', JSON.stringify(r.cat), '| Corrige na DB ou adiciona a _CAT_OVERRIDES.');
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    // ── Inject local extra platforms (not yet in Supabase) ──────────────────────
    // Merge _LOCAL_EXTRA_PLATFORMS into P, skipping any that already exist (by name).
    const _existingNames = new Set(P.map(p => p.name.toLowerCase().trim()));
    const _extras = _LOCAL_EXTRA_PLATFORMS.filter(ep => !_existingNames.has(ep.name.toLowerCase().trim()));
    if(_extras.length) P = P.concat(_extras);
    // ────────────────────────────────────────────────────────────────────────────
    ...
    const _kbdHintBtn = document.getElementById('kbdHintBtn');
    if(_kbdHintBtn) _kbdHintBtn.style.display = 'inline-flex';
...
(function _bindEvents(){
  ...
  const heroKbdBtn = document.getElementById('heroKbdBtn');
  if(heroKbdBtn) heroKbdBtn.addEventListener('click', () => {
    const p = document.getElementById('kbdPanel');
  ...
  if(catSearchEl){
    let _st;
    const _catClear = document.getElementById('catSearchClear');
    catSearchEl.addEventListener('input', () => {
      // Keep hidden #search in sync so render() reads the correct value
      if(searchEl) searchEl.value = catSearchEl.value;
      // Show/hide clear button
      if(_catClear) _catClear.style.display = catSearchEl.value ? 'block' : 'none';
      clearTimeout(_st); _st = setTimeout(() => { render(); renderCatExplorer(); }, 150);
    });
    // Clear button handler
    if(_catClear) _catClear.addEventListener('click', () => {
      catSearchEl.value = '';
      if(searchEl) searchEl.value = '';
      _catClear.style.display = 'none';
      catSearchEl.focus();
      render(); renderCatExplorer();
    });
  }
  if(searchEl && !catSearchEl){ let _st; searchEl.addEventListener('input', () => { clearTimeout(_st); _st = setTimeout(render, 150); }); }


  // ── Filter boxes — 5 independent toggle buttons, all use setCuration ──
  // fboxAll = clear all filters; fboxF2f/fboxRemote = work type; fboxStable/fboxVariable = earnings type
  function _bindFbox(id, key) {
    const el = document.getElementById(id);
    if(!el) return;
    el.addEventListener('click', () => setCuration(activeCuration === key ? '' : key));
    el.addEventListener('keydown', e => { if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setCuration(activeCuration === key ? '' : key); } });
  }
  // fboxAll clears all curation filters
  const _fboxAllEl = document.getElementById('fboxAll');
  if(_fboxAllEl) {
    _fboxAllEl.addEventListener('click', () => setCuration(''));
    _fboxAllEl.addEventListener('keydown', e => { if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setCuration(''); } });
  }
  _bindFbox('fboxF2f',      'work_f2f');
  _bindFbox('fboxRemote',   'work_remote');
  _bindFbox('fboxStable',   'earnings_stable');
  _bindFbox('fboxVariable', 'earnings_variable');
  ...
  if(tabsEl) {
    ...
    // Arrow key navigation for role=tablist accessibility
    tabsEl.addEventListener('keydown', e => {
      const tabs = Array.from(tabsEl.querySelectorAll('.tab[role="tab"]:not([style*="display:none"])'));
      ...
      if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setTab(document.activeElement.dataset.v); }
    });
  }
  ...
  if(grid) grid.addEventListener('click', function(e) {
    ...
    if(clearBtn) {
      const _srEl = document.getElementById('search');
      if(_srEl) _srEl.value = '';
      const _csEl = document.getElementById('catSearch');
      if(_csEl) _csEl.value = '';
      activeTab = ''; activeCuration = '';
      // Also reset favorites-only view if active
      if(showFavsOnly) {
        showFavsOnly = false;
        const _fb = document.getElementById('favBtn');
        if(_fb) { _fb.style.background=''; _fb.style.color=''; _fb.style.borderColor=''; }
      }
      document.querySelectorAll('.tab').forEach(t => { const isAll=t.dataset.v===''; t.classList.toggle('active',isAll); t.setAttribute('aria-selected',isAll?'true':'false'); t.setAttribute('tabindex',isAll?'0':'-1'); });
      document.querySelectorAll('.curation-pill').forEach(p => p.classList.toggle('active', p.dataset.curation === ''));

      // Reset all 5 filter boxes
      ['fboxAll','fboxF2f','fboxRemote','fboxStable','fboxVariable'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.classList.toggle('active', id === 'fboxAll');
      });
      render();
  ...
  if(hoursRange) hoursRange.addEventListener('input', function() {
  ...
  if(welcomeCloseBtn) welcomeCloseBtn.addEventListener('click', () => {
    _closeModalTrapped('welcomeModal');
    // Smooth scroll to catExplorer after closing
    setTimeout(() => {
      const ce = document.getElementById('catExplorer');
      if(ce) ce.scrollIntoView({ behavior:'smooth', block:'start' });
    }, 200);
  });
  // Quick-start buttons in welcome modal
  const _wqRemoto = document.getElementById('welcomeQuickRemoto');
  if(_wqRemoto) _wqRemoto.addEventListener('click', () => {
    _closeModalTrapped('welcomeModal');
    setCuration('work_remote');
    const _cs = document.getElementById('catSearch');
    if(_cs) _cs.scrollIntoView({ behavior:'smooth', block:'start' });
  });
  const _wqPresencial = document.getElementById('welcomeQuickPresencial');
  if(_wqPresencial) _wqPresencial.addEventListener('click', () => {
    _closeModalTrapped('welcomeModal');
    setCuration('work_f2f');
    const _cs = document.getElementById('catSearch');
    if(_cs) _cs.scrollIntoView({ behavior:'smooth', block:'start' });
  });
  const boostCtaEl = document.getElementById('boostCtaEl');
  if(boostCtaEl) boostCtaEl.addEventListener('click', () => { openBoostPay(); _closeModalTrapped('welcomeModal'); });
  ...
  if(boostOpenFormBtn) boostOpenFormBtn.addEventListener('click', async () => {
    ...
    if(!tokenInput) { _closeModalTrapped('boostPayModal'); openBoostForm(); return; }
    const raw = tokenInput.value.trim().replace(/[^A-Za-z0-9\-]/g,'').substring(0,20).toUpperCase();
    if(!raw || raw.length < 8 || !/^BOOST-[A-Z0-9]{4}-[A-Z0-9]{4}$/.test(raw)) {
  ...
  if(legalModal) {
    legalModal.addEventListener('click', e => { if(e.target === legalModal) _closeModalTrapped('legalModal'); });
  }
  ...
  if(favExportBtn) favExportBtn.addEventListener('click', () => {
    ...
    // Copy the bookmark URL to clipboard, show overlay with info
    ...
    _box.innerHTML = `
      ...
      <div style="margin-top:12px;background:rgba(201,168,76,.08);border:1px solid rgba(201,168,76,.2);border-radius:10px;padding:12px 14px;font-size:12px;color:var(--ink);line-height:1.6">
        🔗 <strong>${isEn ? 'Bookmark this link' : 'Guarda este link como marcador'}</strong> — ${isEn ? 'it restores your favourites even after clearing browser data.' : 'restaura os teus favoritos mesmo que o browser apague os dados.'}
        <div style="margin-top:8px;word-break:break-all;font-size:11px;color:var(--grey);background:var(--cream);border-radius:6px;padding:6px 8px;font-family:monospace">${escHtml(bookmarkUrl)}</div>
      </div>
      <div style="display:flex;gap:8px;margin-top:16px">
        ...
        <button id="_favCopyUrlBtn" style="flex:1;height:40px;border-radius:10px;border:none;background:var(--ink);color:var(--paper);font-family:'Instrument Sans',sans-serif;font-size:13px;font-weight:700;cursor:pointer">
          🔗 ${escHtml(isEn ? 'Copy link' : 'Copiar link')}
        </button>
    ...
    // Set textarea value safely (avoid HTML injection via platform names)
    ...
    _overlay.querySelector('#_favCopyUrlBtn').addEventListener('click', function() {
  ...
  if(kbdHintBtn) kbdHintBtn.addEventListener('click', () => {
    const p = document.getElementById('kbdPanel');
    if(p) { p.classList.add('open'); }
  });
  // Guide Step 1 card — click scrolls to catExplorer
  ...
  window.addEventListener('scroll', function(){
    ...
    if(!btn) return;
    // Back-to-top position — GDPR notice is now floating (not full-width)
    ...
    try {
      ...
      if(okBtn) {
        okBtn.addEventListener('click', function(){
          ...
          // Reposition back-to-top button now that GDPR is gone
...
const CAT_EXPLORER_DEFS = [
  {
    id: 'deliveries',
    emoji: '🚗',
    titlePt: 'Conduz ou faz entregas',
    titleEn: 'Drive or make deliveries',
    descPt: 'App no telemóvel, horário 100% livre',
    descEn: 'App on your phone, fully flexible hours',
    color: '#2d7a4f',
    platforms: [
      { name:'Uber Driver',  desc:'Conduz para o Uber', url:'https://www.uber.com/pt/pt-br/drive/', icon:'🚕' },
      { name:'Bolt Driver',  desc:'Conduz para o Bolt', url:'https://driver.bolt.eu/pt-pt/', icon:'🟡' },
      { name:'Uber Eats',    desc:'Entrega comida de bicicleta ou mota', url:'https://www.uber.com/pt/pt-br/deliver/', icon:'🍔' },
      { name:'Bolt Food',    desc:'Estafeta Bolt Food', url:'https://food.bolt.eu/pt-pt/partner/', icon:'🟡' },
      { name:'Glovo',        desc:'Estafeta multi-categoria', url:'https://glovoapp.com/pt/en/couriers/', icon:'📦' },
      { name:"Domino's Pizza", desc:'Estafeta de pizzas', url:'https://www.dominospizza.pt/careers', icon:'🍕' },
      { name:'Food Delivery Brands (Telepizza)', desc:'Entregador / operador', url:'https://jobs.telepizza.com', icon:'🍕' },
      { name:'Carteiro CTT', desc:'Carteiro sazonal / a prazo', url:'https://recrutamento.ctt.pt', icon:'✉️' },
    ]
  },
  {
    id: 'pickup',
    emoji: '📦',
    titlePt: 'Usa o teu espaço como ponto de recolha',
    titleEn: 'Turn your space into a parcel point',
    descPt: 'Tens loja ou café? Ganha por cada encomenda recebida',
    descEn: 'Have a shop or café? Earn per parcel received',
    color: '#0e64b4',
    platforms: [
      { name:'DHL ServicePoints',  desc:'Ponto de entrega e recolha DHL', url:'https://www.dhl.com/pt-pt/home/express/shipping/parcel-shop-partner.html', icon:'🟡' },
      { name:'UPS Access Point',   desc:'Ponto de acesso UPS', url:'https://www.ups.com/pt/pt/business-solutions/access-point.page', icon:'🟫' },
      { name:'DPD Pickup',         desc:'Ponto de recolha DPD', url:'https://www.dpd.com/pt/pt/ferramentas/pickup-parcelshop/', icon:'🔴' },
      { name:'Vinted Go Partner',  desc:'Ponto de entrega Vinted', url:'https://vintedgo.com/pt/become-a-partner?country=pt&region=europe', icon:'🟢' },
      { name:'Payshop',            desc:'Agente Payshop — pagamentos e entregas', url:'https://www.payshop.pt/rede-de-agentes/ser-agente-payshop/candidatura', icon:'🔵' },
      { name:'Radical Storage',    desc:'Guarda-bagagem para turistas no teu espaço', url:'https://radicalstorage.com/work-with-us', icon:'🧳' },
      { name:'Bounce',             desc:'Rede de armazenamento de bagagens', url:'https://usebounce.com/partners', icon:'🎒' },
      { name:'Western Union Agente', desc:'Ponto de agente de transferências na tua loja', url:'https://www.westernunion.com/pt/pt/become-agent.html', icon:'💸' },
    ]
  },
  {
    id: 'surveys',
    emoji: '📝',
    titlePt: 'Responde a inquéritos e testa produtos',
    titleEn: 'Fill surveys and test products',
    descPt: 'De casa, ao teu ritmo — sem experiência',
    descEn: 'From home, your pace — no experience needed',
    color: '#7a3e00',
    platforms: [
      { name:'Freecash',          desc:'Tarefas, inquéritos e jogos pagos', url:'https://freecash.com', icon:'💰' },
      { name:'SmartSpotter',      desc:'Auditorias em lojas físicas', url:'https://www.smartspotter.com', icon:'🏪' },
      { name:'Roamler',           desc:'Tarefas de campo e auditorias', url:'https://roamler.com', icon:'📍' },
      { name:'More Results',      desc:'Cliente mistério e auditoria', url:'https://www.moreresults.pt', icon:'🕵️' },
      { name:'GBW Mystery Shopping', desc:'Cliente mistério certificado', url:'https://www.gbwmystery.pt', icon:'🔍' },
      { name:'Clicks Research',   desc:'Painéis e estudos de mercado', url:'https://www.clicksresearch.com', icon:'📊' },
      { name:'Netsonda',          desc:'Inquéritos online pagos', url:'https://www.netsonda.pt', icon:'📋' },
      { name:'Crowdville',        desc:'Testes de apps e produtos', url:'https://www.crowdville.com', icon:'🧪' },
      { name:'Boutique Opiniões', desc:'Inquéritos de opinião pagos', url:'https://www.boutiqueopinioes.pt', icon:'💬' },
      { name:'Prime Opinion',     desc:'Inquéritos pagos internacionais', url:'https://primeopinion.com', icon:'⭐' },
      { name:'Opiniões de Valor', desc:'Painel de consumidores PT', url:'https://www.em-estudosdemercado.pt/painel-consumidores/', icon:'🗣️' },
      { name:'AttaPoll',          desc:'Inquéritos rápidos no telemóvel', url:'https://attapoll.app', icon:'📱' },
      { name:'Psicologia ULisboa', desc:'Participa em estudos académicos', url:'https://www.psicologia.ulisboa.pt/investigacao/recrutamento-participantes/', icon:'🧠' },
    ]
  },
  {
    id: 'summer',
    emoji: '🎪',
    titlePt: 'Trabalha ao sol — turismo, parques e festivais',
    titleEn: 'Work outdoors — tourism, parks and festivals',
    descPt: 'Sazonal, ao ar livre, ideal para junho a setembro',
    descEn: 'Seasonal, outdoors, ideal June to September',
    color: '#e84c3d',
    platforms: [
      { name:'Guia Tuk Tuk',      desc:'Guia turístico em Lisboa', url:'https://www.guiatuktuklisboa.com', icon:'🛺' },
      { name:'SalvaMais',         desc:'Nadador-salvador sazonal', url:'https://www.salvamais.pt', icon:'🏊' },
      { name:'Douro Azul',        desc:'Tripulação e serviços fluviais', url:'https://www.douroazul.com/emprego', icon:'⛴️' },
      { name:'Lisbon Boats',      desc:'Staff em passeios de barco', url:'https://www.lisbonboats.com', icon:'⛵' },
      { name:'Lisbon Pub Crawl',  desc:'Guia nocturno em Lisboa', url:'https://www.lisbonpubcrawl.com', icon:'🍺' },
      { name:'GuruWalk',          desc:'Free walking tour guide', url:'https://www.guruwalk.com', icon:'🚶' },
      { name:'City Sightseeing',  desc:'Staff em autocarros turísticos', url:'https://www.city-sightseeing.com/en/jobs', icon:'🚌' },
      { name:'New Europe Tours',  desc:'Guia de walking tours', url:'https://www.neweuropetours.eu', icon:'🗺️' },
      { name:'Badoca Safari Park', desc:'Tratador e guia de safari', url:'https://www.badoca.com/emprego', icon:'🦁' },
      { name:'Zoomarine',         desc:'Emprego sazonal no Zoomarine', url:'https://recrutamento.zoomarine.pt/Offers', icon:'🐬' },
      { name:'Aqualand',          desc:'Staff sazonal no Aqualand', url:'https://www.aqualand.pt/emprego/', icon:'💦' },
      { name:'Aquashow',          desc:'Trabalha no Aquashow Park', url:'https://aquashowpark.com/pt/trabalhar-connosco/', icon:'🎢' },
      { name:'Sirius Park',       desc:'Emprego no Sirius Park', url:'https://siriuspark.pt/trabalhar-connosco/', icon:'🦜' },
      { name:'Orbitur',           desc:'Staff em parques de campismo', url:'https://www.orbitur.pt/pt/trabalhe-connosco', icon:'⛺' },
      { name:'INATEL',            desc:'Animadores e staff de turismo social', url:'https://www.inatel.pt/pt/inatel/mandar-cv/', icon:'🏖️' },
      { name:'Kria Eventos',      desc:'Staff e produção de eventos', url:'https://www.kriaevents.pt', icon:'🎉' },
      { name:'Aporfest',          desc:'Staff em festivais de música', url:'https://www.aporfest.pt', icon:'🎵' },
      { name:'Spring Events',     desc:'Produção de eventos e concertos', url:'https://www.springevents.pt', icon:'🎤' },
      { name:'Cloe Events',       desc:'Staff em eventos nacionais', url:'https://www.cloeevents.pt', icon:'🎭' },
      { name:'Ritmos Fortes',     desc:'Staff e logística de eventos', url:'https://www.ritmosfortes.pt', icon:'🥁' },
      { name:'Monitores Oeiras',  desc:'Monitor jovem – Município de Oeiras (até 30 anos)', url:'https://fw-monitoresjm.oeiras.pt/', icon:'🏃' },
      { name:'Campanha do Tomate', desc:'Trabalho sazonal na apanha do tomate', url:'https://www.agrotejo.pt', icon:'🍅' },
      { name:'Worldpackers',      desc:'Voluntariado em troca de alojamento', url:'https://www.worldpackers.com', icon:'🌍' },
      { name:'Living Tours Group', desc:'Guias, transfers e operações turísticas em PT', url:'https://www.livingtoursgroup.com/pt/grupo-living-tours-emprego', icon:'🗺️' },
      { name:'Quantum Parks',     desc:'Staff sazonal em parques de lazer PT', url:'https://www.quantumparks.com/recrutamento/', icon:'🎡' },
      { name:'Boost Portugal',    desc:'Staff de eventos e ativações de marca', url:'https://boostportugal.com/pt/recrutamento', icon:'🚀' },
    ]
  },
  {
    id: 'commissions',
    emoji: '💸',
    titlePt: 'Promove produtos e ganha comissões',
    titleEn: 'Promote products and earn commissions',
    descPt: 'Sem salário fixo: quanto mais vendes, mais ganhas',
    descEn: 'No fixed salary: the more you sell, the more you earn',
    color: '#c9a84c',
    platforms: [
      { name:'ERA Portugal',        desc:'Consultor imobiliário independente', url:'https://www.era.pt/recrutamento', icon:'🏠' },
      { name:'MetLife',             desc:'Consultor de seguros de vida', url:'https://www.metlife.pt/carreiras', icon:'🛡️' },
      { name:'Consultor Kobold',    desc:'Venda direta de produtos de limpeza', url:'https://www.kobold.vorwerk.pt', icon:'🧹' },
      { name:'Temu Affiliates',     desc:'Programa de afiliados Temu', url:'https://www.temu.com/pt/affiliate_recruit.html', icon:'🛍️' },
      { name:'Booking.com Affiliates', desc:'Comissões em reservas de hotel', url:'https://www.booking.com/affiliate-program/v2/index.pt-pt.html', icon:'🏨' },
      { name:'Amazon Associates',   desc:'Afiliados Amazon', url:'https://affiliate-program.amazon.com/', icon:'📦' },
      { name:'Shopify Affiliates',  desc:'Comissões por referir Shopify', url:'https://www.shopify.com/pt/afiliados', icon:'🛒' },
      { name:'Trade Republic Affiliates', desc:'Afiliados de investimento', url:'https://traderepublic.com/pt-pt/afiliados', icon:'📈' },
      { name:'GetYourGuide',        desc:'Afiliados de atividades turísticas', url:'https://partner.getyourguide.com', icon:'🗺️' },
      { name:'Revolut Afiliados',   desc:'Comissão por cada conta Revolut referenciada', url:'https://www.revolut.com/en-US/become-a-revolut-affiliate/', icon:'💳' },
      { name:'XTB Partnerships',    desc:'Afiliados e IB da corretora XTB', url:'https://www.xtb.com/pt/partnerships', icon:'📈' },
    ]
  },
  {
    id: 'clinical',
    emoji: '🧪',
    titlePt: 'Participa em estudos médicos ou científicos',
    titleEn: 'Participate in medical or scientific studies',
    descPt: 'Por projeto — podem pagar centenas de euros',
    descEn: 'Per project — can pay hundreds of euros',
    color: '#0a6ba8',
    platforms: [
      { name:'BlueClinical',      desc:'Ensaios clínicos pagos em Portugal', url:'https://www.blueclinical.com', icon:'💊' },
      { name:'iVidador',          desc:'Estudos de saúde e bem-estar', url:'https://www.ividador.pt', icon:'🩺' },
      { name:'iVidoa',            desc:'Investigação clínica participativa', url:'https://www.ividoa.pt', icon:'🔬' },
      { name:'AVA Clinic',        desc:'Dadora de óvulos — informação e compensação', url:'https://avaclinic.pt/doar/', icon:'🏥' },
    ]
  },
  {
    id: 'gigs_services',
    emoji: '🛠️',
    titlePt: 'Faz biscates e presta serviços',
    titleEn: 'Do odd jobs and offer services',
    descPt: 'Montagens, reparações, tutoria, pets, babysitting',
    descEn: 'Assembly, repairs, tutoring, pets, babysitting',
    color: '#8228b4',
    platforms: [
      { name:'Óscar',       desc:'Plataforma PT de biscates e serviços', url:'https://www.oscar.pt', icon:'🔨' },
      { name:'TaskRabbit',  desc:'Tarefas domésticas e montagens', url:'https://www.taskrabbit.com', icon:'🔧' },
      { name:'Merytu',      desc:'Serviços domésticos verificados', url:'https://www.merytu.com', icon:'🏠' },
      { name:'Preply',      desc:'Ensino de idiomas online', url:'https://preply.com/become-a-tutor', icon:'🧑‍🏫' },
      { name:'Explicas.me', desc:'Explicações e tutoria PT', url:'https://explicas.me', icon:'📚' },
      { name:'Rover',       desc:'Pet sitting e passeios de cão', url:'https://www.rover.com', icon:'🐕' },
      { name:'PetBacker',   desc:'Cuidados com animais de estimação', url:'https://www.petbacker.com', icon:'🐾' },
      { name:'Babysits',    desc:'Babysitting e cuidados infantis', url:'https://www.babysits.pt', icon:'👶' },
      { name:'Crowd',       desc:'Micro-tarefas e trabalho remoto', url:'https://crowd.com', icon:'👥' },
      { name:'Quickcasting', desc:'Trabalho de figurante e casting', url:'https://www.quickcasting.pt/', icon:'🎬' },
      { name:'TransPerfect', desc:'Tradução e localização freelance', url:'https://www.transperfect.com/careers', icon:'🌐' },
      { name:'Acclaro',     desc:'Tradução e revisão freelance', url:'https://www.acclaro.com/about/careers', icon:'📝' },
      { name:'Casamentos.pt', desc:'Fotógrafo, DJ e serviços para casamentos', url:'https://www.casamentos.pt/fornecedores', icon:'💍' },
    ]
  },
  {
    id: 'fundraising',
    emoji: '🚶‍♂️',
    titlePt: 'Angaria fundos para ONGs na rua',
    titleEn: 'Street fundraising for NGOs',
    descPt: 'Salário fixo + bónus, entrada fácil, sem experiência',
    descEn: 'Fixed salary + bonus, easy entry, no experience',
    color: '#1a7a3a',
    platforms: [
      { name:'WWF Portugal',         desc:'Angariador de fundos WWF', url:'https://wwf.pt/o-que-fazemos/trabalha-connosco/', icon:'🐼' },
      { name:'ACNUR',                desc:'Angariador de donativos ACNUR', url:'https://www.acnur.org/pt/angariacao', icon:'🕊️' },
      { name:'Aldeias SOS',          desc:'Angariador Aldeias de Crianças SOS', url:'https://www.aldeiassos.pt/participa/trabalha-connosco', icon:'👨‍👩‍👧' },
      { name:'APDES',                desc:'Trabalho social e angariação', url:'https://www.apdes.pt', icon:'🤝' },
      { name:'Associação Salvador',  desc:'Angariador de fundos Salvador', url:'https://www.associacaosalvador.com', icon:'♿' },
      { name:'Amnistia Internacional', desc:'Angariador de direitos humanos', url:'https://www.amnistia.pt/trabalha-connosco/', icon:'🕯️' },
      { name:'Ponto Verde',          desc:'Sensibilizador ambiental', url:'https://www.pontoverde.pt/recrutamento', icon:'♻️' },
    ]
  },
  {
    id: 'retail_stores',
    emoji: '🏪',
    titlePt: 'Trabalha numa loja, café ou supermercado',
    titleEn: 'Work at a shop, café or supermarket',
    descPt: 'Contrato de trabalho, formação incluída',
    descEn: 'Employment contract, training included',
    color: '#5a3a0a',
    platforms: [
      { name:'Mercadona',       desc:'Operador de loja Mercadona', url:'https://empleo.mercadona.es/pt', icon:'🛒' },
      { name:'IKEA',            desc:'Co-worker IKEA Portugal', url:'https://www.ikea.com/pt/pt/jobs/', icon:'🪑' },
      { name:'Leroy Merlin',    desc:'Colaborador Leroy Merlin', url:'https://www.leroymerlin.pt/emprego', icon:'🔩' },
      { name:'Sonae',           desc:'Colaborador Continente / Sonae', url:'https://careers.sonae.pt', icon:'🏢' },
      { name:'Grupo Ibersol',   desc:'Staff de restauração e hotelaria', url:'https://www.ibersol.pt/pt/carreiras', icon:'🍽️' },
      { name:'Portugal Duty Free', desc:'Staff em duty-free de aeroportos', url:'https://ptdf.pt/carreiras', icon:'✈️' },
      { name:'Makro',           desc:'Colaborador Makro cash & carry', url:'https://talento.makro.pt/jobs', icon:'📦' },
      { name:'Repsol',          desc:'Operador de posto de combustível', url:'https://www.repsol.com/pt/carreiras', icon:'⛽' },
      { name:'Aldi Campanha Verão', desc:'Operador sazonal de verão', url:'https://emprego.aldi.pt', icon:'🛒' },
      { name:'Grupo Nabeiro',   desc:'Staff Delta Cafés e hotelaria', url:'https://www.gruponabeiro.com/pt/recursos-humanos', icon:'☕' },
      { name:'Salesland',       desc:'Promotor de vendas em loja', url:'https://www.salesland.net/emprego', icon:'🏬' },
      { name:'RGIS',            desc:'Inventário e auditoria de stock em lojas', url:'https://careers.rgis.com', icon:'📊' },
      { name:'Central de Cervejas', desc:'Operador e logística de bebidas', url:'https://centraldecerevieja.pt', icon:'🍺' },
      { name:'Norauto',         desc:'Mecânico e staff de autopeças', url:'https://www.norauto.pt/emprego', icon:'🚗' },
      { name:'Midas',           desc:'Mecânico e técnico automóvel', url:'https://www.midas.pt/emprego', icon:'🔧' },
      { name:'Too Good To Go',  desc:'Parceiro de venda de excedentes alimentares', url:'https://toogoodtogo.com/pt/business', icon:'🍱' },
      { name:'Newrest',         desc:'Restauração coletiva — aeroportos, hospitais, empresas', url:'https://careers.newrest.eu/en/annonces', icon:'🍽️' },
      { name:'Grupo Primor',    desc:'Lojas de cosmética e perfumaria — atendimento em loja', url:'https://grupoprimor.pt/category/oportunidades/', icon:'💄' },
    ]
  },
  {
    id: 'callcenter',
    emoji: '📞',
    titlePt: 'Atende clientes por telefone — salário fixo',
    titleEn: 'Customer support by phone — fixed salary',
    descPt: 'Sem experiência — a empresa forma e paga desde o 1.º dia',
    descEn: 'No experience — company trains and pays from day one',
    color: '#0a5a5a',
    platforms: [
      { name:'Accenture',         desc:'Customer service e BPO', url:'https://www.accenture.com/pt-pt/careers', icon:'🔵' },
      { name:'Teleperformance',   desc:'Maior call center em Portugal', url:'https://jobs.teleperformance.com', icon:'📱' },
      { name:'Concentrix',        desc:'Suporte ao cliente multilingue', url:'https://jobs.concentrix.com', icon:'🎧' },
      { name:'Foundever',         desc:'Customer experience multilingue', url:'https://jobs.foundever.com', icon:'💼' },
      { name:'Indiecampers',      desc:'Customer service de autocaravanas', url:'https://www.indiecampers.com/careers', icon:'🚐' },
      { name:'Fujitsu Careers',   desc:'Suporte técnico e IT service desk', url:'https://careers.fujitsu.com', icon:'💻' },
    ]
  },
  {
    id: 'transport',
    emoji: '✈️',
    titlePt: 'Motorista, handling aeroportuário ou obras',
    titleEn: 'Driver, airport handling or construction',
    descPt: 'Transportes públicos, aviação e infraestruturas',
    descEn: 'Public transport, aviation and infrastructure',
    color: '#1a4a8a',
    platforms: [
      { name:'Ryanair',                    desc:'Tripulação de cabine e staff de terra', url:'https://jobs.ryanair.com', icon:'🛩️' },
      { name:'Portway',                    desc:'Handling aeroportuário em Portugal', url:'https://www.portway.pt/pt/carreiras', icon:'✈️' },
      { name:'Transdev',                   desc:'Motorista e staff de transportes públicos', url:'https://www.transdev.pt/candidaturas', icon:'🚌' },
      { name:'Barraqueiro',                desc:'Motorista de autocarros urbanos e rodoviários', url:'https://www.barraqueiro.pt/emprego', icon:'🚍' },
      { name:'Torrestir',                  desc:'Motorista e logística de transportes', url:'https://www.torrestir.pt/carreiras', icon:'🚛' },
      { name:'Brisa',                      desc:'Operador de portagem e autoestradas', url:'https://www.brisa.pt/carreiras', icon:'🛣️' },
      { name:'Infraestruturas de Portugal', desc:'Gestão de infraestruturas rodoviárias e ferroviárias', url:'https://www.infraestruturasdeportugal.pt/sobre-nos/carreiras', icon:'🛤️' },
      { name:'Mota-Engil',                 desc:'Construção civil e obras públicas', url:'https://www.mota-engil.pt/carreiras', icon:'🏗️' },
      { name:'Pitagorica',                 desc:'Engenharia e tecnologia de infraestrutura', url:'https://www.pitagorica.pt/carreiras', icon:'💡' },
    ]
  },
  {
    id: 'security',
    emoji: '🔒',
    titlePt: 'Trabalha em segurança privada',
    titleEn: 'Work in private security',
    descPt: 'Formação paga pela empresa, contrato de trabalho',
    descEn: 'Company-paid training, employment contract',
    color: '#3a3a5a',
    platforms: [
      { name:'Prosegur',       desc:'Segurança privada e transporte de valores', url:'https://www.prosegur.pt/quero-trabalhar-na-prosegur', icon:'🔒' },
      { name:'Securitas',      desc:'Segurança privada e vigilância', url:'https://www.securitas.pt/recrutamento/', icon:'🛡️' },
      { name:'Trablisa ESEgur', desc:'Segurança e vigilância privada', url:'https://www.esegur.pt/carreiras', icon:'🔐' },
    ]
  },
  {
    id: 'health',
    emoji: '🏥',
    titlePt: 'Trabalha em saúde ou cuida de pessoas',
    titleEn: 'Work in healthcare or caregiving',
    descPt: 'Hospitais privados, lares e clínicas — contrato',
    descEn: 'Private hospitals, care homes and clinics',
    color: '#0a6b6b',
    platforms: [
      { name:'Luz Saúde',            desc:'Auxiliar e staff de saúde privada (Grupo Luz)', url:'https://www.luzsaude.pt/pt/trabalhar-connosco', icon:'🏥' },
      { name:'Lusíadas Saúde',       desc:'Auxiliar e staff hospitalar Lusíadas', url:'https://www.lusiadassaude.pt/carreiras', icon:'🩺' },
      { name:'Residências Montepio', desc:'Auxiliar em lares e residências sénior', url:'https://residenciasmontepio.pt/recrutamento/', icon:'🏠' },
      { name:'Servilusa',            desc:'Agente funerário e serviços de apoio', url:'https://www.servilusa.pt/emprego', icon:'🌷' },
    ]
  },
  {
    id: 'misc_local',
    emoji: '🌍',
    titlePt: 'Outras oportunidades presenciais',
    titleEn: 'Other local opportunities',
    descPt: 'Parques, cinemas, casinos, câmaras e muito mais',
    descEn: 'Parks, cinemas, casinos, municipalities and more',
    color: '#4a6a2a',
    platforms: [
      { name:'Parques de Sintra',  desc:'Staff e guias em Parques de Sintra — Mon. Nac.', url:'https://www.parquesdesintra.pt/institucional/trabalhar-connosco/', icon:'🌿' },
      { name:'Jardim Zoológico',   desc:'Staff e tratadores no Zoo de Lisboa', url:'https://www.zoo.pt/pt/zoo/trabalha-connosco', icon:'🦁' },
      { name:'Solverde',           desc:'Staff em casinos e hotelaria Solverde', url:'https://www.solverde.pt/emprego', icon:'🎰' },
      { name:'Blue & Green Hotels', desc:'Staff hoteleiro Blue & Green', url:'https://www.blueandgreenhotels.com/trabalhar-connosco', icon:'🏨' },
      { name:'Prezero',            desc:'Gestão de resíduos e reciclagem', url:'https://www.prezero.com/pt/carreiras', icon:'♻️' },
      { name:'Cinema City',        desc:'Staff de cinema — bilheteira e bar', url:'https://www.cinemacity.pt/careers', icon:'🎬' },
      { name:'NOS Cinemas',        desc:'Staff de cinema NOS Cinemas', url:'https://www.noscinemas.pt/emprego', icon:'🎥' },
      { name:'ComparaJá',          desc:'Consultor de produtos financeiros', url:'https://www.comparaja.pt/careers', icon:'💳' },
      { name:'JF Ajuda',           desc:'Oportunidades de emprego na JF Ajuda (Lisboa)', url:'https://www.jf-ajuda.pt/freguesia/oportunidades', icon:'🏛️' },
      { name:'Procme',             desc:'Ofertas de emprego Procme', url:'https://recrutamento.procme.pt/Offers', icon:'🏢' },
      { name:'Pestana Group',      desc:'Hotéis premium em Portugal e Brasil — todas as áreas', url:'https://careers.pestanagroup.com/go/OPORTUNIDADES-DE-TRABALHO/3480501/', icon:'🏨' },
    ]
  },
];
...
function renderCatExplorer() {
  ...
  // Build active filter predicates from current state
  ...
  const _cecEl = document.getElementById('catExplorerCount');
  if(_cecEl && P.length) {
    const _total = P.length;
    const _vis = curationFn ? P.filter(curationFn).length : _total;
    _cecEl.textContent = (_vis < _total)
      ? _vis + ' ' + (isEn ? 'of ' : 'de ') + _total + (isEn ? ' platforms' : ' plataformas')
      : _total + (isEn ? ' platforms' : ' plataformas');
    _cecEl.style.display = '';
  }
  // Map platform name → DB platform object (for filter matching)
  const _pByName = {};
  P.forEach(p => { _pByName[p.name.toLowerCase().trim()] = p; });

  // Does a platform pass the active filters?
  function _platformPasses(p) {
    const dbP = _pByName[p.name.toLowerCase().trim()];
    if(!dbP) return !curationFn; // unknown to DB: show unless curation filter active
    if(curationFn && !curationFn(dbP)) return false;
    if(q && !dbP.name.toLowerCase().includes(q) && !(isEn&&dbP.descEn?dbP.descEn:dbP.desc||'').toLowerCase().includes(q)) return false;
    return true;
  }
  ...
  const html = CAT_EXPLORER_DEFS.map(cat => {
    ...
    // Filter platforms for this category
    const filteredPlatforms = cat.platforms.filter(_platformPasses);
    const count = filteredPlatforms.length;
    const totalCount = cat.platforms.length;

    // Hide card entirely if filter active and no platforms match
    if((curationFn || q) && count === 0) return '';

    const badgeLabel = (curationFn || q) && count < totalCount
      ? labelPlatforms(count) + ` / ${totalCount}`
      : labelPlatforms(count);
    ...
    const platformsHtml = filteredPlatforms.map((p, i) => {
      // Validate URL scheme before rendering — only allow https:// and http://
      ...
      <a class="cat-platform-item" href="${escHtml(_safeUrl)}" target="_blank" rel="noopener noreferrer"
        ...
        <div class="cat-platform-item-body">
          ...
          <div class="cat-platform-item-desc">${escHtml(p.desc)}</div>
        </div>
    ...
    const expandHtml = `
      <div class="cat-expand-row${isOpen?' open':''}" data-expandid="${cat.id}" style="--cat-color:${cat.color}">
        ...
        <div class="cat-platform-list">${platformsHtml || `<div style="padding:16px;text-align:center;color:var(--grey);font-size:13px">${isEn?'No results for this filter.':'Sem resultados para este filtro.'}</div>`}</div>
      </div>`;
  ...
  // Bind clear button in empty state
...
function _toggleCatCard(id) {
  ...
  if(_hint) _hint.style.opacity = _catOpenId ? '0' : '';
...
(function _initKeyboardShortcuts() {
  // Note: listener is always registered; individual shortcuts check hasAccess at call time

  let _ggTimer = null;
  let _kbdPanelOpen = false;

  function _openKbdPanel() {
    const p = document.getElementById('kbdPanel');
    if(!p) return;
    _kbdPanelOpen = true;
    p.classList.add('open');
    p.addEventListener('click', function _kClose(e) {
      if(e.target === p) { _closeKbdPanel(); p.removeEventListener('click', _kClose); }
    }, {once:true});
  }
  function _closeKbdPanel() {
    _kbdPanelOpen = false;
    const p = document.getElementById('kbdPanel');
    if(p) p.classList.remove('open');
  }

  // Expose for lang updates
  window._closeKbdPanel = _closeKbdPanel;

  document.addEventListener('keydown', function(e) {
    // Only active post-auth
    if(!hasAccess) return;
    // Never fire when typing in an input/textarea
    const tag = (document.activeElement || {}).tagName;
    const inInput = tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT';

    // ? — keyboard panel (works even in input only if panel is open)
    if(e.key === '?' && !inInput) { e.preventDefault(); _kbdPanelOpen ? _closeKbdPanel() : _openKbdPanel(); return; }
    if(_kbdPanelOpen && e.key === 'Escape') { _closeKbdPanel(); return; }
    if(_kbdPanelOpen) return; // absorb all keys when panel open
    ...
    if(e.key === '/' || e.key === 'k' && (e.metaKey || e.ctrlKey)) {
      e.preventDefault();
      const cs = document.getElementById('catSearch');
      if(cs) { cs.focus(); cs.select(); _showToast('🔍 Pesquisar…', 1400); }
      return;
    }
    ...
    if(e.key === 'g' || e.key === 'G') {
      ...
      if(_ggTimer) {
        ...
        _showToast('⬆️ Início', 1200);
      } else {
        _ggTimer = setTimeout(() => { _ggTimer = null; }, 700);
      }
      return;
    }

    // Escape → clear search if active, else clear all filters
    if(e.key === 'Escape') {
      const cs = document.getElementById('catSearch');
      const s = document.getElementById('search');
...
(function _hookCatExplorer() {
  const _origRender = render;
  // Override render to also re-render cat explorer when platforms are loaded
  window.render = function() {
    _origRender.apply(this, arguments);
    // Re-render cat explorer only if access granted and grid exists
    if(hasAccess && document.getElementById('catGrid')) {
      const prevOpen = _catOpenId;
      renderCatExplorer();
      // Restore open state if any (renderCatExplorer uses _catOpenId)
    }
  };
})();
