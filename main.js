...
// ══ TABS & RENDER ══
// '_NONE' = no category chosen yet (shows "pick a category" prompt)
// ''      = "Todas" (all platforms explicitly chosen)
let activeTab = '_NONE';
let activeWorkType = '';
...
function setTab(v){
  ...
  // Scroll to main content when a tab is picked
  if(v !== '_NONE') {
    const mainEl = document.getElementById('main-content');
    if(mainEl) setTimeout(() => mainEl.scrollIntoView({ behavior:'smooth', block:'start' }), 80);
  }
  render();
}
...
function render(){
  ...
  // If no category chosen and no search/curation active, show the "pick a category" prompt
  ...
  if(_isNoneState) {
    ...
    if(grid) grid.innerHTML=`<div class="cat-empty-state">
      ...
      <div class="cat-empty-sub">${isEn
        ? 'Tap any category card above to see the platforms in that group — or use the tabs to filter by type.'
        : 'Clica numa categoria acima para ver as plataformas desse grupo — ou usa os filtros para pesquisar por tipo.'
      }</div>
    </div>`;
    ...
    if(_sCatsEl) _sCatsEl.textContent = '12';
    document.querySelectorAll('.tab').forEach(t=>{
  ...
  // Treat '_NONE' like '' (all) when a search/curation kicks in
...
const translations = {
  pt: {
    ...
    barTitle: 'Escolhe uma categoria acima ↑',
    // Lock screen
  ...
  en: {
    ...
    barTitle: 'Choose a category above ↑',
    // Lock screen
...
function renderCatExplorer() {
  ...
  // Build HTML: cat cards interleaved with expand rows
  // We render one expand row per category, placed after its card's grid row.
  // CSS grid-column:1/-1 makes expand rows span full width.

  const html = CAT_EXPLORER_DEFS.map(cat => {
    const title = isEn ? cat.titleEn : cat.titlePt;
    const desc  = isEn ? cat.descEn  : cat.descPt;
    const count = cat.platforms.length;
      ...
      <div class="cat-card${isOpen?' open':''}" data-catid="${cat.id}"
        ...
        <div class="cat-card-header">
          ...
          <div class="cat-card-body">
            ...
            <span class="cat-card-count">${labelPlatforms(count)}</span>
          </div>
  ...
  // "See all in list" buttons → scroll to main content (tab already set by card toggle)
...
function _toggleCatCard(id) {
  ...
  // Update all cards and expand rows
  ...
  document.querySelectorAll('.cat-expand-row').forEach(row => {
    ...
    if(open) {
      // Scroll the card into view with a small delay for animation
      setTimeout(() => {
  ...
  // Also set the tab filter to match the opened category
  // Map cat explorer IDs to tab filter keys
  const _idToTab = {
    deliveries: 'deliveries', pickup: '', surveys: 'surveys',
  ...
  if(_catOpenId !== null) {
    ...
    // Set activeTab and update tab UI without scrolling (card expand is the primary action)
    activeTab = tabV;
    activeCuration = '';
    document.querySelectorAll('.curation-pill').forEach(el=>el.classList.toggle('active',el.dataset.curation===''));
    document.querySelectorAll('.tab').forEach(t => {
    ...
    render(); // re-render grid with new filter
  } else {
    // Card closed — revert to _NONE (no category shown)
    activeTab = '_NONE';
    document.querySelectorAll('.tab').forEach(t => {
      t.classList.remove('active');
      t.setAttribute('aria-selected', 'false');
