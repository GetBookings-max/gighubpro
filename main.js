...
(function _initPWA() {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/gighubpro/sw.js').catch(function () {});
    });
  }
  ...
  try {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        lines.push('manifest: ' + (mRes.ok ? '✅' : '❌ ' + mRes.status));
        lines.push('start_url: ' + (mJson.start_url || '?'));
        lines.push('scope: ' + (mJson.scope || '?'));
        var icon192 = (mJson.icons||[]).find(function(i){ return i.sizes==='192x192' && i.purpose==='any'; });
        if(icon192) {
          var iRes = await fetch(icon192.src, {method:'HEAD'});
          lines.push('icon-192: ' + (iRes.ok ? '✅' : '❌ 404 — ' + icon192.src));
        }
        var icon512 = (mJson.icons||[]).find(function(i){ return i.sizes==='512x512' && i.purpose==='any'; });
        if(icon512) {
          var i2Res = await fetch(icon512.src, {method:'HEAD'});
          lines.push('icon-512: ' + (i2Res.ok ? '✅' : '❌ 404 — ' + icon512.src));
        }
      } catch(e) { lines.push('manifest: ❌ ' + e.message); }
