...
function toggleFav(name,e){
  ...
  try { localStorage.setItem('gh_favs',JSON.stringify(favs)); } catch(err) { console.warn('[GigHub] localStorage write failed (favs):', err); }
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  },
  {
    ...
    cat: 'gigs',
    icon: '🛴',
    desc: 'Torna-te parceiro logístico da Lime em Lisboa: recolhes trotinetes elétricas à noite, carregas em casa e repões de manhã. Pagas por trotinete — 3€ a 5€ cada, máximo 12/dia. Precisas de carro ou mota. Sem horário fixo.',
    descEn: 'Become a Lime logistics partner in Lisbon: collect e-scooters at night, charge them at home and reposition them in the morning. Paid per scooter — €3 to €5 each, max 12/day. Car or motorbike required. No fixed schedule.',
    earn: '3€–5€ por trotinete',
    earnN: 7,
    minPay: 'Por trotinete',
    pt: true, eu: false,
    url: 'https://www.li.me/pt-pt/sobre/parceiros/logistic-partners',
    easy: 4, geo: '🇵🇹 Lisboa',
    top: false, dimmed: false, beginner: true,
    ratings: {}, aff: { has: false },
    entry_level: 'facil', earnings_type: 'variavel', tier: 2,
  ...
  // Adicionados — segurança, casting, mobilidade
  'trablisa esegur',
  'becasting',
  'lime',
  // Adicionados — ambiente e resíduos
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    try {
      const _storedFavs = JSON.parse(localStorage.getItem('gh_favs') || '[]');
