...
function setWorkType(val) {
  ...
  ['fboxAll','fboxStable','fboxVariable'].forEach(id => {
    const el = document.getElementById(id);
    if(el) el.classList.toggle('active', el.dataset.fbox === val);
  });
  render();
}
...
const translations = {
  ...
  en: {
    ...
    guideTip: '<strong>★ Top 3 for Portugal in 2026 —</strong> <strong>AttaPoll</strong> (survey app, simple, pays via PayPal, available in PT) · <strong>Toloka AI</strong> (AI micro-tasks, good PT availability, easy entry) · <strong>Amnistia Internacional</strong> (recognised volunteering, great for CV, no requirements)',
...
function applyLang(){
  ...
  if(fboxAll) fboxAll.textContent = isEn ? '💼 All' : '💼 Tudo';
  if(fboxStable) fboxStable.textContent = isEn ? '🖥️ Remote' : '🖥️ Remoto';
  if(fboxVariable) fboxVariable.textContent = isEn ? '🤝 In-Person' : '🤝 Presencial';
...
(function _bindEvents(){
  ...
  // ── Filter boxes (remoto/presencial/all) ──
  document.querySelectorAll('.fbox[data-fbox]').forEach(el => {
    el.addEventListener('click', () => setWorkType(el.dataset.fbox));
    el.addEventListener('keydown', e => { if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setWorkType(el.dataset.fbox); } });
  });
