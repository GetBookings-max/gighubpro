...
// ── DevTools hardening: block direct `hasAccess = true` override ─────────────
// The nonce check in render() already blocks the trivial bypass, but this adds
// a second layer: any write to hasAccess from DevTools console is silently ignored
// unless it comes from validarTokenSupabase (which sets _sessionNonce atomically).
(function _hardendHasAccess(){
  let _ha = false;
  Object.defineProperty(window, 'hasAccess', {
    get(){ return _ha; },
    set(v){
      // Only allow setting true if _sessionNonce is already populated
      // (validarTokenSupabase sets both atomically — DevTools cannot replicate this)
      if(v === true && !_sessionNonce) return; // silently block
      _ha = v;
    },
...
// ── Pre-computed lowercase set for case-insensitive block matching ────────────
// Covers DB name variants that differ in casing, spacing, or encoding from
// what's in _BLOCKED_PLATFORMS (e.g. "Qualidade21" vs "Qualidade 21").
const _BLOCKED_LOWER = new Set([..._BLOCKED_PLATFORMS].map(n => n.toLowerCase().trim()));

function _fmt(r){
  // Hard-block: exact match OR case-insensitive/trimmed match
  if(_BLOCKED_PLATFORMS.has(r.name) || _BLOCKED_LOWER.has(r.name.toLowerCase().trim())) return null;
