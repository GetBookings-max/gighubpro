...
(function _initPWA() {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/gighubpro/sw.js').then(function(reg) {
        window._swScope = reg.scope;
        window._swOk = true;
      }).catch(function(err) {
        window._swOk = false;
        window._swErr = err.message;
      });
    });
  }
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    setTimeout(() => {
      ...
      // PWA diagnostic overlay — remove after debugging
      var d = document.createElement('div');
      d.id = 'pwaDiag';
      d.style.cssText = 'position:fixed;bottom:70px;left:12px;right:12px;z-index:9999;background:#111;color:#fff;font-size:12px;padding:10px 12px;border-radius:8px;border:1px solid #333;font-family:monospace;line-height:1.8';
      var sw = window._swOk === true ? '✅ SW ok — ' + (window._swScope||'?') : window._swOk === false ? '❌ SW FALHOU: ' + (window._swErr||'?') : '⏳ SW pendente';
      var bip = window._pwaPromptCaptured ? '✅ beforeinstallprompt capturado' : '❌ beforeinstallprompt NÃO disparou';
      d.innerHTML = sw + '<br>' + bip + '<br><small style="color:#888">toca para fechar</small>';
