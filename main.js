...
function toggleFavView(){
  ...
  if(favBtn){
    favBtn.innerHTML=t.navFavs+' (<span id="favCount">'+favs.length+'</span>)';
...
function openBoostPay(){
  ...
  if(modal) {
    ...
    // Update badge price display from constant
    const badge = modal.querySelector('.boost-badge');
    const priceStr = PRICE_PER_BOOST.toFixed(2).replace('.',',') + '€';
    if(badge) badge.textContent = '✨ GigBoost · ' + priceStr;
    // Update displayed price in payment box
...
//
// PILAR 2 — ESTABILIDADE DE GANHOS (integrado no tier badge como tooltip/chip):
...
const _MODERADO_PLATFORMS = new Set([
  ...
  // 🎬 Cinemas & entretenimento presencial
  'NOS Cinemas Recrutamento',
]);
...
// ══ TIER SYSTEM — Fiabilidade de rendimento ══════════════════════════════════
//
// TIER 1 🟢 Consistente      — trabalho estável, pode pagar contas
// TIER 2 🟡 Variável         — depende de skill / disponibilidade
// TIER 3 🔴 Complementar     — baixa consistência, tempos mortos
// TIER 4 🟣 Descoberta       — ferramentas para encontrar trabalho
//
// Nota: a etiqueta de tier aparece no card-cats (junto com a categoria)
// como chip compacto. O tooltip contém a descrição completa.
// Lógica: nome explícito > categoria default
// ─────────────────────────────────────────────────────────────────────────────

// Plataformas explicitamente Tier 1 (fora das categorias default)
const _TIER1_NAMES = new Set([
  // Tutoria estabelecida
  'Preply','Superprof','Explicas.me',
  // Biscates / serviços com procura real
  'TaskRabbit',
  // Fundraising (salário fixo + comissão)
  'ACNUR','Amnistia Internacional','Aldeias SOS','WWF Portugal','APDES','Associação Salvador','Associacao Salvador',
  // Voluntariado remunerado / housing
  'Worldpackers',
  // Novas plataformas presenciais
  'Ritmos Fortes','Infraestruturas de Portugal','Transdev Portugal',
  'Solverde Recrutamento','SalvaMais','Grupo Nabeiro',
]);

// Plataformas explicitamente Tier 2 (fora das categorias default)
const _TIER2_NAMES = new Set([
  // Cliente mistério — procura irregular mas remuneração real
  'BeMyEye','More Results','Pontis','SmartSpotter','Roamler',
  // Pet/babysitting — variável por procura local
  'BabySits','PetBacker','Rover',
  // Aluguer / passive com gestão activa
  'Yescapa','Sonder','Sonder People','Radical Storage','Bounce',
  // Bookycar
  'BooyCar','Bookycar',
]);

// Plataformas explicitamente Tier 3 (fora das categorias default)
const _TIER3_NAMES = new Set([
  'AttaPoll','Boutique Opiniões','CLSBE PEO (Católica)','Freecash',
  'HeyPiggy','LifePoints','Mobrog','MORE Results',
  'Netsonda','Opiniões de Valor','Prime Opinion','Qmee',
  'SproutGigs','Storewards','TGM Panel',
  'Toloka AI','Triaba','YouGov',
  'PlaytestCloud','We Are Testers',
  'Beruby',
]);

// Default por categoria quando o nome não está em nenhum set explícito
const _TIER_CAT_DEFAULT = {
  f2f:1, gigs:1, retail:1, clinical:1,
...
function _getTier(p){
  if(_TIER1_NAMES.has(p.name)) return 1;
  if(_TIER2_NAMES.has(p.name)) return 2;
  if(_TIER3_NAMES.has(p.name)) return 3;
  return _TIER_CAT_DEFAULT[p.cat] || 2;
}

function _tierBadge(p){
  const tier = _getTier(p);
  const isEn = currentLang === 'en';
  const map = isEn
    ? {1:'🟢 Consistent', 2:'🟡 Variable', 3:'🔴 Supplementary', 4:'🟣 Discovery'}
    : {1:'🟢 Consistente', 2:'🟡 Variável', 3:'🔴 Complementar', 4:'🟣 Descoberta'};
  return `<span class="tier-badge tier-${tier}" title="${isEn ? ['','Stable income — can pay bills','Variable but real — depends on skill','Low consistency / supplementary','Tool to find work'][tier] : ['','Rendimento estável — pode pagar contas','Variável mas real — depende de skill','Baixa consistência / complementar','Ferramenta para encontrar trabalho'][tier]}">${map[tier]}</span>`;
}

// ── Direct-hire employers — show specialist tags regardless of tab ────────────
const _DIRECT_HIRING_NAMES = new Set([
  'DHL','RGIS','Securitas','Prosegur','Salesland',
  'Portway','Mercadona','Prezero','Jardim Zoológico','Jardim Zoologico','Eurest',
  'NOS Cinemas Recrutamento',
]);
...
function renderRatings(p){
  const r = p.ratings || {};
  const isEn = currentLang === 'en';
  const isDirectHire = _DIRECT_HIRING_NAMES.has(p.name);
  ...
  // ── PILAR 2: Destaque (opcional) ─────────────────────────────────────────────
  // Nota: estabilidade de ganhos está integrada no tier badge (card-cats) — sem redundância.
  const highlightTag = _SUGGESTED_SET.has(p.name)
    ? `<span class="rtag rtag-suggested">⭐ ${isEn?'Recommended':'Recomendado'}</span>`
    : '';

  return `<div class="ratings-row">${entryTag}${highlightTag}</div>`;
}
...
const _F2F_CATS = new Set(['f2f', 'gigs', 'petsitting', 'babysitting', 'retail', 'clinical']);
...
const TAB_GROUP_LABELS = {
  pt: {
    ...
    'f2f':        '🤝 Trabalho Presencial e Voluntariado',
  ...
  en: {
    ...
    'caregiving': '🫶 Caregiving',
    'f2f':        '🤝 In-Person Work & Volunteering',
...
function render(){
  ...
  const _sTotalEl=document.getElementById('s-total'); if(_sTotalEl) _sTotalEl.textContent=list.length;
  ...
  const _curationTitles={
    pt:{portugal:'Top picks Portugal 🇵🇹',bestpay:'Melhores pagamentos 💰',fastest:'Ganhos estáveis ⚡',noexp:'Sem experiência necessária 🚀'},
    en:{portugal:'Top picks Portugal 🇵🇹',bestpay:'Best payouts 💰',fastest:'Stable earnings ⚡',noexp:'No experience needed 🚀'}
  };
...
const _NO_TAG_NAMES = new Set([
  'Eloquence Events','Casamentos.pt','Óscar','Oscar',
  'Worldpackers','Guia Tuk Tuk',
]);
const _NO_TAG_CATS = new Set([
  'conteudo','criativo','ugc','freelance',
  'tutoring','transcricao','petsitting','babysitting',
]);

const curationFilters = {
  // Only hand-picked platforms (in _SUGGESTED_SET) that are available in PT
  portugal: p => _SUGGESTED_SET.has(p.name) && p.pt === true,

  // Best payouts: f2f/fundraising and clinical always qualify; passive never;
  // everything else needs earnN >= 15
  bestpay: p => {
    if(p.cat === 'passive') return false;
    if(_NO_TAG_NAMES.has(p.name) || _NO_TAG_CATS.has(p.cat)) return false;
    if(p.cat === 'clinical' || p.cat === 'f2f') return true;
    return p.earnN >= 15;
  },

  // Stable earnings: fixed per-delivery/per-hour pay once active.
  // Surveys, tasks, micro, testing, mystery are excluded (variable/sporadic like surveys).
  // Clinical excluded (per-project, not ongoing). Passive excluded.
  fastest: p => {
    if(_NO_TAG_NAMES.has(p.name) || _NO_TAG_CATS.has(p.cat)) return false;
    if(['surveys','tasks','micro','testing','mystery','passive','clinical'].includes(p.cat)) return false;
    if(['TaskRabbit','Merytu','Grabr'].includes(p.name)) return false;
    return _ESTAVEL_PLATFORMS.has(p.name) || _CAT_EARNINGS_FALLBACK[p.cat] === 'estavel';
  },

  // No experience needed: merges old "beginners" and "noexp" into one tag.
  // Overrides: clinical, passive, and retail always qualify.
  // Uber/Bolt drivers and Grabr qualify despite low easy score.
  noexp: p => {
    if(_NO_TAG_NAMES.has(p.name) || _NO_TAG_CATS.has(p.cat)) return false;
...
const translations = {
  pt: {
    ...
    lockPayText: 'Para obter acesso, transfere 24,99€ por MB Way ou transferência bancária e envia o comprovativo para <a href="mailto:gighubpt@gmail.com" style="color:var(--gold);text-decoration:none">gighubpt@gmail.com</a>',
    lockVerifying: 'A verificar acesso…',
    lockEnter: 'Entrar →',
    lockStep1Label: 'Como obter acesso',
    lockStep1Desc: 'Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">29,99€</span><strong>24,99€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.',
    ...
    guideCommunityTip: '<strong style="color:#0e64b4">💬 Canais informais —</strong> Muitos gigs remotos não aparecem em nenhuma plataforma. Junta-te a grupos Facebook de teletrabalho e remote work PT, grupos WhatsApp de freelancers e acompanha o Sapo Emprego e IT Jobs regularmente. <a href="#canais" style="color:#0e64b4;font-weight:600;text-decoration:none">Ver todos os canais →</a>',
    canaisBadge: '💬 Canais de Descoberta',
    tierLegendLabel: 'Fiabilidade:',
    tierLeg1: '🟢 Consistente — pode pagar contas',
    tierLeg2: '🟡 Variável — depende de skill e disponibilidade',
    tierLeg3: '🔴 Complementar — baixa consistência',
    tierLeg4: '🟣 Descoberta — ferramenta para encontrar trabalho',
  ...
  // GigBoost
    ...
    boostPayInstr: 'Transfere <strong>4,99€</strong> via <strong>MB Way</strong> ou IBAN e envia o comprovativo para <strong>gighubpt@gmail.com</strong>.<br>O acesso GigBoost é enviado após confirmação.',
  ...
  en: {
    ...
    lockPayText: 'To get access, transfer €24.99 via MB Way or bank transfer and email your receipt to <a href="mailto:gighubpt@gmail.com" style="color:var(--gold);text-decoration:none">gighubpt@gmail.com</a>',
    ...
    guideTip: '<strong>★ Top 3 for Portugal in 2026 —</strong> <strong>AttaPoll</strong> (survey app, simple, pays via PayPal, available in PT) · <strong>Toloka</strong> (AI micro-tasks, good PT availability, easy entry) · <strong>Netsonda</strong> (market research in PT, easy to start, no requirements)',
    guideCommunityTip: '<strong style="color:#0e64b4">💬 Informal channels —</strong> Many gigs don\'t appear on any platform. Join Facebook groups in your city, Portuguese freelancer WhatsApp groups and regularly check Sapo Emprego and OLX. <a href="#canais" style="color:#0e64b4;font-weight:600;text-decoration:none">See all channels →</a>',
    canaisBadge: '💬 Discovery Channels',
    tierLegendLabel: 'Reliability:',
    tierLeg1: '🟢 Consistent — can pay bills',
    tierLeg2: '🟡 Variable — depends on skill & availability',
    tierLeg3: '🔴 Supplementary — low consistency',
    tierLeg4: '🟣 Discovery — tool to find work',
    ...
    monoDesc: 'Transfer €24.99 via MB Way or IBAN and email your receipt to <strong>gighubpt@gmail.com</strong>. You\'ll receive the access key in minutes. Receipts may contain personal data — see <a href="#" data-modal="privacy" style="color:rgba(247,245,240,.6)">Privacy Policy</a>. Never share your access key.',
    ...
    lockStep1Desc: 'Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€29.99</span><strong>€24.99 via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.',
  ...
  // GigBoost
    ...
    boostPayInstr: 'Transfer <strong>€4.99</strong> via <strong>MB Way</strong> or IBAN and email your receipt to <strong>gighubpt@gmail.com</strong>.<br>Access is sent after payment confirmation.',
...
function applyLockLang(){
  ...
  if(desc1) desc1.innerHTML = isEn
    ? 'Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€29.99</span><strong>€24.99 via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.'
    : 'Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">29,99€</span><strong>24,99€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.';
...
function applyLang(){
  ...
  set2('guideCommunityTip', 'guideCommunityTip', true);
  ...
  setText2('canaisCatClassificados', '📋 Classificados & Emprego', '📋 Classifieds & Jobs');
  setText2('canaisCatFacebook',      '👥 Grupos Facebook',          '👥 Facebook Groups');
  setText2('canaisCatWhatsapp',      '💬 WhatsApp & Outros',        '💬 WhatsApp & Other');
  ...
  setInner2('canaisFbLisboaDesc',  'Grupos dedicados a trabalho remoto em PT — oportunidades, dicas e networking.',
                                   'Groups dedicated to remote work in Portugal — opportunities, tips and networking.');
  setInner2('canaisFbPortoDesc',   'Comunidade em inglês e português para quem trabalha remotamente ou procura oportunidades remote.',
                                   'English and Portuguese community for those working remotely or looking for remote opportunities.');
  setInner2('canaisFbFreelanceDesc','Comunidade nacional de freelancers — projetos, dicas, networking e oportunidades.',
                                   'National freelancer community — projects, tips, networking and opportunities.');
  setInner2('canaisFbPartTimeDesc','Grupos de trabalho online e remoto — útil para encontrar projetos e clientes em Portugal.',
                                   'Online and remote work groups — useful for finding projects and clients in Portugal.');
  ...
  setInner2('canaisItJobsDesc',    'Melhor portal para trabalho tech e remote em Portugal — dev, design, marketing digital.',
                                   'Best portal for tech and remote work in Portugal — dev, design, digital marketing.');
  // Tier legend
  set2('tierLegendLabel', 'tierLegendLabel', false);
  set2('tierLeg1', 'tierLeg1', false);
  set2('tierLeg2', 'tierLeg2', false);
  set2('tierLeg3', 'tierLeg3', false);
  set2('tierLeg4', 'tierLeg4', false);
  // Calculator
...
// ── Local extra platforms (injected client-side, not from Supabase) ───────────
// Used for platforms that are not yet in the DB but should appear in the listing.
const _LOCAL_EXTRA_PLATFORMS = [
  {
    name: 'NOS Cinemas Recrutamento',
    cat: 'f2f',
    icon: '🎬',
    desc: 'Trabalho part-time nos cinemas NOS em Portugal. Funções de atendimento ao público, bilheteira e operações. Horários flexíveis, ideal para estudantes. Candidatura directa pelo site.',
    descEn: 'Part-time work at NOS Cinemas in Portugal. Customer service, box office and operations roles. Flexible hours, great for students. Apply directly on the website.',
    earn: 'Salário conforme acordo',
    earnN: 9,
    minPay: 'Salário mensal',
    pt: true, eu: false,
    url: 'https://www.cinemas.nos.pt/contactos/Recrutamento',
    easy: 3, geo: '🇵🇹 Portugal',
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    if(data.platforms && data.platforms.length > 0) {
      ...
      // Inject local extra platforms (not from Supabase)
      _LOCAL_EXTRA_PLATFORMS.forEach(lp => {
        const k = lp.name.toLowerCase().trim().replace(/\s+/g,' ').replace(/\s*\([^)]*\)\s*$/, '');
        if(!_seen.has(k)) { _seen.add(k); P.push(lp); }
      });
    ...
    // Show Canais nav link now that user is authenticated
    const _navCanais = document.getElementById('navCanaisLink');
    if(_navCanais) _navCanais.style.display = '';
    if(lockErr) lockErr.textContent = '';
