...
function calcEarnings(){
  ...
  const sPT={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats ou Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.pt.',10:'99designs + Outlier (freelance e treino de IA para perfis qualificados — entrada seletiva, ganhos variáveis por projeto).',18:'TestingTime (plataforma europeia de estudos de utilizador com projetos PT disponíveis — pagamento variável por sessão, seletivo).'};
  const sEN={2:'Prolific + Mobrog + AttaPoll.',6:'Uber Eats or Bolt Food.',7:'DataAnnotation.tech + Clickworker.',8:'Rover + Zoowish + Babysits.',10:'99designs + Outlier (freelance and AI training for qualified profiles — selective entry, variable earnings per project).',18:'TestingTime (European user research platform with PT projects available — variable pay per session, selective).'};
...
const TAB_CAT_FILTERS = {
  ...
  'creative':   p => ['criativo', 'conteudo', 'ugc'].includes(p.cat),
  'tech':       p => ['micro', 'testing'].includes(p.cat),
  ...
  'gigs_events':p => (p.cat === 'gigs' && !_DELIVERY_NAMES.has(p.name)) || p.cat === 'f2f',
  'other':      p => ['tasks', 'passive'].includes(p.cat),
};
...
const TAB_GROUP_LABELS = {
  pt: {
    ...
    'skills': 'Freelance & Tutoring',
    'gigs_events': 'Events & Field Work',
    'other': 'Other',
  },
  en: {
    '': 'All platforms',
