...
// ══ TABS & RENDER ══
// '_NONE' = nenhuma categoria seleccionada ainda (grid vazio, mostra prompt)
// ''      = "Ver tudo" (explícito)
let activeTab = '_NONE';
let activeWorkType = '';
...
// No remote exceptions needed currently
const _REMOTE_EXCEPTION_NAMES = new Set([]);
// Parcel pickup points and vehicle rental platforms also require physical presence
const _NOT_REMOTE_NAMES = new Set([
  'radical storage','bounce',
  'dhl servicepoints','ups access point','dpd pickup',
]);
...
const TAB_CAT_FILTERS = {
  ...
  // pickup = plataformas de ponto de recolha (passive cat com is_delivery=false, ou nome específico)
  'pickup':     p => p.cat === 'passive' && !p.is_delivery &&
                     ['DHL ServicePoints','UPS Access Point','DPD Pickup',
                      'Vinted Go Parceiro','Payshop Agente','Ponto CTT',
                      'Radical Storage','Bounce'].includes(p.name),
};
...
const TAB_GROUP_LABELS = {
  pt: {
    ...
    'pickup':     '📦 Pontos de Recolha e Espaço Próprio',
  },
  en: {
    '':           'All platforms',
    ...
    'pickup':     '📦 Parcel Points & Use Your Space',
  },
};
function setTab(v){
  activeTab=v;
...
function render(){
  ...
  // ── _NONE state: no category picked yet, no search/curation active ──
  const _isNoneState = cat === '_NONE' && !curationFn && !showFavsOnly && !q;
  if(_isNoneState) {
    const grid=document.getElementById('grid');
    if(grid) grid.innerHTML=`<div class="cat-empty-state">
      <div class="cat-empty-ico">👆</div>
      <div class="cat-empty-title">${isEn?'Pick a category to explore':'Escolhe uma categoria para explorar'}</div>
      <div class="cat-empty-sub">${isEn
        ?'Tap a category card above to see platforms in that group.'
        :'Clica num card de categoria acima para ver as plataformas desse grupo.'}</div>
    </div>`;
    const _barTitleEl=document.getElementById('barTitle');
    if(_barTitleEl) _barTitleEl.textContent=isEn?'Choose a category above ↑':'Escolhe uma categoria acima ↑';
    const _barCountEl=document.getElementById('barCount');
    if(_barCountEl) _barCountEl.textContent='';
    const _catNoteEl=document.getElementById('catNote');
    if(_catNoteEl) _catNoteEl.style.display='none';
    const _sTotalEl=document.getElementById('s-total');
    if(_sTotalEl) _sTotalEl.textContent=P.length||'—';
    const _sCatsEl=document.getElementById('s-cats');
    if(_sCatsEl) _sCatsEl.textContent='10';
    document.querySelectorAll('.tab').forEach(t=>{
  ...
  // When _NONE + search/curation kicks in, treat as "all"
  const effectiveCat = (cat === '_NONE') ? '' : cat;
  ...
  let list=P.filter(p=>{
    ...
    if(effectiveCat) { const _tf = TAB_CAT_FILTERS[effectiveCat]; if(!_tf || !_tf(p)) return false; }
  ...
  const _sTotalEl=document.getElementById('s-total');
  const _sCatsEl=document.getElementById('s-cats');
  const _isFiltered = effectiveCat !== '' || activeCuration !== '' || activeWorkType !== '' || q !== '' || showFavsOnly;
  ...
  const _barCountEl=document.getElementById('barCount'); if(_barCountEl) _barCountEl.textContent=list.length+(isEn?' result'+(list.length!==1?'s':''):(` resultado${list.length!==1?'s':''}`));
  ...
  document.querySelectorAll('.tab[data-v]').forEach(tab=>{
    ...
    if(cnt===0){
      ...
      if(activeTab===v){ activeTab='_NONE'; document.querySelectorAll('.tab').forEach(t=>{ t.classList.remove('active'); t.setAttribute('aria-selected','false'); t.setAttribute('tabindex','-1'); }); }
    } else {
      tab.style.display='';
    }
  });
  ...
  const _tabGroupLabel = (TAB_GROUP_LABELS[currentLang]||TAB_GROUP_LABELS.pt)[effectiveCat];
  ...
  const _catNoteEl = document.getElementById('catNote');
  if(_catNoteEl) {
    const _noteData = _CAT_NOTES[effectiveCat];
...
function setCuration(key) {
  activeCuration = key;
  // When curation active, show all platforms (tab='') not _NONE
  activeTab = '';
  document.querySelectorAll('.tab').forEach(t => {
...
const translations = {
  pt: {
    ...
    lockStep1Desc: 'Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">12,99€</span><strong>8,99€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.',
  ...
  en: {
    ...
    lockStep1Desc: 'Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€12.99</span><strong>€8.99 via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.',
...
function applyLockLang(){
  ...
  if(desc1) desc1.innerHTML = isEn
    ? `Transfer <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">€12.99</span><strong>€${PRICE_PER_ACCESS.toFixed(2)} via MB Way or IBAN</strong></span> and email your receipt to <strong>gighubpt@gmail.com</strong>. You will receive your access key in minutes.`
    : `Transfere <span style="white-space:nowrap"><span style="font-size:11px;opacity:.4;text-decoration:line-through;margin-right:3px">12,99€</span><strong>${PRICE_PER_ACCESS.toFixed(2).replace('.',',')}€ via MB Way ou IBAN</strong></span> e envia o comprovativo para <strong>gighubpt@gmail.com</strong>. Recebes a tua chave em minutos.`;
...
function applyLang(){
  ...
  const _TAB_SHORT = {
    pt:{'':'⊞ Ver tudo','surveys':'📝 Inquéritos & Microtasks',
      ...
      'pickup':'📦 Pontos de Recolha'},
    en:{'':'⊞ See all','surveys':'📝 Surveys & Microtasks',
...
function _sanitize(s){ 
  if(!s) return ''; 
  // Strip null bytes and HTML tags first, then dangerous characters
  ...
  // Note: tag-stripping above already removes <script> etc.; these catch residual patterns
...
function _startSessionTimeout(){
  ...
  // Warn at 110 min (10 min before 2h timeout)
...
(function _bindEvents(){
  ...
  if(grid) grid.addEventListener('click', function(e) {
    ...
    if(clearBtn) {
      ...
      if(_fa3) { _fa3.textContent = currentLang === 'en' ? '💼 All formats' : '💼 Qualquer formato'; _fa3.setAttribute('aria-expanded','false'); }
  ...
  if(boostFormBox){
    boostFormBox.addEventListener('change', function(e) {
      ...
      if(ta) { boostAnswers[ta.dataset.key] = ta.value.trim(); boostCheckNext(); }
    });
    boostFormBox.addEventListener('input', function(e) {
      const ta = e.target.closest('textarea.boost-textarea');
      if(ta) { boostAnswers[ta.dataset.key] = ta.value.trim(); }
    });
  ...
  if(favExportBtn) favExportBtn.addEventListener('click', () => {
    ...
    _box.innerHTML = `
      ...
      <textarea readonly style="width:100%;height:130px;background:var(--cream);border:1px solid var(--border-md);border-radius:10px;color:var(--ink);font-family:'Instrument Sans',sans-serif;font-size:12px;padding:12px 14px;resize:none;outline:none;line-height:1.7">${nameList}</textarea>
...
function renderCatExplorer() {
  ...
  const isEn = currentLang === 'en';
  const labelSeeAll = isEn ? 'See all in full list ↓' : 'Ver todas na lista completa ↓';
  const labelPlatforms = (n) => isEn ? `${n} platform${n!==1?'s':''}` : `${n} plataforma${n!==1?'s':''}`;

  // Map explorer cat IDs → TAB_CAT_FILTERS key (same as _idToTab below)
  const _idToTab = {
    deliveries: 'deliveries', pickup: 'pickup', surveys: 'surveys',
    summer: 'gigs_events', commissions: 'passive', clinical: 'clinical',
    gigs_services: 'gigs_events', fundraising: 'retail',
    retail_stores: 'f2f', callcenter: 'support',
    transport: 'f2f', security: 'f2f', health: 'f2f',
    misc_local: '', construction: 'f2f'
  };

  // Compute DB count per tab key (from live P array)
  // Multiple explorer cats can map to same tab — count is for that tab
  const _dbCountForTab = {};
  Object.keys(TAB_CAT_FILTERS).forEach(tabKey => {
    if(tabKey === '') { _dbCountForTab[''] = P.length; return; }
    const fn = TAB_CAT_FILTERS[tabKey];
    _dbCountForTab[tabKey] = P.filter(fn).length;
  });
  ...
  const html = CAT_EXPLORER_DEFS.map(cat => {
    ...
    // Show DB count for this card's tab, not the hardcoded list length
    const tabKey = _idToTab[cat.id];
    const dbCount = (tabKey !== undefined && tabKey !== '') ? (_dbCountForTab[tabKey] || 0) : P.length;
    // For cats that share a tab (summer+gigs_services both → gigs_events),
    // show the explorer list count so the inline expansion matches what's visible
    const displayCount = cat.platforms.length;
      ...
      <div class="cat-card${isOpen?' open':''}" data-catid="${cat.id}"
        ...
        <div class="cat-card-header">
          ...
          <div class="cat-card-body">
            ...
            <span class="cat-card-count">${labelPlatforms(displayCount)}</span>
          </div>
          <div class="cat-card-chevron">▾</div>
        </div>
      </div>`;

    const platformsHtml = cat.platforms.map((p, i) => `
      <a class="cat-platform-item" href="${escHtml(p.url)}" target="_blank" rel="noopener noreferrer"
         style="animation-delay:${i * 0.03}s">
        <div class="cat-platform-item-ico">${p.icon}</div>
    ...
    const expandHtml = `
      <div class="cat-expand-row${isOpen?' open':''}" data-expandid="${cat.id}" style="--cat-color:${cat.color}">
        <div class="cat-expand-header">
          <span class="cat-expand-header-emoji">${cat.emoji}</span>
          <div>
            <div class="cat-expand-header-title">${escHtml(title)}</div>
            <div class="cat-expand-header-sub">${escHtml(desc)}</div>
          </div>
        </div>
        <div class="cat-platform-list">${platformsHtml}</div>
        <button class="cat-expand-see-all" data-tabv="${cat.id}">
          ${labelSeeAll}
        </button>
      </div>`;
  ...
  // "See all in full list" buttons — scroll to main grid (tab already set by card click)
  grid.querySelectorAll('.cat-expand-see-all').forEach(btn => {
...
function _toggleCatCard(id) {
  ...
  // Set tab filter to match the opened category
  const _idToTab = {
    deliveries: 'deliveries', pickup: 'pickup', surveys: 'surveys',
    summer: 'gigs_events', commissions: 'passive', clinical: 'clinical',
    gigs_services: 'gigs_events', fundraising: 'retail',
  ...
  if(_catOpenId !== null) {
    const tabV = _idToTab[_catOpenId] !== undefined ? _idToTab[_catOpenId] : '';
    activeTab = tabV;
    activeCuration = '';
    document.querySelectorAll('.curation-pill').forEach(el => el.classList.toggle('active', el.dataset.curation === ''));
    document.querySelectorAll('.tab').forEach(t => {
      const isActive = t.dataset.v === tabV;
      t.classList.toggle('active', isActive);
      t.setAttribute('aria-selected', isActive ? 'true' : 'false');
      t.setAttribute('tabindex', isActive ? '0' : '-1');
    });
