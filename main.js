...
// Registers sw.js and handles the Add to Home Screen (A2HS) prompt.
// The install banner is only shown AFTER successful auth (see _showPwaInstallBanner).
...
(function _initPWA() {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/sw.js')
        .catch(function () {}); // silent — SW is progressive enhancement only
    });
  }

  // ── Capture the beforeinstallprompt event ────────────────────────────────
  // Chrome/Edge/Android fire this before showing the browser's own A2HS UI.
  // We suppress the default mini-infobar and replace it with our own banner.
  let _deferredPrompt = null;

  window.addEventListener('beforeinstallprompt', function (e) {
    e.preventDefault();
    _deferredPrompt = e;
    // Show banner only if the user has already passed auth
    if (typeof hasAccess !== 'undefined' && hasAccess) {
      _showPwaInstallBanner();
    }
  });

  // ── Show the install banner (called after auth OR when prompt fires post-auth) ──
  window._showPwaInstallBanner = function () {
    // Don't show if: already dismissed, already installed, prompt unavailable
    try { if (localStorage.getItem('gh_pwa_dismissed')) return; } catch (_) {}
    if (!_deferredPrompt) return;
    const banner = document.getElementById('pwaInstallBanner');
    if (!banner) return;

    // Adjust text to current language
    const textEl = document.getElementById('pwaBannerText');
    const btnEl  = document.getElementById('pwaInstallBtn');
    if (typeof currentLang !== 'undefined' && currentLang === 'en') {
      if (textEl) textEl.innerHTML = 'Add <strong>GigHub</strong> to your home screen — quick access, no browser needed.';
      if (btnEl)  btnEl.textContent = 'Install';
    }

    banner.classList.remove('hidden');
    // Nudge the back-to-top button up so it doesn't overlap the banner
  ...
  document.addEventListener('click', function (e) {
    ...
    _deferredPrompt.userChoice.then(function (choice) {
      ...
      // If accepted, record so we never nag again
  ...
  // ── Dismiss button ───────────────────────────────────────────────────────
  document.addEventListener('click', function (e) {
    if (e.target.id !== 'pwaInstallDismiss') return;
    try { localStorage.setItem('gh_pwa_dismissed', '1'); } catch (_) {}
    _hidePwaInstallBanner();
  });
  ...
  function _hidePwaInstallBanner() {
    ...
    // Restore back-to-top button to its default position
    const btt = document.getElementById('backToTop');
    if (btt) btt.style.bottom = '80px';
  }

  // ── appinstalled: fired when the user adds the app from elsewhere ────────
