...
function submitBoostForm(){
  ...
  try {
    const _brlRaw = localStorage.getItem('_boost_rl');
    let _brl = { c: 0, t: 0 };
    try {
    ...
    _brl.c = Math.min((_brl.c || 0) + 1, 99); localStorage.setItem('_boost_rl', JSON.stringify(_brl));
  } catch(e) {}
...
(function _bindEvents(){
  ...
  window.addEventListener('scroll', function(){
    ...
    if(!btn) return;
    const gdpr = document.getElementById('gdprNotice');
    const gdprVisible = gdpr && !gdpr.classList.contains('hidden');
    btn.style.bottom = gdprVisible ? '72px' : '16px';
...
function renderCatExplorer() {
  ...
  const curationFn = activeCuration ? curationFilters[activeCuration] : null;
  const _workType = (document.getElementById('fWorkType')?.value) || activeWorkType;
  ...
  function _platformPasses(p) {
    ...
    if(!dbP) return !curationFn && !_workType; // unknown to DB: show unless filter active
    ...
    if(_workType === 'f2f' && !_F2F_CATS.has(dbP.cat)) return false;
    if(_workType === 'remote' && (_NOT_REMOTE_NAMES.has(dbP.name.toLowerCase()) || (_F2F_CATS.has(dbP.cat) && !_REMOTE_EXCEPTION_NAMES.has(dbP.name.toLowerCase())))) return false;
    return true;
  }
  ...
  const html = CAT_EXPLORER_DEFS.map(cat => {
    ...
    if((curationFn || q || _workType) && count === 0) return '';

    const badgeLabel = (curationFn || q || _workType) && count < totalCount
  ...
  grid.innerHTML = html || `<div style="grid-column:1/-1;text-align:center;padding:32px 0;color:var(--grey);font-size:14px">${isEn?'No categories match your search.':'Nenhuma categoria encontrada.'}</div>`;
