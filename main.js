  ...
  function _showIosTooltip(isEn) {
    ...
    const steps = isEn
      ...
      : ['Toca no botão <strong>⬆ Partilhar</strong> no fundo do Safari',
         'Toca em <strong>"Instalar app"</strong>',
         'Toca em <strong>"Instalar"</strong> — pronto! 🚀'];
...
function applyLang(){
  ...
  // Update PWA install banner language
  const _pwaBannerText = document.getElementById('pwaBannerText');
  const _pwaBtn = document.getElementById('pwaInstallBtn');
  const _pwaBanner = document.getElementById('pwaInstallBanner');
  if(_pwaBanner && !_pwaBanner.classList.contains('hidden')) {
    if(_pwaBannerText) _pwaBannerText.innerHTML = isEn ? 'Add <strong>GigHub</strong> to your home screen — quick access, no browser needed.' : 'Adiciona o <strong>GigHub</strong> ao ecrã inicial — acesso rápido, sem browser.';
    if(_pwaBtn) _pwaBtn.textContent = isEn ? 'Install' : 'Instalar';
  }
}
