...
function calcEarnings(){
  ...
  const sPT={
    2:'AttaPoll + Netsonda + YouGov. Para ganhos extra junta Freecash ou ySense (Get-Paid-To).',
    6:'Uber Eats (Courier), Glovo ou Bolt Food (Courier) — alta procura nas cidades PT.',
    7:'DataAnnotation.tech + Outlier + Clickworker. Nota: Outlier tem entrada seletiva — candidata-te enquanto exploras Clickworker.',
    8:'Rover + Babysits + Yoopies — combinar pet sitting e babysitting maximiza as horas.',
    10:'Superprof + Preply + Cambly (tutoria/idiomas) ou TaskRabbit + Merytu + Moço de Recados (biscates locais).',
    18:'Prolific + UserInterviews + Respondent.io — estudos e entrevistas bem pagos, disponibilidade limitada em PT.'
  };
  const sEN={
    2:'AttaPoll + Netsonda + YouGov. Add Freecash or ySense (Get-Paid-To) for extra earnings.',
    6:'Uber Eats (Courier), Glovo or Bolt Food (Courier) — high demand in PT cities.',
    7:'DataAnnotation.tech + Outlier + Clickworker. Note: Outlier has selective entry — apply while exploring Clickworker.',
    8:'Rover + Babysits + Yoopies — combining pet sitting and babysitting maximises hours.',
    10:'Superprof + Preply + Cambly (tutoring/languages) or TaskRabbit + Merytu + Moço de Recados (local gigs).',
    18:'Prolific + UserInterviews + Respondent.io — well-paid studies and interviews, limited availability in PT.'
  };
...
function renderRatings(p){
  ...
  // ── Earnings type — driven solely by `realistic` (payout is neutralised at DB level) ──
  // reliable: realistic >= 4 | variable: realistic <= 2 | neutral (3): no tag shown
  if(r.realistic >= 4) tags += `<span class="rtag rtag-realistic-high">⚡ ${isEn?'Reliable earnings':'Ganhos estáveis'}</span>`;
...
const translations = {
  pt: {
    ...
    navGuia: 'Guia',
    footerText: '<strong>GigHub</strong> · Agregador curado de oportunidades de trabalho · Sem afiliações pagas',
    ...
    monoDesc: 'Envia 24,99€ por MB Way para <strong>938 556 803</strong> e envia o comprovativo via WhatsApp. Recebes a chave de acesso em minutos.',
  ...
  en: {
    ...
    monoDesc: 'Send €24.99 via MB Way to <strong>938 556 803</strong> and share the receipt via WhatsApp. You\'ll receive the access key in minutes.',
...
function applyLang(){
  ...
  // Tabs — translate all group labels
  const _tabGroupLabels = TAB_GROUP_LABELS[currentLang] || TAB_GROUP_LABELS.pt;
  document.querySelectorAll('.tab[data-v]').forEach(tab => {
    const v = tab.dataset.v;
    const label = _tabGroupLabels[v];
    if(label !== undefined) {
      // Preserve the colour dot and count badge inside the tab
  ...
  const _boostOpenBtn = document.getElementById('boostOpenFormBtn');
  if(_boostOpenBtn) _boostOpenBtn.textContent = currentLang==='en'
...
function _fmt(r){
  ...
  return {
    ...
    pt:r.pt, eu:r.eu, url: urlOv || r.url||'',
  ...
  _sessionTimeoutInterval = setInterval(() => {
    ...
    if(idle > 6600000 && !_timeoutWarned) { // 110 min — warn 10 min before
      ...
      if(!toast) {
        ...
        toast.style.cssText = 'position:fixed;bottom:24px;left:50%;transform:translateX(-50%);background:var(--ink);color:var(--paper);font-family:\'Instrument Sans\',sans-serif;font-size:13px;padding:10px 20px;border-radius:10px;z-index:9800;box-shadow:0 4px 20px rgba(0,0,0,.4);border:1px solid rgba(201,168,76,.3);white-space:nowrap;';
...
(function _bindEvents(){
  ...
  if(grid) grid.addEventListener('click', function(e) {
    ...
    if(clearBtn) {
      ...
      const _geoEl = document.getElementById('fGeo'); if(_geoEl) _geoEl.value = '';
      const _wtEl = document.getElementById('fWorkType'); if(_wtEl) _wtEl.value = '';
      const _sortEl = document.getElementById('fSort'); if(_sortEl) _sortEl.value = 'earn';
  ...
  if(boostOpenFormBtn) boostOpenFormBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      // If RPC doesn't exist yet, fail open with a warning (remove this fallback once RPC is deployed)
      console.warn('[GigBoost] validate_boost_token RPC not found — allowing form open without validation.');
    }
  ...
  (async function() {
    ...
    // Admin UI: only shown if #admin= matches expected PBKDF2-derived key.
    // PBKDF2(passphrase, salt="gh-admin-2026", iterations=100000, SHA-256) → hex
    // To rotate: pick a new passphrase, compute new hash offline and update below.
    const _ADMIN_HASH = '4e8b7b493f615434b909c3862e39d613c353e9e4f55e0740ca6a7bc5d2c02a09';
    const _ADMIN_SALT = 'gh-admin-2026';
    const adminParam = urlParams.get('admin');
    if(adminParam) {
      try {
        const _keyMat = await crypto.subtle.importKey('raw', new TextEncoder().encode(adminParam), 'PBKDF2', false, ['deriveBits']);
        const _bits = await crypto.subtle.deriveBits(
          { name:'PBKDF2', salt:new TextEncoder().encode(_ADMIN_SALT), iterations:100000, hash:'SHA-256' },
          _keyMat, 256
        );
        const h = Array.from(new Uint8Array(_bits)).map(x=>x.toString(16).padStart(2,'0')).join('');
        if(h === _ADMIN_HASH) { showAdminLogin(); return; }
      } catch(e) { /* PBKDF2 not available — fail silently */ }
    }
