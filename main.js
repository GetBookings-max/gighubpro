...
function renderRatings(p){
  ...
  if(p.direct_hire){
    ...
    return `<div class="ratings-row">${directTag}${highlightTag}</div>`;
  }
...
function applyLang(){
  ...
  setText2('canaisCatFacebook',      '👥 Grupos & Comunidades',    '👥 Groups & Communities');
...
// _LOCAL_EXTRA_PLATFORMS: migradas para Supabase via gighub_supabase_setup.sql
// Array mantido vazio — a lógica de injecção abaixo torna-se no-op.
...
const _ALLOWED_PLATFORMS = new Set([
  ...
  'blueclinical',
  // 🎧 Atendimento & Suporte
  'teleperformance','concentrix','foundever','accenture careers','indiecampers',
]);
...
(function _bindEvents(){
  ...
  // ── Filter boxes (stable/variable/all) — listeners replaced inline onclick removed for CSP ──
