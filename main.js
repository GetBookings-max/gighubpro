...
// Android/Chrome: intercepts beforeinstallprompt → botão "Instalar"
// iOS/Safari:     beforeinstallprompt não existe → instruções manuais Share → A2HS
// Storage key: 'gh_pwa_dismissed' — set when the user dismisses the banner.
...
(function _initPWA() {
  ...
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', function () {
      navigator.serviceWorker.register('/gighubpro/sw.js', {scope: '/gighubpro/'}).then(function(reg){ window._swOk=true; window._swScope=reg.scope; }).catch(function(e){ window._swOk=false; window._swErr=e.message; });
    });
  }
  ...
  const _isInStandaloneMode = ('standalone' in navigator && navigator.standalone) ||
  ...
  // ── Show banner ──────────────────────────────────────────────────────────
  window._showPwaInstallBanner = function () {
    // Never show if already installed or already dismissed
    if (_isInStandaloneMode) return;
    try { if (localStorage.getItem('gh_pwa_dismissed')) return; } catch (_) {}
    // iOS: show if on Safari; Android: show only when prompt is ready
    if (!_isIOS && !_deferredPrompt) return;

    const banner  = document.getElementById('pwaInstallBanner');
    const textEl  = document.getElementById('pwaBannerText');
    const btnEl   = document.getElementById('pwaInstallBtn');
    if (!banner) return;

    const isEn = typeof currentLang !== 'undefined' && currentLang === 'en';

    if (_isIOS) {
      if (textEl) textEl.innerHTML = isEn
        ? 'Add <strong>GigHub</strong> to your home screen.'
        : 'Adiciona o <strong>GigHub</strong> ao ecrã inicial.';
      if (btnEl) { btnEl.style.display = ''; btnEl.textContent = isEn ? 'How?' : 'Como?'; }
    } else {
      if (textEl) textEl.innerHTML = isEn
        ? 'Add <strong>GigHub</strong> to your home screen — quick access, no browser needed.'
        : 'Adiciona o <strong>GigHub</strong> ao ecrã inicial — acesso rápido, sem browser.';
      if (btnEl) { btnEl.style.display = ''; btnEl.textContent = isEn ? 'Install' : 'Instalar'; }
    }

    banner.classList.remove('hidden');
    const btt = document.getElementById('backToTop');
    if (btt) btt.style.bottom = (banner.offsetHeight + 12) + 'px';
  };
  ...
  document.addEventListener('click', function (e) {
    ...
    if (_isIOS) {
      const isEn = typeof currentLang !== 'undefined' && currentLang === 'en';
      _showIosTooltip(isEn);
      return;
    }

    // Android/Chrome: native install prompt
    if (!_deferredPrompt) {
      // beforeinstallprompt not fired yet — show Chrome menu fallback
      const isEn = typeof currentLang !== 'undefined' && currentLang === 'en';
      _showAndroidFallback(isEn);
      return;
    }
    ...
    try {
      ...
      _deferredPrompt.userChoice.then(function (choice) {
        ...
        _hidePwaInstallBanner();
        if (choice && choice.outcome === 'accepted') {
          try { localStorage.setItem('gh_pwa_dismissed', '1'); } catch (_) {}
        }
      }).catch(function (err) {
        _deferredPrompt = null;
        btn.textContent = '⚠️ ' + (err && err.message ? err.message.slice(0,30) : 'Erro');
        btn.disabled = false;
      });
    } catch (err) {
      btn.textContent = '⚠️ ' + (err && err.message ? err.message.slice(0,30) : 'Erro prompt');
      btn.disabled = false;
    }
  });

  // ── iOS tooltip: step-by-step floating bubble ────────────────────────────
  function _showIosTooltip(isEn) {
    ...
    tip.style.cssText = `
      ...
      z-index:9998; width:min(320px, 90vw);
    ...
    const steps = isEn
      ...
      : ['Toca no botão <strong>⬆ Partilhar</strong> no fundo do Safari',
         'Faz scroll e toca em <strong>"Adicionar ao ecrã inicial"</strong>',
         'Toca em <strong>"Adicionar"</strong> — pronto! 🚀'];

    tip.innerHTML = `
      <div style="font-weight:700;margin-bottom:12px;font-size:13px">
      ...
      <div style="text-align:right;margin-top:6px">
        <button id="pwaIosTooltipClose"
          style="font-size:12px;color:var(--grey);background:none;border:none;cursor:pointer;
            font-family:'Instrument Sans',sans-serif;padding:4px 0">
          ${isEn ? 'Got it' : 'Percebido'} ✓
        </button>
      </div>
      <div style="position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);
        width:14px;height:8px;overflow:hidden">
        <div style="width:14px;height:14px;background:var(--paper);
          border-right:1px solid var(--border-md);border-bottom:1px solid var(--border-md);
          transform:rotate(45deg);margin-top:-7px"></div>
      </div>`;
    ...
    setTimeout(function () {
      document.addEventListener('click', function _closeTip(ev) {
        if (!tip.contains(ev.target) || ev.target.id === 'pwaIosTooltipClose') {
          tip.remove();
  ...
  // ── Android fallback: Chrome menu method ────────────────────────────────
  function _showAndroidFallback(isEn) {
    const existing = document.getElementById('pwaAndroidTooltip');
    if (existing) { existing.remove(); return; }

    const tip = document.createElement('div');
    tip.id = 'pwaAndroidTooltip';
    tip.style.cssText = `
      position:fixed; bottom:80px; left:50%; transform:translateX(-50%);
      z-index:9998; width:min(320px, 90vw);
      background:var(--paper); color:var(--ink);
    ...
    const steps = isEn
      ? ['Open the <strong>⋮ Chrome menu</strong> (top right)',
         'Tap <strong>"Add to Home Screen"</strong>',
         'Tap <strong>"Add"</strong> — done! 🚀']
      : ['Abre o <strong>⋮ menu do Chrome</strong> (canto superior direito)',
         'Toca em <strong>"Adicionar ao ecrã inicial"</strong>',
         'Toca em <strong>"Adicionar"</strong> — pronto! 🚀'];

    tip.innerHTML = `
      <div style="font-weight:700;margin-bottom:12px;font-size:13px">
        ${isEn ? '📲 Install on Android' : '📲 Instalar no Android'}
      </div>
      ${steps.map((s, i) => `
        <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:9px">
          <span style="flex-shrink:0;width:20px;height:20px;border-radius:50%;
      ...
      <div style="text-align:right;margin-top:6px">
        <button id="pwaAndroidTooltipClose"
          style="font-size:12px;color:var(--grey);background:none;border:none;cursor:pointer;
            font-family:'Instrument Sans',sans-serif;padding:4px 0">
          ${isEn ? 'Got it' : 'Percebido'} ✓
        </button>
    ...
    setTimeout(function () {
      document.addEventListener('click', function _closeTip(ev) {
        if (!tip.contains(ev.target) || ev.target.id === 'pwaAndroidTooltipClose') {
          tip.remove();
          document.removeEventListener('click', _closeTip);
        }
      });
  ...
  function _hidePwaInstallBanner() {
    const banner = document.getElementById('pwaInstallBanner');
    if (banner) banner.classList.add('hidden');
    const btt = document.getElementById('backToTop');
    if (btt) btt.style.bottom = '80px';
  }
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    // Show PWA install banner if the browser has already queued a beforeinstallprompt
    setTimeout(async () => {
      if(typeof window._showPwaInstallBanner === 'function') window._showPwaInstallBanner();
      var lines = [];
      try {
        var mRes = await fetch('/gighubpro/manifest.json');
        var mJson = await mRes.json();
        lines.push('manifest: ' + (mRes.ok?'✅':'❌'));
        lines.push('start_url: ' + mJson.start_url);
        lines.push('id: ' + (mJson.id||'❌ em falta'));
        lines.push('screenshots: ' + (mJson.screenshots?'✅':'❌ em falta'));
        var i1 = await fetch('/gighubpro/icon-192.png',{method:'HEAD'});
        var i2 = await fetch('/gighubpro/icon-512.png',{method:'HEAD'});
        lines.push('icon-192: '+(i1.ok?'✅':'❌'));
        lines.push('icon-512: '+(i2.ok?'✅':'❌'));
      } catch(e){ lines.push('erro: '+e.message); }
      lines.push('SW: '+(window._swOk?'✅ '+window._swScope:'❌ '+(window._swErr||'pendente')));
      lines.push(window._pwaPromptCaptured?'✅ prompt capturado':'❌ prompt NÃO disparou');
      var d=document.createElement('div');
      d.style.cssText='position:fixed;bottom:70px;left:12px;right:12px;z-index:9999;background:#111;color:#fff;font-size:12px;padding:10px 12px;border-radius:8px;border:1px solid #444;font-family:monospace;line-height:2';
      d.innerHTML=lines.join('<br>')+'<br><small style="color:#888">toca para fechar</small>';
