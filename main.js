...
function renderRatings(p){
  ...
  if(p.direct_hire){
    ...
    const highlightTag = p.suggested
  ...
  // entry_level vem da DB; fallback: easy score > categoria
  let entryLevel = p.entry_level;
  if(!entryLevel) {
    if(p.easy <= 2) entryLevel = 'rigoroso';
    else if(p.easy >= 4) entryLevel = 'facil';
    else entryLevel = _CAT_ENTRY_FALLBACK[p.cat] || 'facil';
  }
...
const curationFilters = {
  portugal: p => p.suggested && p.pt === true,
...
// _LOCAL_EXTRA_PLATFORMS: NOS Cinemas migrado para a DB via migration_platform_metadata.sql
// Manter array vazio para não quebrar referências existentes ao código de injecção.
