...
function renderRatings(p){
  ...
  if(p.direct_hire){
    ...
    return `<div class="ratings-row">${highlightTag}</div>`;
  }
...
function setTab(v){
  ...
  const _wtRowTab = document.getElementById('wtRow');
  if(_wtRowTab) _wtRowTab.style.display = 'flex';
...
function render(){
  ...
  const _sTotalEl=document.getElementById('s-total'); if(_sTotalEl) _sTotalEl.textContent=P.length||'—';
...
function setCuration(key) {
  ...
  // Show work-type sub-filter only for "Qualquer formato" (key='')
  const _wtRow = document.getElementById('wtRow');
  if(_wtRow) _wtRow.style.display = key === '' ? 'flex' : 'none';
  render();
}
...
function setWorkType(val) {
  ...
  document.querySelectorAll('.wt-pill[data-wt]').forEach(el => {
...
const translations = {
  pt: {
    ...
    guideTip: '<strong>★ Top 3 para Portugal em 2026 —</strong> <strong>AttaPoll</strong> (app de surveys, simples, paga por PayPal, disponível em PT) · <strong>Toloka AI</strong> (micro-tarefas de IA, boa disponibilidade PT, entrada fácil) · <strong>Amnistia Internacional</strong> (angariação de fundos paga, entrada fácil, ótima para começar)',
  ...
  en: {
    ...
    guideTip: '<strong>★ Top 3 for Portugal in 2026 —</strong> <strong>AttaPoll</strong> (survey app, simple, pays via PayPal, available in PT) · <strong>Toloka AI</strong> (AI micro-tasks, good PT availability, easy entry) · <strong>Amnistia Internacional</strong> (paid fundraising, easy entry, great to get started)',
...
function applyLang(){
  ...
  setInner2('canaisNosDesc',       'Recrutamento directo para trabalho part-time nos cinemas NOS em Portugal.',
                                   'Direct recruitment for part-time work at NOS Cinemas in Portugal.');
  setInner2('canaisCinemaCityDesc','Vagas part-time nas salas Cinema City — atendimento, bilheteira e operações de cinema.',
                                   'Part-time vacancies at Cinema City — front-of-house, ticketing and cinema operations.');
  setInner2('canaisPubCrawlDesc',  'Trabalha como guia ou staff no circuito de pub crawl de Lisboa — ideal para quem gosta de turismo e vida noturna.',
                                   'Work as a guide or staff on the Lisbon pub crawl circuit — ideal for those who enjoy tourism and nightlife.');
  setInner2('canaisNavDesc',       'Vagas na empresa de navegação aérea de Portugal — controlo de tráfego e funções de apoio.',
                                   'Vacancies at Portugal\'s air navigation company — air traffic control and support roles.');
  setInner2('canaisRyanairDesc',   'Vagas de tripulação de cabine e handling em Portugal — uma das maiores empregadoras de aviação low-cost da Europa.',
                                   'Cabin crew and ground handling vacancies in Portugal — one of Europe\'s largest low-cost aviation employers.');
  setInner2('canaisDouroAzulDesc', 'Oportunidades de trabalho em cruzeiros e turismo fluvial no Douro — staff de bordo, animação e restauração.',
                                   'Work opportunities in river cruises and tourism on the Douro — on-board staff, entertainment and catering.');
  setInner2('canaisLisbonBoatsDesc','Trabalho sazonal em passeios de barco em Lisboa — tripulação, guias e staff de atendimento.',
                                   'Seasonal work on boat tours in Lisbon — crew, guides and front-of-house staff.');
  // Calculator
  ...
  if(fboxAll) fboxAll.textContent = isEn ? '💼 All formats' : '💼 Qualquer formato';
...
(function _bindEvents(){
  ...
  // ── Filter boxes (curation) ──
  document.querySelectorAll('.fbox[data-fbox]').forEach(el => {
    el.addEventListener('click', () => setCuration(el.dataset.fbox));
    el.addEventListener('keydown', e => { if(e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setCuration(el.dataset.fbox); } });
  });
  // ── Work type sub-filter (Remote / Presencial) ──
  document.querySelectorAll('.wt-pill[data-wt]').forEach(el => {
    el.addEventListener('click', () => setWorkType(el.dataset.wt));
  });
  ...
  if(grid) grid.addEventListener('click', function(e) {
    ...
    if(clearBtn) {
      ...
      // Reset work type pills
      activeWorkType = '';
      const _fwtClear = document.getElementById('fWorkType'); if(_fwtClear) _fwtClear.value = '';
      document.querySelectorAll('.wt-pill[data-wt]').forEach(el => el.classList.toggle('active', el.dataset.wt === ''));
      // Reset filter boxes
      ['fboxAll','fboxStable','fboxVariable'].forEach(id => {
        const el = document.getElementById(id);
