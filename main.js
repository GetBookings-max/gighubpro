...
function renderRatings(p){
  ...
  let entryLevel;
  if(_RIGOROSO_PLATFORMS.has(p.name) || p.easy <= 2) {
    entryLevel = 'rigoroso';
  } else if(_MODERADO_PLATFORMS.has(p.name)) {
    entryLevel = 'moderado';
  } else if(p.easy >= 4) {
    entryLevel = 'facil';
  } else {
...
const _CAT_OVERRIDES = {
  ...
  'Worldpackers':       'f2f',
  // Casamentos: canonical category = gigs_events (presential work), not split across two categories
  'Casamentos':         'gigs',
};
