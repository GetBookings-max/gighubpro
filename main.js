...
function admRender(){
  ...
  <div style="background:var(--green-pale);border:1.5px solid rgba(45,122,79,.3);border-radius:12px;padding:20px 18px;margin-bottom:16px">
    ...
    ${sheetUrl
      ? `<a href="${sheetUrl}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:8px;height:40px;padding:0 18px;border-radius:8px;background:var(--green);color:#fff;text-decoration:none;font-size:13px;font-weight:700">
           📊 Abrir Google Sheet →
         </a>`
...
// ══════════════════════════════════════════════════════════════════
// EVENT LISTENERS — direct bindings, no wrapper scope issues
// ══════════════════════════════════════════════════════════════════
function _bind(id, evt, fn){
  var el = document.getElementById(id);
  if(el) el.addEventListener(evt, fn);
}
function _bindQ(sel, evt, fn){
  var el = document.querySelector(sel);
  if(el) el.addEventListener(evt, fn);
}

// Lock screen
_bind('lockInput',    'keydown', function(e){ if(e.key==='Enter') window.unlock(); });
_bind('lockBtn',      'click',   function(){ window.unlock(); });
_bind('lockLangBtn',  'click',   toggleLockLang);
_bind('lockPrivacyLink','click', function(e){ e.preventDefault(); openLegalModal('privacy'); });

// Nav
_bind('langToggle',  'click', toggleLang);
_bind('favBtn',      'click', toggleFavView);
_bind('calcIconBtn', 'click', openCalc);
_bind('boostNavBtn', 'click', openBoostPay);

// Toolbar
_bind('search', 'input',  render);
_bind('fGeo',   'change', render);
_bind('fSort',  'change', render);

// Calc modal
_bind('calcModalCloseBtn', 'click', function(){ document.getElementById('calcModal').style.display='none'; });
_bind('hoursRange', 'input',  calcEarnings);
_bind('calcType',   'change', calcEarnings);

// Welcome modal
_bind('welcomeCloseBtn', 'click', function(){ document.getElementById('welcomeModal').style.display='none'; });
_bind('boostCtaEl',      'click', function(){ openBoostPay(); document.getElementById('welcomeModal').style.display='none'; });

// Boost pay modal
_bind('boostPayModalCloseBtn', 'click', function(){ document.getElementById('boostPayModal').style.display='none'; });
_bind('boostOpenFormBtn',      'click', function(){ document.getElementById('boostPayModal').style.display='none'; openBoostForm(); });

// Back to top
_bind('backToTop', 'click', function(){ window.scrollTo({top:0,behavior:'smooth'}); });

// Admin panel
_bind('admAddBoostTokenBtn', 'click', admAddBoostToken);
_bind('boostCopyBtn',        'click', admCopyBoostCode);
_bind('admDownloadBoostBtn', 'click', function(){ if(typeof admDownloadWithBoost==='function') admDownloadWithBoost(); });
...
// Curation pills delegation
(function(){
  var pills = document.getElementById('curationPills');
  if(pills) pills.addEventListener('click', function(e){
    var pill = e.target.closest ? e.target.closest('.curation-pill') : null;
    if(pill) setCuration(pill.dataset.curation);
  });
})();

// Grid delegation (fav buttons, clear filters)
(function(){
  var grid = document.getElementById('grid');
  if(grid) grid.addEventListener('click', function(e){
    var favBtn = e.target.closest ? e.target.closest('.fav-btn') : null;
    if(favBtn){ e.stopPropagation(); toggleFav(favBtn.dataset.name, e); return; }
    var clearBtn = e.target.closest ? e.target.closest('.clear-filters-btn') : null;
    if(clearBtn){
      document.getElementById('search').value='';
      activeTab=''; activeCuration='';
      document.querySelectorAll('.tab').forEach(function(t){ t.classList.toggle('active', t.dataset.v===''); });
      document.querySelectorAll('.curation-pill').forEach(function(p){ p.classList.toggle('active', p.dataset.curation===''); });
      render();
    }
  });
})();

// Boost form box delegation
(function(){
  var box = document.getElementById('boostFormBox');
  if(!box) return;
  box.addEventListener('change', function(e){
    var sel = e.target.closest ? e.target.closest('select.boost-select') : null;
    if(sel){ boostAnswers[sel.dataset.key]=sel.value; boostCheckNext(); }
  });
  box.addEventListener('click', function(e){
    var r=e.target.closest?e.target.closest('.boost-radio'):null;     if(r){boostSelectRadio(r.dataset.key,r.dataset.val,r);return;}
    var c=e.target.closest?e.target.closest('.boost-chip'):null;      if(c){boostToggleChip(c);return;}
    if(e.target.closest?e.target.closest('.boost-btn-back'):null)     {boostBack();return;}
    if(e.target.closest?e.target.closest('.boost-btn-next'):null)     {boostNext();return;}
    if(e.target.closest?e.target.closest('.boost-btn-ai'):null)       {submitBoostForm();return;}
    if(e.target.closest?e.target.closest('.boost-close-modal-btn'):null){document.getElementById('boostFormModal').style.display='none';return;}
    if(e.target.closest?e.target.closest('.boost-redo-btn'):null)     {boostStep=0;renderBoostStep();return;}
    if(e.target.closest?e.target.closest('.boost-explore-btn'):null)  {document.getElementById('boostFormModal').style.display='none';return;}
    if(e.target.closest?e.target.closest('.boost-retry-btn'):null)    {openBoostForm();return;}
  });
})();

// Legal modal
(function(){
  var lm = document.getElementById('legalModal');
  if(lm) lm.addEventListener('click', function(e){ if(e.target===lm) lm.style.display='none'; });
  _bind('legalModalCloseBtn','click',function(){ document.getElementById('legalModal').style.display='none'; });
})();

// Global data-modal delegation (footer links + lock privacy link)
document.addEventListener('click', function(e){
  var link = e.target.closest ? e.target.closest('[data-modal]') : null;
  if(link){ e.preventDefault(); openLegalModal(link.dataset.modal); }
});

// Admin list delegation
(function(){
  var al = document.getElementById('adm-list');
...
// Welcome & calc modal backdrop close
['welcomeModal','calcModal'].forEach(function(id){
  var el=document.getElementById(id);
...
window.addEventListener('scroll', function(){
  ...
  var tw=document.getElementById('tabsWrap');
  if(!tw) return;
  var y=window.scrollY;
  if(y>_tabScrollLastY&&y>120) tw.classList.add('collapsed');
  else tw.classList.remove('collapsed');
  _tabScrollLastY=y;
},{passive:true});
...
(function(){
  ...
  var yr=document.getElementById('heroYear');
  if(yr) yr.textContent=new Date().getFullYear();
  render();
  // Auto-unlock from URL hash
  (async function(){
    var t=new URLSearchParams(window.location.hash.slice(1)).get('key');
