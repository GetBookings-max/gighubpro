...
(function _initPWA() {
  ...
  window._showPwaInstallBanner = function () {
    ...
    // iOS: always show; Android: show even without native prompt (fallback instructions)
  ...
  function _showAndroidFallback(isEn) {
    ...
    tip.innerHTML = `
      <div style="font-weight:700;margin-bottom:10px;font-size:13px">
        ${isEn ? '📲 Install on Android (Chrome)' : '📲 Instalar no Android (Chrome)'}
      </div>
      <div style="background:rgba(201,168,76,.12);border:1px solid rgba(201,168,76,.3);border-radius:8px;padding:9px 11px;margin-bottom:12px;font-size:12px;line-height:1.6">
        ${isEn
          ? '💡 Look for an <strong>install icon (⊕)</strong> in Chrome's address bar and tap it — fastest way.'
          : '💡 Procura o <strong>ícone instalar (⊕)</strong> na barra de endereço do Chrome e toca — é o mais rápido.'}
      </div>
      <div style="font-size:11px;color:var(--grey);margin-bottom:8px;text-transform:uppercase;letter-spacing:.05em">
        ${isEn ? 'Or via the menu:' : 'Ou pelo menu:'}
      </div>
      ${[
        isEn ? 'Tap <strong>⋮</strong> (top-right of Chrome)' : 'Toca no <strong>⋮</strong> (canto sup. direito do Chrome)',
        isEn ? 'Tap <strong>"Install app"</strong>' : 'Toca em <strong>"Instalar app"</strong>',
        isEn ? 'Tap <strong>"Install"</strong> — done! 🚀' : 'Toca em <strong>"Instalar"</strong> — pronto! 🚀'
      ].map((s, i) => `
        <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:9px">
          <span style="flex-shrink:0;width:20px;height:20px;border-radius:50%;
            background:var(--ink);color:var(--paper);
...
async function validarTokenSupabase(token, fromUrl) {
  ...
  try {
    ...
    setTimeout(() => { if(typeof window._showPwaInstallBanner === 'function') window._showPwaInstallBanner(); }, 1200);
